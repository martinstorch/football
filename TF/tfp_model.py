...
if __name__ == "__main__":
    ...
    #         tf.keras.layers.InputLayer(input_shape=[x.shape[1]], dtype=tf.float64),  #
    #         tf.keras.layers.Dense(5, kernel_initializer='normal', use_bias=True),
    #
    ...
    #                 np.random.rand(y.shape[1], num_inducing_points, 5) * 4 - 1
    #                 # ,
    #                 # np.stack([
    #                 #     np.stack(
    #                 #         [np.linspace(start=-10,  # +i*0.5-j*0.333,
    #                 #                      stop=10,  # -i*0.5+j*0.333,
    #                 #                      # *x_range,
    #                 #                      num=num_inducing_points, dtype=x.dtype)  # [..., np.newaxis]
    #                 #          for j in range(5)],
    #                 #         axis=-1
    ...
    #                 tf.constant_initializer(np.array([0.14]).astype(x.dtype))
    #             ),
    #         )
    #         ,
    #         # tfp.layers.DistributionLambda(
    #         #     make_distribution_fn=lambda t: tfb.Exp()(t) #, #   tfd.Poisson(tf.exp(t)),
    #         #     #convert_to_tensor_fn=lambda s: s.sample(5)
    #         # )
    #         #lambda l: tfp.distributions.Poisson(log_rate=l)
    #         tfp.layers.IndependentPoisson(y.shape[1])
    #
    #         # tfp.distributions.poisson_lognormal()
    #     ])
    #     poisson = tfp.layers.IndependentPoisson(y.shape[1])(
    #         tf.keras.layers.Dense(y.shape[1], kernel_initializer='normal', use_bias=True, activation=tf.exp)(
    #             m.layers[0].output)
    #     )
    #     # outdist = tfb.Exp()
    #     # newmodel = tf.keras.Model(inputs=m.inputs, outputs=outdist(m.outputs[0]), name="exp_model")
    #     # outlayer = tfp.layers.DistributionLambda(
    #     #     make_distribution_fn=lambda t: tfb.Exp(t) #, #   tfd.Poisson(tf.exp(t)),
    #     # )
    #     # return newmodel
    #     # print("model outputs")
    ...
    def build_model(x, y):
        ...
        vgp = tfp.layers.VariationalGaussianProcess(
                ...
                unconstrained_observation_noise_variance_initializer=(
                    tf.constant_initializer(np.array([0.0014]).astype(x.dtype))
                ),
            ) (dense)
        poisson = tfp.layers.IndependentPoisson(y.shape[1])(vgp)

        newmodel = tf.keras.Model(inputs=inputLayer, outputs=[vgp, poisson, dense], name="exp_model")
        return newmodel
    ...
    #     batch_size = 32
    #     num_training_points_ = x.shape[0]
    #     vgp = model.layers[1]
    #     #@tf.function
    ...
    #         x_train_batch = x[batch_idxs, ...]
    #         y_train_batch = y[batch_idxs]
    ...
    for i in range(num_samples):
        ...
        plt.scatter(Y_test[test_range, 0],
                    sample_[..., 0],
                    c=Y_test[test_range, 0],
                    alpha=0.1,
                    );
    plt.show()
    # plt.close()
