    ...
    if __name__ == "__main__":
        ...
        regularizer2 = tf.keras.regularizers.L1L2(l2=0.0003)
        ...
        def make_joint_mixture_model(x):
            def make_outputs(l1w_in, l1w_out, l2w_in, l2w_out, w_in, w_out, sm_in, sm_out, ): #tfd.Sample(
               ...
               return tfd.JointDistributionNamed(dict(
                   mainresult = tfd.Independent(tfd.OneHotCategorical(logits=l), reinterpreted_batch_ndims=0),
                   sidebets = tfd.Independent(tfd.Poisson(log_rate=x2), reinterpreted_batch_ndims=1),
                   softmax = tfd.Independent(tfd.OneHotCategorical(logits=sml), reinterpreted_batch_ndims=0)
               ))
            sc = 2.0
        ...
        def loss_function(l1w_in, l1w_out, l2w_in, l2w_out, w_in, w_out, sm_in, sm_out, X, Y, Y2): # weight_mean, weight_scale,
            ...
            rl_l2 = regularizer2(l2weights)
            rl_w = regularizer2(weights)
            rl_sm = regularizer2(smweights)
            ...
            actual_points = tf.clip_by_value(predprob, 0.0, 1.0) * tf.cast(achievable_points, tf.float64)#[...,tf.newaxis, :] # [1,mixcom,1]
            ...
            softmax_lp = pred.parameters["model"]["softmax"].log_prob(Y)
        ...
        import functools
        forward_kl_loss = functools.partial(tfp.vi.monte_carlo_variational_loss, discrepancy_fn=tfp.vi.kl_forward,use_reparameterization=True)
        reverse_kl_loss = functools.partial(tfp.vi.monte_carlo_variational_loss, discrepancy_fn=tfp.vi.kl_reverse,use_reparameterization=True)
