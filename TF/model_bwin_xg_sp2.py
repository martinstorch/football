...
from Estimators import Model_BWIN_xg_sp2 as themodel
...
def load_file(model_dir, file_name):
  new_data = pd.DataFrame()
  cur_dir = os.path.dirname(model_dir + "/" + file_name) # Dir from where search starts 
  
  while True:
    file_list = os.listdir(cur_dir)
    parent_dir = os.path.dirname(cur_dir)
    if file_name in file_list:
      print ("Using " + cur_dir + "/" + file_name +" as input")
      new_data =  pd.read_csv(
          tf.gfile.Open(cur_dir + "/" + file_name),
        skipinitialspace=True,
        engine="python",
        skiprows=0)
      break
      ...
      if cur_dir == parent_dir: #if dir is root dir
        raise Exception(cur_dir+"/" + file_name+" not found")
      else:
        cur_dir = parent_dir  
  return new_data
