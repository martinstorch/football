...
if __name__ == "__main__":
    ...
    def make_joint_mixture_model(x):
        def make_outputs(weights, smweights, l2weights): #tfd.Sample(
           # #weights = tf.where(tf.equal(tf.rank(weights),2), weights[tf.newaxis, ...], weights)  # enforce rank 3
           # l2weights = tf.cond(tf.equal(tf.rank(l2weights),2), lambda: l2weights[tf.newaxis, ...], lambda: l2weights)  # enforce rank 3
           # smweights = tf.cond(tf.equal(tf.rank(smweights), 2), lambda: smweights[tf.newaxis, ...], lambda: smweights)  # enforce rank 3
           # weights = tf.cond(tf.equal(tf.rank(weights),2), lambda: weights[tf.newaxis, ...], lambda: weights)  # enforce rank 3

           #l = tf.matmul(x[tf.newaxis, ...], weights[:-1]) + weights[-1:]
           # Batch matrix multiplication
           l = tf.einsum('...ij,...jk->...ik', x, weights[...,:-1,:]) + weights[...,-1:,:] # out[a,i,k] = sum_j s[a,i,j] * t[a, j, k]
           #l2 = tf.matmul(tf.math.tanh(l), l2weights[:-1]) + l2weights[-1:]
           l2 = tf.einsum('...ij,...jk->...ik', tf.math.tanh(l), l2weights[...,:-1,:]) + l2weights[...,-1:,:]
           multiples = tf.tensor_scatter_nd_update(tensor = tf.ones(dtype=tf.int32, shape=[tf.rank(smweights)+1]),
                                                   indices = [[tf.rank(smweights)-2]],
                                                   updates = [x.shape[0]])
           compdist_logits = 100.0*tf.tile(smweights[..., tf.newaxis,:,:], multiples = multiples)
           #p = (tf.stack([1-(x[:,1]+1)*0.5, 0.5*(1+x[:,1])], axis=1))
           return (#tfd.Sample(
               tfd.Independent(
               tfd.MixtureSameFamily(
               mixture_distribution=tfd.Categorical(logits = l2*10.0),
               ...
               components_distribution=tfd.OneHotCategorical(logits=compdist_logits) # batch
        ...
        # def make_batch(batch_dist):
        #     return tfd.Sample(tfd.Categorical(logits=batch_dist), sample_shape=batch_size)


        return make_outputs, tfd.JointDistributionNamed(dict(
            weight_mean=tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[x.shape[1] + 1, l1dim], dtype=tf.float64),
                                                   scale=tf.ones(shape=[x.shape[1] + 1, l1dim], dtype=tf.float64)),
                                        reinterpreted_batch_ndims=2),
            weight_scale=tfd.Independent(
                tfd.LogNormal(loc=tf.zeros(shape=[x.shape[1] + 1, l1dim], dtype=tf.float64),
                              scale=tf.ones(shape=[x.shape[1] + 1, l1dim], dtype=tf.float64)),
                reinterpreted_batch_ndims=2),
            weights=lambda weight_scale, weight_mean: tfd.Independent(
                tfd.Normal(loc=weight_mean, scale=1.0 * weight_scale), reinterpreted_batch_ndims=2),
            ...
            # batch_dist = tfd.Independent(tfd.Normal(loc=tf.ones(shape=[x.shape[0]], dtype=tf.float64),
            #                                        scale=0.0000001*tf.ones(shape=[x.shape[0]], dtype=tf.float64)), reinterpreted_batch_ndims=1),
            # batch = make_batch, #()tfd.Sample(tfd.Categorical(probs=tf.ones(x.shape[0], dtype=tf.float64)/x.shape[0], dtype=tf.int32), sample_shape=batch_size),
    ...
    joint_model.sample()["l2weights"]
    _ = joint_model.sample()["outputs"]
    _ = joint_model.sample(2)["outputs"]
    ...
    {s:v.shape for s,v in joint_model.sample(1).items()}
    {s:v.shape for s,v in joint_model.sample(2).items()}
    #empiricalDist = tfd.Empirical(empirical_results, event_ndims=0)  # [x.shape[0], d]



    batch_size = 1024
    def target_log_prob_cat_batched( weight_mean, weight_scale, weights, smweights, l2weights):
        ...
        _ , joint_model = make_joint_mixture_model(x0)
        ...
        return lp-tf.reduce_sum(reg_loss)


    # reinterpreted_batch_ndims=0),
    def target_log_prob_cat( weight_mean, weight_scale, weights, smweights, l2weights):
        lp = joint_model.log_prob({
            'weight_mean': weight_mean,
            'weight_scale': weight_scale,
            'weights': weights,
            ...
            'outputs': outputs_ #tf.gather(params=outputs_, indices=batch) # Y_train[:,0:2]
        })
        reg_loss = create_laplacian_loss(smweights, alpha=10.0)
        return tf.reduce_sum(lp)-tf.reduce_sum(reg_loss)

    def analyse_target_log_prob_cat( weight_mean, weight_scale, weights, smweights, l2weights):
        lp = joint_model.log_prob_parts(
            ...
            # 'batch_dist':batch_dist,
            # 'batch':batch,
            'outputs': outputs_# tf.gather(outputs_, batch) # Y_train[:,0:2]
        })
        reg_loss = create_laplacian_loss(smweights, alpha=10.0)
    ...
    def target_log_prob_cat_test(weight_mean, weight_scale, weights, smweights, l2weights): # , component_distribution
    ...
    sampler = tfp.mcmc.HamiltonianMonteCarlo(
            target_log_prob_fn=target_log_prob_cat_batched,
    ...
    initial_state = [#np.random.randn(x_train.shape[1]+1, mixcom), # np.zeros # weight_mean
                     tf.zeros(shape=[x_train.shape[1]+1, l1dim], dtype=tf.float64), # weight_mean
                     tf.ones(shape=[x_train.shape[1]+1, l1dim], dtype=tf.float64), # weight_scale
                     tf.ones(shape=[x_train.shape[1]+1, l1dim], dtype=tf.float64), # weights
        #np.ones(shape=[mixcom, 49]) / 49.
        tf.zeros(shape=[mixcom, 49], dtype=tf.float64),  # smweight
        tf.ones(shape=[l1dim+1, mixcom], dtype=tf.float64),  # l2weight
        #np.log(joint_model.sample()["component_distribution"]),
        #joint_model.sample()["component_distribution"]
        # joint_model.sample()["batch_dist"],
        # joint_model.sample()["batch"]
    ]  # compdist
    s = joint_model.sample(nchains)
    print(joint_model.log_prob(s))
    initial_state = [s['weight_mean'].numpy(), s['weight_scale'].numpy(), s['weights'].numpy(), s['smweights'].numpy(), s['l2weights'].numpy()] #, s['component_distribution'].numpy()]
    #initial_state = [s['weight_mean'], s['weight_scale'], s['weights'], s['smweights']] #, s['component_distribution'].numpy()]
    print(target_log_prob_cat_batched(*initial_state))
    # inputfilename = "mcmc_allstatesL2_20200902_084017.pickle"
    # filehandler = open(inputfilename, 'rb')
    # all_states = pickle.load(filehandler)
    # weight_mean, weight_scale, weights, smweights, l2weights = all_states
    # initial_state = [weight_mean[-1], weight_scale[-1], weights[-1], smweights[-1], l2weights[-1]]
    print("initial_state")
    ...
    filename = "mcmc_allstatesL2_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
    filehandler = open(filename, 'wb')
    pickle.dump(states.all_states, filehandler)

    weight_mean, weight_scale, weights, smweights, l2weights = states.all_states # , cdist
    current_state = [weight_mean[-1], weight_scale[-1], weights[-1], smweights[-1], l2weights[-1]]
    current_state2 = [weight_mean[-10], weight_scale[-10], weights[-10], smweights[-10], l2weights[-10]]
    print(target_log_prob_cat(*initial_state))
...
#        cdist[-1, 2]-cdist[0, 2]
    ...
    if True:
        loss_curve = [target_log_prob_cat(weight_mean[i], weight_scale[i], weights[i], smweights[i], l2weights[i]).numpy() for i in range(weights.shape[0])]
        loss_curve_test = [target_log_prob_cat_test(weight_mean[i], weight_scale[i], weights[i], smweights[i], l2weights[i]).numpy() for i in range(weights.shape[0])]
        ...
        sns.kdeplot(l2weights[-1].numpy().flatten(), ax=ax[2][0])
        ...
        sns.kdeplot(l2weights[0].numpy().flatten(), ax=ax[2][0])
        plt.show()
        #plt.close()
    ...
    if True:
        #for plotcompontent in range(nchains):
            # to do: enable batch - remove [0] index
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[w], smweights[w], l2weights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_train.shape[0]), Y_train, "Train") for i in range(10)], axis=0)
        ...
        # df2 = create_maxpoint_prediction(df)
        #for plotcompontent in range(nchains):
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[w], smweights[w], l2weights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_test.shape[0]), Y_test, "Test") for i in range(10)], axis=0)
...
for i in range(mixcom):
    plot_softprob_simple(make_mixture_probs_train(weights[w], smweights[w], l2weights[w]).distribution.components_distribution.mean()[plotcompontent,0,i], title="component "+str(i))
