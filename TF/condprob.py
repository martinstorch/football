...
with CALC_GRAPH.as_default():
  ...
  calc_sky_points_tensor = themodel.calc_sky_points(pl_pGS,pl_pGC, pl_GS, pl_GC)
  calc_pistor_points_tensor = themodel.calc_pistor_points(pl_pGS,pl_pGC, pl_GS, pl_GC)
...
#  predictions[1].update({"labels": labels[1,:]})
  ...
  with tf.Session(graph=CALC_GRAPH) as sess:
    ...
    df['Pt'], df['SkyPt'], df['PistorPt'] = sess.run(
        [tf.cast(calc_points_tensor, tf.int8), 
         tf.cast(calc_sky_points_tensor, tf.int8), 
         tf.cast(calc_pistor_points_tensor, tf.int8)], 
      feed_dict=feed_dict)
