    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x):
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                weights=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64),
                               scale=tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=2),
        ...
        def loss_function(weights, smweights, X, Y): # weight_mean, weight_scale,
            ...
            lp = tf.reduce_sum(lpp['weights']+lpp['smweights'])*(X.shape[-2]) + tf.reduce_sum(lpp['outputs'], axis=-1)*0.75 # lpp['weight_mean']+lpp['weight_scale']+
            ...
            lpp["total"] = tf.reduce_sum(lp) + 5.0*tf.reduce_sum(actual_points) - tf.reduce_sum(reg_loss) #- lpp["outputs"] + lpp["outputs"]*0.3 #

            return lpp

        def analyse_losses(weights, smweights, X, Y): # weight_mean, weight_scale,
            return loss_function(weights, smweights, X, Y)
        ...
        def target_log_prob_cat(weights, smweights):
            return loss_function(weights, smweights, x_train_scaled, outputs_)["total"]


        def target_log_prob_cat_test(weights, smweights):
            return loss_function(weights, smweights, x_test_scaled, test_outputs_)["total"]


        def analyse_target_log_prob_cat(weight_mean, weight_scale, weights, smweights):
            return analyse_losses(weights, smweights, x_train_scaled, outputs_)

        print(joint_model)
        surrogate_posterior = tfp.experimental.vi.build_factored_surrogate_posterior(
            event_shape=list(joint_model.event_shape_tensor().values())[:-1],
            constraining_bijectors=[None, None],
        ...
        (weights0, smweights0), _ = surrogate_posterior.sample_distributions()
        ...
        (weights, smweights), _ = surrogate_posterior.sample_distributions()
        #print(weights.mean())

        filename = "mcmc_deterministic_"+FLAGS.prefix+"_"+FLAGS.target_system+"_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        filehandler = open(filename, 'wb')
        pickle.dump((weights.mean(), weights.stddev(), smweights.mean(), smweights.stddev()), filehandler)

        fig, ax = plt.subplots(3, 2)
        plt.subplots_adjust(left=0.04, bottom=0.03, right=0.96, top=0.97, wspace=0.08, hspace=0.18)
        sns.kdeplot(weights.mean().numpy().flatten(), ax=ax[0][0])
        sns.kdeplot(weights.stddev().numpy().flatten(), ax=ax[0][1])
        sns.kdeplot(smweights.mean().numpy().flatten(), ax=ax[1][0])
        sns.kdeplot(smweights.stddev().numpy().flatten(), ax=ax[1][1])
        sns.kdeplot(weights0_mean.flatten(), ax=ax[0][0])
        sns.kdeplot(weights0_stddev.flatten(), ax=ax[0][1])
        sns.kdeplot(smweights0_mean.flatten(), ax=ax[1][0])
#        sns.kdeplot(smweights0_stddev.flatten(), ax=ax[1][1])
        ax[2][0].plot(losses.numpy())
        ax[2][1].plot(losses.numpy()[500:])
        plt.show()

        target_log_prob_cat_test(weights.sample(), smweights.sample())
        train_points = [analyse_losses(weights.sample(), smweights.sample(), x_train_scaled, outputs_)["real_points"].numpy() for i in range(100)]
        train_mean_points = analyse_losses(weights.mean(), smweights.mean(), x_train_scaled, outputs_)["real_points"].numpy()

        test_points = [analyse_losses(weights.sample(), smweights.sample(), x_test_scaled, test_outputs_)["real_points"] .numpy() for i in range(100)]
        test_mean_points = analyse_losses(weights.mean(), smweights.mean(), x_test_scaled, test_outputs_)["real_points"].numpy()

        train_softpoints = [analyse_losses(weights.sample(), smweights.sample(), x_train_scaled, outputs_)["mean_points"].numpy() for i in range(100)]
        train_mean_softpoints = analyse_losses(weights.mean(), smweights.mean(), x_train_scaled, outputs_)["mean_points"].numpy()

        test_softpoints = [analyse_losses(weights.sample(), smweights.sample(), x_test_scaled, test_outputs_)["mean_points"] .numpy() for i in range(100)]
        test_mean_softpoints = analyse_losses(weights.mean(), smweights.mean(), x_test_scaled, test_outputs_)["mean_points"].numpy()
        ...
        t2 = make_mixture_probs_train(weights.mean(), smweights.mean()).mean().numpy()
        ...
        t2 = make_mixture_probs_test(weights.mean(), smweights.mean()).mean().numpy()
        ...
        pred_probs = make_mixture_probs_pred(weights.mean(), smweights.mean()).mean().numpy()
        ...
        if True:
            ...
            for i in range(mixcom):
                pr = make_mixture_probs_train(weights.mean(), smweights.mean()).components_distribution[0].mean()[i]
