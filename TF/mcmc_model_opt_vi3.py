    ...
    if __name__ == "__main__":
        ...
        l1weights = tf.Variable(tf.random.normal(shape=[x.shape[1] + 1, L1size], dtype=tf.float64))
        l2weights = tf.Variable(tf.random.normal(shape=[L1size + 1, L2size], dtype=tf.float64))
        weights = tf.Variable(tf.random.normal(shape=[L1size + 1, mixcom], dtype=tf.float64))
        smweights = tf.Variable(tf.random.normal(shape=[mixcom, 49], dtype=tf.float64))
        regularizer = tf.keras.regularizers.L1L2(l2=0.001)
        ...
        def make_joint_mixture_model(x):
            def make_outputs(l1w_in, l1w_out, l2w_in, l2w_out, w_in, w_out, sm_in, sm_out, ): #tfd.Sample(
               ...
               x1 = tf.math.tanh(tf.matmul(x, l1w[..., :-1, :]) + l1w[..., -1:, :])
               x2 = tf.matmul(x1, l2w[..., :-1, :]) + l2w[..., -1:, :]
               l = tf.matmul(x1, wgt[..., :-1, :]) + wgt[..., -1:, :]
               compdist_logits = 10.0 * smw[..., tf.newaxis, :mixcom, :] * tf.ones([x.shape[0], 1, 1], dtype=tf.float64)
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                l1w_in=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[x.shape[1] + 1], dtype=tf.float64),
                               scale=tf.ones(shape=[x.shape[1] + 1], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                l1w_out=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L1size], dtype=tf.float64),
                               scale=tf.ones(shape=[L1size], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                l2w_in=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L1size + 1], dtype=tf.float64),
                               scale=tf.ones(shape=[L1size + 1], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                l2w_out=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L2size], dtype=tf.float64),
                               scale=tf.ones(shape=[L2size], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                w_in=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L1size + 1], dtype=tf.float64),
                               scale=tf.ones(shape=[L1size + 1], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                w_out=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[mixcom], dtype=tf.float64),
                               scale=tf.ones(shape=[mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                sm_in=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[mixcom], dtype=tf.float64),
                               scale=tf.ones(shape=[mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=1),
                sm_out=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[49], dtype=tf.float64),
                               scale=tf.ones(shape=[49], dtype=tf.float64)), reinterpreted_batch_ndims=1),
        ...
        def loss_function(l1w_in, l1w_out, l2w_in, l2w_out, w_in, w_out, sm_in, sm_out, X, Y, Y2): # weight_mean, weight_scale,
            ...
            lp = tf.reduce_sum(lpp['l1w_in']+lpp['l1w_out']+lpp['l2w_in']+lpp['l2w_out']+lpp['w_in']+lpp['w_out']+lpp['sm_in']+lpp['sm_out'])*(X.shape[-2]) #+ tf.reduce_sum(lpp['outputs'], axis=-1)*0.00001 # lpp['weight_mean']+lpp['weight_scale']+
            reg_loss = create_laplacian_loss(smweights, alpha=0.3)
            rl_l1 = regularizer(l1weights)
            rl_l2 = regularizer(l2weights)
            rl_w = regularizer(weights)
            rl_sm = regularizer(smweights)
            ...
            lpp.update({"laplacian_reg_loss":reg_loss, "rl_l1":rl_l1, "rl_l2":rl_l2, "rl_w":rl_w, "rl_sm":rl_sm,
                        "joint_model_log_prob":lp,  "softmax_lp_mean":tf.reduce_mean(softmax_lp), "poisson_lp_mean":tf.reduce_mean(poisson_lp), "mean_points":tf.reduce_mean(actual_points), "real_points":tf.reduce_mean(real_points)
                       })
            lpp["total"] = tf.reduce_sum(lp) + 5.0*tf.reduce_sum(actual_points) - tf.reduce_sum(reg_loss) - tf.reduce_sum(reg_loss2) +0.01*tf.reduce_sum(poisson_lp) + 1.8*tf.reduce_sum(softmax_lp)#- lpp["outputs"] + lpp["outputs"]*0.3 #
            return lpp
        ...
        if FLAGS.checkpoints != "" and len(inputfilename) > 0:
            ...
            all_states, all_weights = pickle.load(filehandler)
        ...
        print(analyse_target_log_prob_cat(*[v.mean() for v in surrpost_vars]))
        ...
        l1w_in, l1w_out, l2w_in, l2w_out, w_in, w_out, sm_in, sm_out = surrpost_vars
        print(analyse_target_log_prob_cat(*[v.mean() for v in surrpost_vars]))
        print(analyse_test_log_prob_cat(*[v.mean() for v in surrpost_vars]))
        ...
        pickle.dump((surrogate_posterior.trainable_variables, (l1weights, l2weights, weights, smweights)), filehandler)

        fig, ax = plt.subplots(5, 3)
        plt.subplots_adjust(left=0.04, bottom=0.03, right=0.96, top=0.97, wspace=0.08, hspace=0.18)

        sns.kdeplot(l1w_in.mean().numpy().flatten(), ax=ax[0][0])
        sns.kdeplot(l1w_in.stddev().numpy().flatten(), ax=ax[0][1])
        sns.kdeplot(l1w_out.mean().numpy().flatten(), ax=ax[0][0])
        sns.kdeplot(l1w_out.stddev().numpy().flatten(), ax=ax[0][1])
        ...
        sns.kdeplot(w_in.mean().numpy().flatten(), ax=ax[2][0])
        sns.kdeplot(w_in.stddev().numpy().flatten(), ax=ax[2][1])
        sns.kdeplot(w_out.mean().numpy().flatten(), ax=ax[2][0])
        sns.kdeplot(w_out.stddev().numpy().flatten(), ax=ax[2][1])
