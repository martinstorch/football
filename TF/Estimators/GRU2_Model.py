...
#        tf.summary.histogram("gru_cell/candidate/bias", tf.get_default_graph().get_tensor_by_name("Model/RNN_12/rnn/gru_cell/candidate/bias/read:0"))
      ...
      with tf.variable_scope("Mapping_Layer"):
        ...
        num_extra_neurons = 50 # for future use
        tf.summary.histogram("Inputs", X)
        W = tf.get_variable(
            name="WM",
            initializer=tf.random_normal_initializer(stddev=0.01),
            regularizer = GLOBAL_REGULARIZER,
            shape=[int(X.shape[1]), output_size+num_extra_neurons])
        b = tf.get_variable(
            name="bM",
            initializer=tf.random_normal_initializer(),
            shape=[output_size+num_extra_neurons])
        ...
        split_layer = tf.split(outputs, [output_size, num_extra_neurons], axis=1)
        outputs = split_layer[0]
        extra_neurons = split_layer[1]
        tf.summary.histogram("Weights", W)
        tf.summary.histogram("Bias", b)
        tf.summary.histogram("Outputs", outputs )
        tf.summary.histogram("Extra", extra_neurons )
      ...
      with tf.variable_scope("HomeBias"):
        X = extra_neurons[:,0:10]
        X = tf.layers.dense(X, 10, activation=tf.nn.relu)
        X = tf.nn.dropout(X, keep_prob=0.7)
        X = tf.layers.dense(X, 1, activation=tf.nn.sigmoid, name="HomeBias")
        tf.summary.histogram("HomeBias", X)
        tf.summary.histogram("Output", tf.get_default_graph().get_tensor_by_name("Model/HomeBias/HomeBias/BiasAdd:0"))
        home_bias = X
        
      return outputs, logits, home_bias
...
#  with tf.Session() as sess:
##    x, x1, t_matrix = calc_joint_poisson_prob(1.1, 2.22)
#    x, x1, x2, x3, t_matrix = calc_joint_poisson_prob(0.0001, 1.0)
#    print(x1.eval())
#    print(x2.eval())
#    print(x3.eval())
#    print(x.eval())
#    print(np.sum(x.eval()))
#    print(x1[0,7:14].eval()*np.exp(-1.0))
#    print(np.sum(x1[0,7:14].eval()*np.exp(-1.0)))
#    print(x1[0,0:7].eval()*np.exp(-0.0001))
#    print(np.sum(x1[0,0:7].eval()*np.exp(-0.0001)))
#    print(t_matrix[:,0].eval())
...
#        #p_pred_softmax = tf.nn.softmax(ev_points, dim=1)
#        p_pred_softmax = ev_points / tf.reduce_sum(ev_points, axis=1, keep_dims=True)
#        blend_factor = home_bias #* 0.25
#        #blend_factor = 0.2 * 0.25
#        p_pred_blended = (1-blend_factor) * p_pred_softmax + blend_factor * p_pred_12
#        a = tf.argmax(p_pred_blended, axis=1)
#        pred_blended = tf.reshape(tf.stack([a // 7, tf.mod(a, 7)], axis=1), [-1,2])
#        pred_blended= tf.cast(pred_blended, tf.int32)

#          p_pred_blended *= normalization_factor # normalize to 1
#          p_pred_softmax *= normalization_factor # normalize to 1
#        
#        ev_points *= normalization_factor # normalize to 1
#        p_pred_12 *= normalization_factor # normalize to 1
        p_pred_12_m = tf.reshape(p_pred_12, [-1,7,7])
...
#          "p_poisson_x2":p_poisson_x2, 
          "p_pred_12":p_pred_12, 
          "ev_points":ev_points, 
#          "ev_points_blended":ev_points_blended,  
          "pred":pred,
          "ev_goals_1":ev_goals_1,
          "ev_goals_2":ev_goals_2, 
#          "ev_goals_x1":ev_goals_x1, 
...
#        l_chisq_poiss_marg_2 = tf.reduce_mean(tf.square(p_marg_2 - p_poisson_2)/(p_poisson_2+0.0001), axis=1)
        ...
        pred_nodraw = tf.reshape(tf.stack([a // 7, tf.mod(a, 7)], axis=1), [-1,2])
        pred_nodraw = tf.cast(pred_nodraw, tf.int32)
#        
##        p_pred_x1 = tf.transpose(tf.reshape(p_pred_12, [-1,7,7]), [0,2,1]) # tf.reshape(p_pred_12_m, [-1,5])
##        p_pred_x1 = tf.reshape(p_pred_x1, [-1,7])
##        ev_goals_x1 = tf.matmul(p_pred_x1, tc_1d1_goals_f)
...
#      l_regularization += tf.reduce_mean(reg_Gates_L22+reg_Candidates_L22)
      ...
      noise_factor = 0.015
      contraction_factor = 0.1 * noise_factor
      def add_weight_noise(name):
        ...
        return tf.assign(W, W+tf.random_normal(shape=tf.shape(W), mean=-contraction_factor*W, stddev=W*noise_factor, dtype=tf.float32)) 
        
      if True: #"Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates"
        make_noise_op = tf.group(add_weight_noise("Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates/kernel:0"), 
                                 add_weight_noise("Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/candidate/kernel:0"), 
...
#      l_regularization += tf.reduce_mean(reg_Kernel_L2)
      ...
      home_bias_raw = tf.get_default_graph().get_tensor_by_name("Model/HomeBias/HomeBias/BiasAdd:0")
...
#          , "reg_Kernel_L2": tf.metrics.mean(reg_Kernel_L2)
          ...
          , "regularization/home_bias_output_raw": tf.metrics.mean(home_bias_raw)
      }
...
#      pt_softpoints = tf.reduce_sum(predictions["sp/p_pred_12"] * achievable_points_mask, axis=1)
      ...
      p3_softpoints = tf.reduce_sum(predictions["p3/p_pred_12"] * achievable_points_mask, axis=1)
      p5_softpoints = tf.reduce_sum(predictions["p5/p_pred_12"] * achievable_points_mask, axis=1)
      p6_softpoints = tf.reduce_sum(predictions["p6/p_pred_12"] * achievable_points_mask, axis=1)
...
#          , "points/pt_home_loss_points": tf.metrics.mean(pt_home_loss_points)
  ...
  def model(features, labels, mode):
    ...
    with tf.variable_scope("Model"):
      outputs, logits, home_bias = buildGraph(features, mode)
...
#      t_is_train_bool = tf.equal(features["Train"] , True)
      ...
      predictions_poisson_rule_based_p5 = apply_home_bias(predictions_poisson_HT, home_bias+0.03)
...
#      predictions.update(predictions_poisson_rule_based_p3)
      ...
      predictions_poisson_rule_based_p6 = apply_home_bias(predictions_poisson_rule_based, home_bias+0.03)
      predictions.update(apply_prefix(predictions_poisson_rule_based_p6, "p6/"))

#      predictions_poisson_rule_based_p5 = {"p5/"+k:v for k,v in predictions_poisson_rule_based.items() }
#      predictions_poisson_rule_based_p5["p5/pred"] = predictions_poisson_rule_based_p5["p5/pred_nodraw"] # use max likelyhood but remap draw results to away win
#      predictions.update(predictions_poisson_rule_based_p5)
...
#      t_is_draw_bool = tf.equal(t_goals_1, t_goals_2)
    ...
    eval_metric_ops.update({"summary/metric_home_bias": tf.metrics.mean(predictions["p3/home_bias"])})
