...
#        eval_metric_ops.update(variable_summaries(X, "Normalized", mode))
      ...
      with tf.variable_scope("Ensemble"):
        ...
        X = tf.stop_gradient(X)
        ensemble_logits,_ = build_dense_layer(X, 18, mode, regularizer = l2_regularizer(scale=0.01), keep_prob=1.0, batch_norm=True, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)
