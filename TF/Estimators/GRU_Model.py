...
def create_estimator(model_dir, label_column_names, staticProbs, save_steps, max_to_keep):    
...
#        
        ...
        if False:
          staticProbs_rank2 = tf.expand_dims(staticProbs, axis=0)
          staticPointsMatrix = tf.concat([tf.matmul(staticProbs_rank2, tc_home_points_i),
                                          tf.matmul(staticProbs_rank2, tc_away_points_i)], 
                                          axis = 0
          )
          ev_points_static =  tf.gather(staticPointsMatrix, tf.cast(t_is_home_bool, tf.int32))
          blend_factor = 0.3
          ev_points_blended = blend_factor * ev_points + (1-blend_factor) * ev_points_static * p_pred_12
          a = tf.argmax(ev_points_blended, axis=1)
          pred = tf.reshape(tf.stack([a // 7, tf.mod(a, 7)], axis=1), [-1,2])
          pred = tf.cast(pred, tf.int32)
        else:
          #p_pred_softmax = tf.nn.softmax(ev_points, dim=1)
          p_pred_softmax = ev_points / 100.0 # tf.reduce_sum(ev_points, axis=1, keep_dims=True)
          blend_factor = 0.10
...
#          "p_poisson_x2":p_poisson_x2, 
          "p_pred_12":p_pred_12, 
          "p_pred_softmax":p_pred_softmax, 
          "p_pred_blended":p_pred_blended, 
          "ev_points":ev_points,  
...
#      t_is_train_bool = tf.equal(features["Train"] , True)
      ...
      predictions_poisson_rule_based_p3 = {"p3/"+k:v for k,v in predictions_poisson_FT.items() }
      predictions_poisson_rule_based_p3["p3/pred"] = predictions_poisson_rule_based_p3["p3/pred_blended"]
      predictions_poisson_rule_based_p3["p3/ev_points"] = predictions_poisson_rule_based_p3["p3/p_pred_softmax"]*100
      predictions_poisson_rule_based_p3["p3/p_pred_12"] = predictions_poisson_rule_based_p3["p3/p_pred_blended"]
      ...
      predictions_poisson_rule_based_p5 = {"p5/"+k:v for k,v in predictions_poisson_HT.items() }
      predictions_poisson_rule_based_p5["p5/pred"] = predictions_poisson_rule_based_p5["p5/pred_blended"]
      predictions_poisson_rule_based_p5["p5/ev_points"] = predictions_poisson_rule_based_p5["p5/p_pred_softmax"]*100
      predictions_poisson_rule_based_p5["p5/p_pred_12"] = predictions_poisson_rule_based_p5["p5/p_pred_blended"]
