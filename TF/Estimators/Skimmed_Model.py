...
def create_estimator(model_dir, label_column_names, save_steps, max_to_keep, teams_count):    
  ...
  def buildGraph(features, mode): 
      ...
      #X = 0.15*X2 + X0 # shortcut connection bypassing two non-linear activation functions
...
#        eval_metric_ops.update(variable_summaries(X, "Normalized", mode))
      ...
      with tf.variable_scope("Skymax"):
        sk_logits,_ = build_dense_layer(X, 49, mode, regularizer = l2_regularizer(scale=0.2), keep_prob=1.0, batch_norm=False, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)
...
#        tf.summary.scalar(key, value[1])
      ...
      return tf.estimator.EstimatorSpec(
          mode=mode, predictions=predictions, loss= loss, eval_metric_ops={k:v[0] for k,v in eval_metric_ops.items()})
