...
#      t_weight = t_weight / t_weight_total
      ...
      l_loglike_poisson = tf.expand_dims(t_weight, axis=1) * tf.nn.log_poisson_loss(targets=t_labels[:,:-8], log_input=outputs[:,:-8])
...
#
      t_xg_weights = tf.concat([tf.ones_like(t_labels[:,-8:-6]), 
                                tf.cast(tf.greater(t_labels[:,-6:-2], 0), tf.float32),
                                tf.ones_like(t_labels[:,-2:])], axis=1)
      l_xg_mse = tf.expand_dims(t_weight, axis=1) * tf.losses.mean_squared_error(
          labels=t_labels[:,-8:], 
          predictions=outputs[:,-8:], 
          weights = t_xg_weights)
...
#      loss += tf.reduce_mean(0.5*l_gfull)  
      ...
      l_softmax_wdl_1h =  t_weight * tf.reduce_sum(-tf.log(p_softmax_wdl_1h+1e-10)*label_wdl_1h, axis=1, name="t_extract_49_to_WDL_mask")
      ...
      l_softmax_wdl_2h =  t_weight * tf.reduce_sum(-tf.log(p_softmax_wdl_2h+1e-10)*label_wdl_2h, axis=1, name="t_extract_49_to_WDL_mask")
