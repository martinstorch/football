...
#        X_inputs = X
#        tf.summary.histogram("Inputs", X)
#        X = tf.layers.dense(inputs=X, units=1, activation=tf.nn.relu, bias_initializer=tf.constant_initializer(0.1), kernel_initializer=tf.random_normal_initializer(stddev=0.2))
...
#        X = tf.layers.dropout(X,rate=0.98, training=(mode == tf.estimator.ModeKeys.TRAIN))
