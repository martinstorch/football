...
def create_estimator(model_dir, label_column_names, my_feature_columns, thedata, thelabels, save_steps, evaluate_after_steps, max_to_keep, teams_count, use_swa, histograms, target_distr):    
  ...
  def build_cond_prob_layer(X, labels, mode, regularizer, keep_prob, eval_metric_ops): 
    ...
    with tf.variable_scope("H1"):
      ...
      create_laplacian_loss(p_pred_12_h1, alpha=1.0) # 100
  ...
  #    p_pred_bothgoal = p_pred_h1[:,7]
    ...
    with tf.variable_scope("H2"):
      ...
      if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        create_laplacian_loss(p_pred_12_h2, alpha=1.0) # 100
        ...
        create_laplacian_loss(test_p_pred_12_h2, alpha=1.0) # 100
...
#      l_gfull = softmax_loss_masked(predictions["sp/p_pred_12"], p_full, t_gdiff_select_mask, t_weight) 
      ...
      loss += 50*tf.reduce_mean(l_softmax_1h) # 13
      loss += 500*tf.reduce_mean(l_softmax_2h) # 130
