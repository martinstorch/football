...
def create_estimator(model_dir, label_column_names, save_steps, max_to_keep, teams_count):    
  ...
  def buildGraph(features, mode): 
      ...
      with tf.variable_scope("Layer0"):
          X0,Z0 = build_dense_layer(X, 128, mode, regularizer = l2_regularizer(scale=3.0), keep_prob=1.0, batch_norm=True, activation=None, eval_metric_ops=eval_metric_ops)
      ...
      X = 0.015*X2 + X0 # shortcut connection bypassing two non-linear activation functions
...
#        eval_metric_ops.update(variable_summaries(X, "Normalized", mode))
      ...
      with tf.variable_scope("Skymax"):
        sk_logits,_ = build_dense_layer(X, 49, mode, regularizer = l2_regularizer(scale=1.2), keep_prob=1.0, batch_norm=False, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)

      with tf.variable_scope("Softmax"):
        sm_logits,_ = build_dense_layer(X, 49, mode, regularizer = l2_regularizer(scale=3.0), keep_prob=1.0, batch_norm=False, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)
...
#        tc_1d1_goals_f, tc_home_points_i, tc_away_points_i, calc_poisson_prob, p_tendency_mask_f, p_gdiff_mask_f, p_fulltime_index_matrix = tc
  ...
  def point_maximization_layer(X, prefix, t_is_home_bool, tc, mode):
      with tf.variable_scope("PointMax_"+prefix):
          ...
          X = tf.stop_gradient(X)
          with tf.variable_scope("Layer1"):
            X,_ = build_dense_layer(X, output_size=10, mode=mode, regularizer = None, keep_prob=0.8, batch_norm=False, activation=tf.nn.relu) #, eval_metric_ops=None, use_bias=None, add_term=None, batch_scale=True)        
          with tf.variable_scope("Layer2"):
            X,_ = build_dense_layer(X, output_size=10, mode=mode, regularizer = None, keep_prob=0.8, batch_norm=False, activation=tf.nn.relu) #, eval_metric_ops=None, use_bias=None, add_term=None, batch_scale=True)        
          with tf.variable_scope("Layer3"):
