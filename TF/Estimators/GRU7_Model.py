...
def harmonize_outputs(outputs, label_column_names):
  ...
  inverse_label_column_names=['T2_GFT', 'T1_GFT', 'T2_GHT', 'T1_GHT', 
                       ...
                       'T2_GH2', 'T1_GH2', 'Win', 'Draw', 'Loss', 'HTWin', 'HTDraw', 'HTLoss', 
...
#                                                   frequency_skip=1)
      ...
      with tf.variable_scope("RNN_1"):
        history_state_t1 = make_rnn(match_history_t1, sequence_length = 10*features_newgame[:,0])  
        if True:
      ...
      with tf.variable_scope("RNN_2"):
        history_state_t2 = make_rnn(match_history_t2, sequence_length = 10*features_newgame[:,1])  
...
#
      with tf.variable_scope("RNN_12"):
        history_state_t12 = make_rnn(match_history_t12, sequence_length = 10*features_newgame[:,3])  
...
#        X = tf.layers.dense(X, 128, name="Dense", kernel_regularizer = MINOR_REGULARIZER)
...
#        X = tf.layers.dense(X, 64, name="Dense", kernel_regularizer = MINOR_REGULARIZER)
#        tf.summary.histogram("Outputs", X)
...
#        tf.summary.histogram("Layer1", X)
#        X = tf.nn.dropout(X, keep_prob=0.7)
        X = tf.layers.dense(X, 18, name="Logits", kernel_regularizer = GLOBAL_REGULARIZER)
        tf.summary.histogram("Outputs", X)
        ensemble_logits = X
...
#        
      ...
      # 30.1.2018
      base_noise_factor = 0.002 * tf.exp((current_tendency - 0.45)/0.05)
      ...
      rnn_noise_factor = base_noise_factor + 0.01 * tf.exp((current_winloss_corr - 0.75)/0.05)
      ...
      # 30.1.2018
      noise_factor_mapping = 0.001 + 0.03 * tf.exp((current_diff_corr - 0.45)/0.05)
      ...
      ens_noise = 0.0001 * tf.exp((ens_tendency - 0.45)/0.05)
      #tf.summary.scalar("ens_noise", ens_noise)
      ens_contraction_factor = 0.3 * ens_noise
      ...
      # 31.01.2018
      sm_noise = 0.001 * tf.exp((sm_tendency - 0.45)/0.05)
      ...
      def apply_noise(axes, name, stddev_factor, contraction_factor):  
        X = tf.get_default_graph().get_tensor_by_name(name)
      ...
      if True: #"Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates"
        make_noise_op = tf.group(
           ...
           apply_noise([1], "Model/Mapping_Layer/WM:0", noise_factor_mapping, contraction_factor_mapping), 
           apply_noise([0], "Model/Ensemble/Logits/kernel:0", ens_noise, ens_contraction_factor), 
           apply_noise([0,1], "Model/Mapping_Layer/WM:0", base_noise_factor*4, contraction_factor_mapping*0.1), 
           apply_noise([0], "Model/Softmax/Logits/kernel:0", sm_noise, sm_noise*0.001), 
           apply_noise([0], "Model/Output_Layer/W:0", sp_noise, sp_noise*0.001), 
#           apply_noise([0,1], "Model/Layer1/Dense/kernel:0", base_noise_factor*10, base_noise_factor*0.01), 
#           apply_noise([0,1], "Model/Layer2/Dense/kernel:0", base_noise_factor*0.5, base_noise_factor*0.001), 
...
#      labels, labels2 = add_noise(labels, labels2, 0.01)      
    ...
    with tf.variable_scope("Losses"):
      ...
      t_weight = tf.exp(0.5*match_date)
...
#      pt_softpoints = tf.reduce_sum(predictions["sp/p_pred_12"] * achievable_points_mask, axis=1)
      pt_log_softpoints = t_weight * tf.reduce_sum(tf.log(predictions["sp/p_pred_12"]+0.2) * achievable_points_mask, axis=1)
