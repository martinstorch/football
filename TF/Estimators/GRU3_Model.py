...
#          W = add_weight_noise(W, 0.05)
      ...
      return outputs, logits, home_bias, sm_logits
  ...
  def create_model_regularization_metrics(eval_metric_ops):
...
#      l_regularization += tf.reduce_mean(reg_Gates_L22+reg_Candidates_L22)
      ...
      current_tendency = eval_metric_ops["p4/metric_is_tendency"][1]
      tf.summary.scalar("current_tendency", current_tendency)
      
      #noise_factor = 0.015
      #noise_factor = 0.01
      #noise_factor = 0.003 + 0.4 * tf.nn.relu(current_tendency - 0.50)
      noise_factor = 0.01 * tf.exp((current_tendency - 0.40)/0.05)
      tf.summary.scalar("noise_factor", noise_factor)
      contraction_factor = 0.2 * noise_factor
      ...
      noise_factor_mapping = 0.001 + 0.01 * tf.exp((current_diff_corr - 0.45)/0.05)
      contraction_factor_mapping = 0.001 
      tf.summary.scalar("noise_factor_mapping", noise_factor_mapping)
      ...
      if True: #"Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates"
        make_noise_op = tf.group(add_weight_noise("Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates/kernel:0", noise_factor, contraction_factor), 
                                 add_weight_noise("Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/candidate/kernel:0", noise_factor, contraction_factor), 
...
#                   (0*tf.log(pt_softpoints)+(pt_softpoints)))
      ...
      l_regularization, reg_eval_metric_ops, make_noise_op = create_model_regularization_metrics(eval_metric_ops)
...
#          , "points/pt_home_loss_points": tf.metrics.mean(pt_home_loss_points)
  ...
  def model(features, labels, mode):
    ...
    with tf.variable_scope("Model"):
      outputs, logits, home_bias, sm_logits = buildGraph(features, mode)
...
#    reset_op = tf.assign(reset_node, reset_node - 0.5)
    ...
    train = tf.group( reset_op , make_noise_op, optimizer.minimize(loss), tf.assign_add(global_step, 1))
