...
plot_list = ["cp/", "cp2/", "sm2/", "ps2/", "pg2/"] # ["pspt/", "cp/", "cp2/", "sp/", "ens/"]
prefix_list = ["pg/", "pgpt/", "pghb/", "ps/", "pspt/", "sp/", "cp1/", "cp/", "cp2/", "cpz/","sm/", "smpt/", "pg2/", "sm2/", "ps2/"]
#ens_prefix_list = ["pg/", "pgpt/", "pghb/", "ps/", "pspt/", "sp/", "smpt/", "cp/", "cp2/"]
ens_prefix_list = ["pgpt/", "pspt/", "sp/", "smpt/", "cp/", "cp2/"]
segmentation_strategies = {"cp":"cp2", "pg":"pg2", "sm":"sm2", "ps":"ps2"}
point_scheme = None
...
def build_cond_prob_layer(X, labels, mode, regularizer, keep_prob, eval_metric_ops): 
  ...
  with tf.variable_scope("H1"):
    ...
    t_map = tf.cast(t_map, tf.float32)
...
#    def produce_h2_prediction(i):
#      # create features from assumed half-time result
#      test_label_oh_h1 = tf.expand_dims(tf.one_hot(i, 49), axis=0)
#      test_label_features_h1 = tf.matmul(test_label_oh_h1, t_mask)
#      test_X = tf.map_fn(lambda x: tf.concat([x, test_label_features_h1[0]], axis=0), X)
#      # this should find the same dense layer with same weights as in training - because name scope is same
#      test_h2_logits,_ = build_dense_layer(test_X, 49, mode, regularizer = regularizer, keep_prob=keep_prob, batch_norm=False, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)
#      test_p_pred_12_h2 = tf.nn.softmax(test_h2_logits)
#      p = p_pred_12_h1[:,i] * test_p_pred_12_h2  # prior * posterior
#      return  p
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      test_label_oh_h1 = tf.one_hot(tf.range(49), 49)
      test_label_features_h1 = tf.matmul(test_label_oh_h1, t_map)
#      with tf.Session() as sess:
#        print(sess.run(test_label_oh_h1))
#        print(sess.run(t_map))
#        print(sess.run(test_label_features_h1))
      #print(test_label_features_h1) # Tensor("Model/condprob/H2/MatMul:0", shape=(49, 8), dtype=float32)
...
#        sk_logits,_ = build_dense_layer(X, 49, mode, regularizer = l2_regularizer(scale=1.2), keep_prob=0.8, batch_norm=False, activation=None, eval_metric_ops=eval_metric_ops, use_bias=True)
      ...
      home_bias = tf.nn.sigmoid(home_bias)+0.0005
  ...
  def create_hierarchical_predictions(outputs, logits, t_is_home_bool, tc, p_pred_12 = None, apply_point_scheme=True):
...
#        p_pred_gdiff = tf.stack(p_pred_gdiff, axis=1, name="p_pred_gdiff")
        ...
        if True:
          ...
          pred = create_fixed_scheme_prediction(outputs, p_pred_12, t_is_home_bool)
  ...
  def create_fixed_scheme_prediction(outputs, p_pred_12, t_is_home_bool):
        t_win_mask = tf.stack([1.0 if i // 7 > np.mod(i, 7) else 0.0  for i in range(49)], name="t_win_mask")
        t_loss_mask = tf.stack([1.0 if i // 7 < np.mod(i, 7) else 0.0  for i in range(49)], name="t_loss_mask")
        t_draw_mask = tf.stack([1.0 if i // 7 == np.mod(i, 7) else 0.0  for i in range(49)], name="t_draw_mask")
  ...
  def create_fixed_scheme_prediction_new(p_pred_12, t_is_home_bool):
        t_win_mask = tf.stack([1.0 if i // 7 > np.mod(i, 7) else 0.0  for i in range(49)], name="t_win_mask")
        t_loss_mask = tf.stack([1.0 if i // 7 < np.mod(i, 7) else 0.0  for i in range(49)], name="t_loss_mask")
        t_draw_mask = tf.stack([1.0 if i // 7 == np.mod(i, 7) else 0.0  for i in range(49)], name="t_draw_mask")
        t_both_teams_score_mask = tf.stack([1.0 if (i // 7) > 0 and np.mod(i, 7) > 0 else 0.0  for i in range(49)], name="t_both_teams_score_mask")
  ...
  def create_flex_cutpoints(name):
    with tf.variable_scope(name):
      #segments = tf.constant([[0,4], [0,3], [0,2], [1,2], [1,1], [2,1], [2,0], [3,0],[4,0]], dtype=tf.int32)
      segments0 = np.array([[0,4], [0,3], [0,2], [0,1], [0,0], [1,0], [2,0], [3,0],[4,0]])
      segments1 = np.array([[0,4], [0,3], [0,2], [1,2], [1,1], [2,1], [2,0], [3,0],[4,0]])
      segments = np.stack([segments0,segments1], axis=2)
      
      cutpoints = tf.get_variable("cutpoints", shape=(segments.shape[0]-1+1), dtype=tf.float32, initializer=tf.zeros_initializer())
      return segments, cutpoints
      
  def create_flexible_scheme_prediction(p_pred_12, t_is_home_bool, segmentation):
    return create_fixed_scheme_prediction_new(p_pred_12, t_is_home_bool)    
  
  
    segments, cutpoints = segmentation
    cutzero = cutpoints[-1]
    ...
    p_pred_loss = p_pred_tendency[:,2]
    p_pred_both = p_pred_tendency[:,3]
    tf.summary.histogram("segmentation/p_pred_both", p_pred_both)
    tf.summary.histogram("segmentation/p_pred_both_shifted", p_pred_both - cutzero)
    diffp = tf.where(t_is_home_bool, p_pred_win - p_pred_loss, p_pred_loss - p_pred_win)
    ncutpoints = int(cutpoints.shape[0])
    t_matched_segment = tf.stack(
        [tf.less_equal(diffp, cutpoints[0])] + 
        [tf.logical_and(tf.less(cutpoints[i-1], diffp), tf.less_equal(diffp, cutpoints[i])) for i in range(1,ncutpoints)] + 
        [tf.less(cutpoints[-1], diffp)] 
        , axis=1)
    t_selected_segments = tf.argmax(tf.cast(t_matched_segment, tf.int32), axis=1)
#        pred = tf.where(tf.less(p_pred_both - cutzero , 0.5),
#                        tf.gather(tf.constant(segments[:,:,0]), t_selected_segments),
#                        tf.gather(tf.constant(segments[:,:,1]), t_selected_segments))
    pred = tf.gather(tf.constant(segments[:,:,1]), t_selected_segments)
    t_is11_b = tf.logical_and(tf.equal(pred[:,0], 1), tf.equal(pred[:,1],1))
    t_is21_b = tf.logical_and(tf.equal(pred[:,0], 2), tf.equal(pred[:,1],1))
    t_is12_b = tf.logical_and(tf.equal(pred[:,0], 1), tf.equal(pred[:,1],2))
    #print(t_is11_b)
    pred = tf.where(tf.logical_and( t_is11_b, tf.greater(p_pred_12[:,0*7+0] , p_pred_12[:,1*7+1])), 
              pred*0, pred) # change 1:1 to 0:0
    pred = tf.where(
              tf.logical_and( 
                    t_is_home_bool, 
                    tf.logical_and( 
                        t_is21_b, 
                        tf.greater(p_pred_12[:,1*7+0],  p_pred_12[:,2*7+1]- cutzero))), 
              tf.stack([pred[:,0]*0+1, pred[:,0]*0+0], axis=1), pred) # change 2:1 to 1:0
    pred = tf.where(
              tf.logical_and( 
                  tf.logical_not(t_is_home_bool), 
                  tf.logical_and( 
                      t_is21_b, 
                      tf.greater(p_pred_12[:,0*7+1], p_pred_12[:,1*7+2]- cutzero))), 
              tf.stack([pred[:,0]*0+1, pred[:,0]*0+0], axis=1), pred) # change 2:1 to 1:0

    pred = tf.where(tf.logical_and( t_is_home_bool, tf.logical_and( t_is12_b, 
                                   tf.greater(p_pred_12[:,0*7+1], p_pred_12[:,1*7+2]- cutzero))), 
              tf.stack([pred[:,0]*0+0, pred[:,0]*0+1], axis=1), pred) # change 1:2 to 0:1
    pred = tf.where(tf.logical_and( tf.logical_not(t_is_home_bool), tf.logical_and( t_is12_b, 
                                   tf.greater(p_pred_12[:,1*7+0], p_pred_12[:,2*7+1]- cutzero))), 
              tf.stack([pred[:,0]*0+0, pred[:,0]*0+1], axis=1), pred) # change 1:2 to 0:1
    pred = tf.where(t_is_home_bool, pred, pred[:,::-1])
    return pred
...
#        tc_1d1_goals_f, tc_home_points_i, tc_away_points_i, calc_poisson_prob, p_tendency_mask_f, p_gdiff_mask_f, p_fulltime_index_matrix = tc
        ...
        p_ensemble = p_ensemble / tf.reduce_sum(p_ensemble, axis=1, keep_dims=True) # normalize to 100%
...
#        pred = tf.gather_nd(t_preds, index2)
  ...
  def point_maximization_layer(outputs, X, prefix, t_is_home_bool, tc, mode):
      with tf.variable_scope("PointMax_"+prefix):
          ...
          logits=X
          #predictions = create_hierarchical_predictions(logits, t_is_home_bool, tc)
...
#          })
  ...
  def create_segmentation_metrics(segmentation_list, mode):
    metrics={}
    for prefix,v in segmentation_list.items():
      segments, cutpoints = v
      cutzero = cutpoints[-1]
      cutpoints = cutpoints[:-1]
      segments = segments[:,:,1]
      metrics.update(collect_summary(prefix, "cut_zero", mode, tensor=cutzero))

      tf.summary.histogram("Segmentation/{}/cutpoints".format(prefix), cutpoints)
      for i in range(int(cutpoints.shape[0])):
        metric_name="cut_{}{}_{}{}".format(segments[i+1,0], segments[i+1,1], segments[i,0], segments[i,1])
        metrics.update(collect_summary(prefix, metric_name, mode, tensor=cutpoints[i]))
    return metrics

  def create_segmentation_loss(mode, p_pred_12, t_weight, segmentation_prefix, segmentation_list, t_is_home_bool, achievable_points_mask, segmentation_metrics):
      p_pred_12 = tf.stop_gradient(p_pred_12)  
      segments, cutpoints = segmentation_list[segmentation_prefix] 
      cutzero = cutpoints[-1]
      cutpoints = cutpoints[:-1]
      ...
      t_tendency_mask = tf.stack([t_win_mask, t_loss_mask, t_both_teams_score_mask], axis=1, name="t_tendency_mask")
      
      p_pred_tendency = tf.matmul(p_pred_12, t_tendency_mask)
      p_pred_win = p_pred_tendency[:,0]
      p_pred_loss = p_pred_tendency[:,1]
      p_pred_both_goals = p_pred_tendency[:,2]
      diffp = tf.where(t_is_home_bool, p_pred_win - p_pred_loss, p_pred_loss - p_pred_win)
      ncutpoints = int(cutpoints.shape[0])
      l_segmentation_losses=[]
      l_segmentation_ordering=[]
      diffp_len = tf.shape(diffp)[0]
      # diffp_len = int(diffp.shape[0])
      print(diffp_len)
      diffp_sorted, diffp_indices = tf.nn.top_k(-diffp, k=diffp_len) # sort all by increasing value
      diffp_sorted = -diffp_sorted 
      for i in range(ncutpoints):
        # check which points are available on either side of the cutpoint
        available_points_below = tf.where(t_is_home_bool, 
                                       achievable_points_mask[:,segments[i,0,1]*7+segments[i,1,1]],
                                       achievable_points_mask[:,segments[i,1,1]*7+segments[i,0,1]])
        available_points_above = tf.where(t_is_home_bool, 
                                       achievable_points_mask[:,segments[i+1,0,1]*7+segments[i+1,1,1]],
                                       achievable_points_mask[:,segments[i+1,1,1]*7+segments[i+1,0,1]])

        t_sorted_predictions = tf.gather(available_points_above - available_points_below, diffp_indices) # order points by increasing diffp
        t_cumsum_predictions = tf.cumsum(t_sorted_predictions)/tf.cast(diffp_len,tf.float32) # target is to drive the diffp value towards the minimum of this list
        cp = cutpoints[i] # the currently chosen cutpoint
        cp_rank = tf.reduce_sum(tf.cast(tf.greater(cp, diffp_sorted), tf.int32)) # how many diffp values are less or equal the current cutoff point?
        # cp_rank ranges from 0 to diffp_len  !
        # approximate the gradient by the empirical slope of a triangle: delta cumsum / delta diffp within a small window
        gradient_list=[]
        for window_size in [2, 5, 10, 15, 25, 50, 100]:
          min_index = tf.maximum(cp_rank - window_size, 0)
          max_index = tf.minimum(cp_rank + window_size-1, diffp_len-1)
          gradient_emp = (t_cumsum_predictions[max_index]-t_cumsum_predictions[min_index]) / tf.maximum(diffp_sorted[max_index]-diffp_sorted[min_index], 1e-3)
          gradient_emp = tf.clip_by_value(gradient_emp, -1.0, 1.0)
          gradient_list.append(gradient_emp)
          
          
          #gradient_emp = tf.clip_by_value(gradient_emp, -0.1*window_size, 0.1*window_size)
          l_segmentation_loss = cp * gradient_emp
          l_segmentation_losses.append(l_segmentation_loss)

        # select median gradient from list
        t_gradient_list = tf.stack(gradient_list)
        gradient_value, _ = tf.nn.top_k(t_gradient_list, k=3)
        l_segmentation_losses = [cp * gradient_value[-1]]
        
        metric_name="gradient_{}{}_{}{}".format(segments[i+1,0,1], segments[i+1,1,1], segments[i,0,1], segments[i,1,1])
        tf.summary.histogram("{}/Segmentation/{}".format(segmentation_prefix, metric_name), t_gradient_list)
        tf.summary.histogram("{}/Segmentation/med_{}".format(segmentation_prefix, metric_name), gradient_value[-1])
        
        if False:
          min_value, min_index = tf.nn.top_k(-t_cumsum_predictions, k=1)         
          min_value = -min_value
  #        print(min_value[0])
  #        print(t_cumsum_predictions[cp_rank])
  #        print(diffp_sorted[min_index])
  #        print(cp)
          gradient_emp = (t_cumsum_predictions[tf.minimum(cp_rank, diffp_len-1)]-min_value[0]) / tf.maximum(diffp_sorted[min_index[0]]-cp, 1e-3)
          gradient_emp = tf.clip_by_value(gradient_emp, -1.0, 1.0)
          l_segmentation_loss = cp * gradient_emp
          l_segmentation_losses.append(l_segmentation_loss)
...
#
#        sigma = 0.2
##        sigmprob = tf.sigmoid((diffp - cutpoints[i])/sigma)
#        segment_labels = tf.cast(tf.less(available_points_below, available_points_above), tf.float32)
#        segment_weights = tf.abs(available_points_below - available_points_above)
#        #local_range_weight = tf.exp(-tf.nn.relu(cutpoints[i-1]-diffp)/sigma)*tf.exp(-tf.nn.relu(diffp-cutpoints[i+1])/sigma)
#        if i == 0:
#          local_range_weight = tf.exp(-tf.nn.relu(diffp-cutpoints[i+1])/sigma)
#        elif i == ncutpoints-1:
#          local_range_weight = tf.exp(-tf.nn.relu(cutpoints[i-1]-diffp)/sigma)
#        else:
#          local_range_weight = tf.exp(-tf.nn.relu(cutpoints[i-1]-diffp)/sigma)*tf.exp(-tf.nn.relu(diffp-cutpoints[i+1])/sigma)
#        
#        if i == 0:
#          is_in_range = tf.less_equal(diffp, cutpoints[1])
#        elif i == ncutpoints-1:
#          is_in_range = tf.less(cutpoints[i-1], diffp)
#        else:
#          is_in_range = tf.logical_and(tf.less(cutpoints[i-1], diffp), tf.less_equal(diffp, cutpoints[i+1]))
#        is_in_range = tf.cast(is_in_range, tf.float32)+0.0001        
##        l_segmentation_loss = tf.log(sigmprob)*segment_labels+tf.log(1-sigmprob)*(1-segment_labels)
##        l_segmentation_loss = - l_segmentation_loss * t_weight * segment_weights  * is_in_range #* local_range_weight  #
#        
#        l_segmentation_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=segment_labels, logits = (diffp - cutpoints[i])/sigma) 
#        l_segmentation_loss = l_segmentation_loss * t_weight * segment_weights  * is_in_range # local_range_weight  
#        
#        l_segmentation_losses.append(l_segmentation_loss)

        if i>0:
          l_segmentation_ordering.append(tf.nn.relu(cutpoints[i-1]-cutpoints[i]+0.0001))
        segmentation_metrics.update(collect_summary(segmentation_prefix, "l_segmentation_{}{}_{}{}".format(segments[i+1,0,1], segments[i+1,1,1], segments[i,0,1], segments[i,1,1]), mode, tensor=l_segmentation_loss))

      l_segmentation_zero_losses=[]
      for i in range(ncutpoints+1):
        available_points_below = tf.where(t_is_home_bool, 
                                       achievable_points_mask[:,segments[i,0,0]*7+segments[i,1,0]],
                                       achievable_points_mask[:,segments[i,1,0]*7+segments[i,0,0]])
        available_points_above = tf.where(t_is_home_bool, 
                                       achievable_points_mask[:,segments[i,0,1]*7+segments[i,1,1]],
                                       achievable_points_mask[:,segments[i,1,1]*7+segments[i,0,1]])
        #sigma = 0.2
        segment_labels = tf.cast(tf.less(available_points_below, available_points_above), tf.float32)
        segment_weights = tf.abs(available_points_below - available_points_above)
        #prob_above=tf.sigmoid((p_pred_both_goals - cutzero)/sigma)
        #pt_above = prob_above*available_points_above+(1-prob_above)*available_points_below
        
        #l_segmentation_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=segment_labels, logits = (p_pred_both_goals - cutzero)/sigma) 
        prob_above=tf.nn.relu(p_pred_both_goals - cutzero)+1e-7
        l_segmentation_loss = -tf.log(prob_above)*segment_labels-tf.log(1-prob_above)*(1-segment_labels)
        l_segmentation_loss = l_segmentation_loss * t_weight * segment_weights  
        #l_segmentation_loss = - pt_above
        l_segmentation_zero_losses.append(l_segmentation_loss)

      l_segmentation_ordering.append(tf.nn.relu(-cutpoints[0]-1))
      l_segmentation_ordering.append(tf.nn.relu(cutpoints[ncutpoints-1]-1))
      
      l_segmentation_loss = tf.stack(l_segmentation_losses)
      #l_segmentation_loss = tf.stack(l_segmentation_losses, axis=1)
      l_segmentation_zero_loss = tf.stack(l_segmentation_zero_losses, axis=1)
      l_segmentation_ordering_loss = tf.stack(l_segmentation_ordering)
      segmentation_metrics.update(collect_summary(segmentation_prefix, "l_segmentation_ordering", mode, tensor=l_segmentation_ordering_loss ))
      segmentation_metrics.update(collect_summary(segmentation_prefix, "l_segmentation_zero", mode, tensor=l_segmentation_zero_loss ))
      return l_segmentation_loss, l_segmentation_ordering_loss, l_segmentation_zero_loss
    
  def create_losses_RNN(outputs, sp_logits, sm_logits, cond_probs, t_labels, features, predictions, t_is_home_bool, mode, tc, t_is_home_win_bool , t_is_home_loss_bool, eval_metric_ops, segmentation_list):
    tc_1d1_goals_f, tc_home_points_i, tc_away_points_i, calc_poisson_prob, p_tendency_mask_f, p_gdiff_mask_f, p_fulltime_index_matrix = tc
...
#      t_labels = add_noise_to_label(t_labels, 0.05)
    ...
    with tf.variable_scope("Losses"):
      ...
      l_mask_h1 = tf.matmul(tf.one_hot(gs_1h*7+gc_1h, 49), t_mask)
      l_mask_h2 = tf.matmul(tf.one_hot(gs*7+gc, 49), t_mask)
      l_loglike_poisson_h1 = l_mask_h1*tf.expand_dims(t_weight, axis=1) * tf.nn.log_poisson_loss(targets=label_features_h1, log_input=tf.log(p_pred_h1))
      l_loglike_poisson_h2 = l_mask_h2*tf.expand_dims(t_weight, axis=1) * tf.nn.log_poisson_loss(targets=label_features_h2, log_input=tf.log(p_pred_h2))
...
#      t_is_loss_f = tf.cast(tf.less(gs, gc), tf.float32)
      ...
      l_tendency = -t_weight * tf.where(t_is_home_bool,
                                       point_scheme[3][0]*tf.log(predictions["sp/p_pred_win"]+epsilon)*tf.cast(t_is_home_win_bool, tf.float32) + 
                                       point_scheme[3][1]*tf.log(predictions["sp/p_pred_draw"]+epsilon)*t_is_draw_f + 
                                       point_scheme[3][2]*tf.log(predictions["sp/p_pred_loss"]+epsilon)*tf.cast(t_is_home_loss_bool, tf.float32) ,
                                       
                                       point_scheme[3][0]*tf.log(predictions["sp/p_pred_loss"]+epsilon)*tf.cast(t_is_home_win_bool, tf.float32) + 
                                       point_scheme[3][1]*tf.log(predictions["sp/p_pred_draw"]+epsilon)*t_is_draw_f + 
                                       point_scheme[3][2]*tf.log(predictions["sp/p_pred_win"]+epsilon)*tf.cast(t_is_home_loss_bool, tf.float32) 
                                       )
#      l_tendency = -t_weight * tf.where(t_is_home_bool,
#                                       tf.log(predictions["sp/p_pred_win"]+epsilon)*tf.cast(t_is_home_win_bool, tf.float32) + 
#                                       tf.log(predictions["sp/p_pred_draw"]+epsilon)*t_is_draw_f + 
#                                       tf.log(predictions["sp/p_pred_loss"]+epsilon)*tf.cast(t_is_home_loss_bool, tf.float32) ,
#                                       
#                                       tf.log(predictions["sp/p_pred_loss"]+epsilon)*tf.cast(t_is_home_win_bool, tf.float32) + 
#                                       tf.log(predictions["sp/p_pred_draw"]+epsilon)*t_is_draw_f + 
#                                       tf.log(predictions["sp/p_pred_win"]+epsilon)*tf.cast(t_is_home_loss_bool, tf.float32) 
#                                       )
...
#      loss += tf.reduce_mean(5.0*l_sm_gdiff)
      ...
      loss += 0*tf.reduce_mean(l_loglike_poisson_h1) 
      loss += 0*tf.reduce_mean(l_loglike_poisson_h2) 
      ...
      for k,v in segmentation_strategies.items():
        l_segmentation_loss, l_segmentation_ordering_loss, l_segmentation_zero_loss = \
          create_segmentation_loss(mode, predictions[k+"/p_pred_12"], t_weight, v, 
                                   segmentation_list, t_is_home_bool, achievable_points_mask, segmentation_metrics)
        loss += 50*tf.reduce_mean(l_segmentation_loss)
        loss += 10000*tf.reduce_mean(l_segmentation_ordering_loss )
        loss += tf.reduce_mean(l_segmentation_zero_loss )
...
#        loss -= tf.reduce_mean(pt_sampled_points)
      ...
      pd.DataFrame({
          ...
          "draw":[1.0 if i // 7 == np.mod(i, 7) else 0.0  for i in range(49)]}).as_matrix()
      
      
      t_lt2_mask = tf.stack([1.0 if i // 7 + np.mod(i, 7) < 2 else 0.0  for i in range(49)], name="t_lt2_mask")
      t_eq2_mask = tf.stack([1.0 if i // 7 + np.mod(i, 7) == 2 else 0.0  for i in range(49)], name="t_eq2_mask")
      t_gt2_mask = tf.stack([1.0 if (i // 7 + np.mod(i, 7)) in [3,4] and (i // 7 in [1,2]) and (np.mod(i, 7) in [1,2]) else 0.0  for i in range(49)], name="t_gt2_mask")
      t_gt3_mask = tf.stack([1.0 if i // 7  >= 3 or np.mod(i, 7) >= 3 else 0.0  for i in range(49)], name="t_gt3_mask")
      
      t_wdl_mask = tf.stack([t_win_mask, t_draw_mask, t_loss_mask], axis=1, name="t_wdl_mask")
      t_leg2_mask = tf.stack([t_lt2_mask, t_eq2_mask, t_gt2_mask, t_gt3_mask ], axis=1, name="t_leg2_mask")
...
#      ops.add_to_collection(tf.GraphKeys.UPDATE_OPS, t_wdl_mask ) # control dependencies
      ...
      def softpoint_loss(p):
        ...
        softpoints = t_weight * weighted_softmax_cross_entropy(-achievable_points_mask* t_draw_bias_mask, p)
...
#        t_wdl_pred = tf.matmul(p, t_wdl_mask)   
#        t_leg2_pred = tf.matmul(p, t_leg2_mask)   
#        l_softmax1 = t_weight * tf.nn.softmax_cross_entropy_with_logits(labels=t_wdl_labels, logits=tf.log(t_wdl_pred+epsilon) )
#        l_softmax2 = t_weight * tf.nn.softmax_cross_entropy_with_logits(labels=t_leg2_labels, logits=tf.log(t_leg2_pred+epsilon) )
#        softpoints = t_weight * tf.reduce_sum(tf.log(1.0+p+epsilon) * achievable_points_mask, axis=1)
#        return 0.05*l_softmax1 + 0.2*l_softmax2, softpoints

      pt_pgpt_sm_loss, pt_pgpt_softpoints = softpoint_loss(predictions["pgpt/p_pred_12"])
      pt_pspt_sm_loss, pt_pspt_softpoints = softpoint_loss(predictions["pspt/p_pred_12"])
      pt_smpt_sm_loss, pt_smpt_softpoints = softpoint_loss(predictions["smpt/p_pred_12"])
      pt_pghb_sm_loss, pt_pghb_softpoints = softpoint_loss(predictions["pghb/p_pred_12"])
...
##      pt_pgpt_softpoints = t_weight * tf.reduce_sum(tf.log(1.0+tf.minimum(predictions["pgpt/p_pred_12"], cap)+epsilon) * achievable_points_mask * t_draw_bias_mask, axis=1)
##      pt_pspt_softpoints = t_weight * tf.reduce_sum(tf.log(1.0+tf.minimum(predictions["pspt/p_pred_12"], cap)+epsilon) * achievable_points_mask * t_draw_bias_mask, axis=1)
##      pt_smpt_softpoints = t_weight * tf.reduce_sum(tf.log(1.0+tf.minimum(predictions["smpt/p_pred_12"], cap)+epsilon) * achievable_points_mask * t_draw_bias_mask, axis=1)
#      pt_pghb_softpoints = t_weight * tf.reduce_sum(predictions["pghb/p_pred_12"] * achievable_points_mask, axis=1)
      ...
      loss += 0.05*tf.reduce_mean(pt_pghb_sm_loss)
      loss += 0.05*tf.reduce_mean(pt_pspt_sm_loss)
      loss += 0.05*tf.reduce_mean(pt_smpt_sm_loss)
...
#        
      ...
      eval_metric_ops.update(segmentation_metrics)
      ...
      eval_metric_ops.update(collect_summary("losses", "pt_pghb_sm_loss", mode, tensor=pt_pghb_sm_loss))
      eval_metric_ops.update(collect_summary("losses", "pt_smpt_sm_loss", mode, tensor=pt_smpt_sm_loss))
      eval_metric_ops.update(collect_summary("losses", "pt_pspt_sm_loss", mode, tensor=pt_pspt_sm_loss))
...
#      t_is_train_bool = tf.equal(features["Train"] , True)
      ...
      predictions = create_hierarchical_predictions(outputs, sp_logits, t_is_home_bool, tc, apply_point_scheme=False)
      ...
      cp_predictions = create_hierarchical_predictions(outputs, h2_logits, t_is_home_bool, tc)
      ...
      smpt_predictions.update(point_maximization_layer(outputs, sm_logits, "smpt", t_is_home_bool, tc, mode))
...
#        predictions_poisson_rule_based ["p_pred_12"]=tf.zeros_like(predictions_poisson_rule_based ["p_pred_12"]) 
      ...
      p4pt_predictions.update(point_maximization_layer(outputs, outputs, "pspt", t_is_home_bool, tc, mode))
      ...
      segmentation_list={}
      for k,v in segmentation_strategies.items():
        segmentation_layer = create_flex_cutpoints(v)
        segm_pred = create_flexible_scheme_prediction(predictions[k+"/p_pred_12"], t_is_home_bool, segmentation_layer)
        ...
        segmentation_list[v]=segmentation_layer
...
#      t_is_draw_bool = tf.equal(t_goals_1, t_goals_2)
      ...
      eval_metric_ops.update(create_segmentation_metrics(segmentation_list, mode))
    ...
    eval_loss_ops, loss = create_losses_RNN(outputs, sp_logits, sm_logits, cond_probs, labels, features, predictions, t_is_home_bool, mode, tc, t_is_home_win_bool , t_is_home_loss_bool, eval_metric_ops, segmentation_list)
