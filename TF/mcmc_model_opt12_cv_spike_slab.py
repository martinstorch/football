    ...
    def get_train_test_data(model_dir, train_seasons, cv_seasons, test_seasons, data_dir, useBWIN):
        print("train_seasons")
        print(train_seasons)
        ...
        is_train_cv = full_data.Season.astype(str).apply(lambda x: x.zfill(4)).isin(cv_seasons).repeat(2)
        ...
        all_data["TrainCV"] = is_train_cv.values & (~all_data["Predict"])
    ...
    def get_input_data(model_dir, train_data, cv_data, test_data, data_dir, useBWIN):
        all_data, all_labels, all_features, teamnames, team_mapping = get_train_test_data(model_dir, train_data, cv_data, test_data,
                                                                                          data_dir, useBWIN)
        print(all_data.iloc[1000])
    ...
    def dispatch_main(target_distr, model_dir, train_steps, train_data, cv_data, test_data,
                      checkpoints, save_steps, data_dir, max_to_keep,
                      reset_variables, skip_plotting, target_system, modes, prefix, histograms, useBWIN):
        ...
        cv_data = [s.strip() for s in cv_data.strip('[]').split(',')]
        ...
        all_data, teamnames, features_arrays, labels_array, team_onehot_encoder, label_column_names = get_input_data(
            model_dir, train_data, cv_data, test_data, data_dir, useBWIN)
        print(features_arrays["match_input_layer"][:, 23:26])
        ...
        cv_idx = all_data.index[all_data['TrainCV']].tolist()
        ...
        if cv_idx:
            print("TrainCV index {}-{}".format(np.min(cv_idx), np.max(cv_idx)))
        if test_idx:
            print("Test index {}-{}".format(np.min(test_idx), np.max(test_idx)))
        print("Prediction index {}-{}".format(np.min(pred_idx), np.max(pred_idx)))

        model_data = (features_arrays, labels_array, train_idx, cv_idx, test_idx, pred_idx)
        #  if modes == "static":
    ...
    def main(_):
        ...
        return dispatch_main(target_distr, FLAGS.model_dir, FLAGS.train_steps,
                             FLAGS.train_data, FLAGS.cv_data, FLAGS.test_data, FLAGS.checkpoints,
    ...
    if __name__ == "__main__":
        ...
        parser.add_argument(
            ...
            default="1617,1718,1819,1920,2021", #
        ...
        features_arrays, labels_array, train_idx, cv_idx, test_idx, pred_idx = model_data
        ...
        X_train_cv_bwin = np.take(X[cv_idx], bwin_index, axis=1)
        ...
        X_train_cv_spi = np.take(X[cv_idx], spi_index, axis=1)
        ...
        cv_outputs_ = tf.one_hot(create_result_index(Y_train_cv[:,0], Y_train_cv[:,1]), 49) #.astype(np.float64)
        ...
        def make_joint_mixture_model(x):
            def make_outputs(xi, weights, smweights): #tfd.Sample(
               l = tf.matmul(x, tf.cast(tf.clip_by_value(xi, clip_value_min=0.0, clip_value_max=1.0)[...,tf.newaxis], tf.float64)*weights[..., :-1, :]) + weights[..., -1:, :]
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                w1=tfd.LogNormal(loc=tf.zeros(shape=[], dtype=tf.float64),
                                 scale=tf.ones(shape=[], dtype=tf.float64)),
                w2=tfd.LogNormal(loc=tf.zeros(shape=[], dtype=tf.float64),
                                 scale=tf.ones(shape=[], dtype=tf.float64)),
                r3=tfd.LogNormal(loc=tf.zeros(shape=[], dtype=tf.float64),
                                 scale=tf.ones(shape=[], dtype=tf.float64)),
                xi=tfd.Independent(tfd.Bernoulli(probs=0.5*tf.ones(shape=[x.shape[1]], dtype=tf.float64), dtype=tf.float64), reinterpreted_batch_ndims=1),
            #     x2=lambda xi: tfd.Independent(tfd.Mixture(cat=tfd.Categorical(probs=tf.stack([xi, 1-xi], axis=-1)),
            #                                                        components=[
            #         tfd.Normal(loc=tf.ones(shape=[x.shape[1]], dtype=tf.float64), scale=0.00001*tf.ones(shape=[x.shape[1]], dtype=tf.float64)),
            #         tfd.Normal(loc=tf.zeros(shape=[x.shape[1]], dtype=tf.float64), scale=0.00001*tf.ones(shape=[x.shape[1]], dtype=tf.float64))
            ...
            #                       scale=tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)),
            #         reinterpreted_batch_ndims=2),
                weights=tfd.Independent(tfd.Laplace(loc=tf.zeros(shape=[x.shape[1]+ 1, mixcom], dtype=tf.float64), scale=1.0 * tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=2),
                #weights=tfd.Independent(tfd.Cauchy(loc=tf.zeros(shape=[x.shape[1]+ 1, mixcom], dtype=tf.float64), scale=1.0 * tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=2),
        ...
        make_mixture_probs_train_cv, joint_model_cv = make_joint_mixture_model(x_train_cv_scaled)
        ...
        def loss_function(w1, w2, r3, xi, weights, smweights, X, Y, X_cv, Y_cv): # x2, weight_mean, weight_scale,

            mk, joint_model = make_joint_mixture_model(X)
            mk_cv, joint_model_cv = make_joint_mixture_model(X_cv)
            Y = tf.cond(tf.rank(weights)==3, lambda: Y[..., tf.newaxis, :, :], lambda: Y)
            Y_cv = tf.cond(tf.rank(weights)==3, lambda: Y_cv[..., tf.newaxis, :, :], lambda: Y_cv)
            ...
            lp = tf.reduce_sum(lpp['w1']+lpp['w2']+lpp['r3']+lpp['xi']+lpp['weights']+lpp['smweights'])*(X.shape[-2]) + tf.reduce_sum(lpp['outputs'], axis=-1)*0.01 # +lpp['x2']+lpp['weight_mean']+lpp['weight_scale']
            reg_loss = create_laplacian_loss(smweights, alpha=10.0+r3)

            achievable_points = tf.matmul(Y, tf.cast(point_matrix, tf.float32))
            achievable_points_cv = tf.matmul(Y_cv, tf.cast(point_matrix, tf.float32))
            pred = mk(xi, weights, smweights)
            pred_cv = mk_cv(xi, weights, smweights)
            dist_probs = tf.nn.softmax(pred.mixture_distribution.logits_parameter(), axis=-1)

            actual_points = tf.nn.softmax(pred.components_distribution.logits, axis=-1) * tf.cast(achievable_points, tf.float64)[...,tf.newaxis, :] # [1,mixcom,1]
            ...
            dist_probs_cv = tf.nn.softmax(pred_cv.mixture_distribution.logits_parameter(), axis=-1)
            actual_points_cv = tf.nn.softmax(pred_cv.components_distribution.logits, axis=-1) * tf.cast(achievable_points_cv, tf.float64)[...,tf.newaxis, :] # [1,mixcom,1]
            actual_points_cv = dist_probs_cv[...,tf.newaxis] * actual_points_cv # (5508, 10, 49)
            actual_points_cv = tf.reduce_sum(tf.reduce_sum(actual_points_cv, axis=-2), axis=-1)

            real_points_cv = tf.reduce_sum(tf.one_hot(tf.argmax(pred_cv.mean(), axis=-1), 49) * achievable_points_cv, axis=-1)
            ...
            lpp.update({"laplacian_reg_loss":reg_loss, "joint_model_log_prob":lp, "mean_points":tf.reduce_mean(actual_points), "real_points":tf.reduce_mean(real_points),
                        "mean_points_cv":tf.reduce_mean(actual_points_cv), "real_points_cv":tf.reduce_mean(real_points_cv)})
            lpp["total"] = tf.reduce_sum(lp) + 1000.0*tf.reduce_sum(tf.math.sqrt(actual_points)) - tf.reduce_sum(reg_loss) #- w2*tf.exp(10*(tf.stop_gradient(tf.reduce_sum(actual_points)/tf.reduce_sum(actual_points_cv) - 1))) #- lpp["outputs"] + lpp["outputs"]*0.3 #

            return lpp

        def analyse_losses(w1, w2, r3, xi, weights, smweights, X, Y, X_cv, Y_cv): # x2, weight_mean, weight_scale,
            return loss_function(w1, w2, r3, xi, weights, smweights, X, Y, X_cv, Y_cv)
        ...
        def target_log_prob_cat(w1, w2, r3, xi,  weights, smweights):
            return loss_function(w1, w2, r3, xi, weights, smweights, x_train_scaled, outputs_, x_train_cv_scaled, cv_outputs_)["total"]


        def target_log_prob_cat_test(w1, w2, r3, xi, weights, smweights):
            return loss_function(w1, w2, r3, xi, weights, smweights, x_test_scaled, test_outputs_, x_test_scaled, test_outputs_)["total"]


        def analyse_target_log_prob_cat(w1, w2, r3, xi, weights, smweights):
            return analyse_losses(w1, w2, r3, xi, weights, smweights, x_train_scaled, outputs_, x_train_cv_scaled, cv_outputs_)

        num_results = FLAGS.save_steps
        num_burnin_steps = FLAGS.train_steps
        ...
        sampler_bij = tfp.mcmc.TransformedTransitionKernel(
            ...
            bijector=[tfb.Softplus(), tfb.Chain([tfb.Softplus(), tfb.Scale(scale=tf.constant(2.0, dtype=tf.float64))]), tfb.Softplus(), tfb.Identity(), tfb.Identity(), tfb.Identity()]) # tfb.Affine(shift=0.135, scale_identity_multiplier=0.01, dtype=tf.float64)
        ...
        initial_state = [#np.random.randn(x_train.shape[1]+1, mixcom), # np.zeros # weight_mean
                         tf.ones(shape=[], dtype=tf.float64), # w1
                         tf.ones(shape=[], dtype=tf.float64), # w2
                         tf.ones(shape=[], dtype=tf.float64), # r3
                         0.5*tf.ones(shape=[x_train.shape[1]], dtype=tf.float64), # xi
                         # tf.ones(shape=[x_train.shape[1]], dtype=tf.float64), # x2
        ...
        inputfilename = sorted([f for f in os.listdir() if re.search("mcmc_"+FLAGS.prefix+"_"+FLAGS.target_system+".*pickle", f)])
        if FLAGS.checkpoints!="" and len(inputfilename) > 0:
            ...
            w1, w2, r3, xi, weights, smweights = all_states # x2, weight_mean, weight_scale,
            initial_state = [w1[-1], w2[-1], r3[-1], xi[-1], weights[-1], smweights[-1]] # x2[-1], weight_mean[-1], weight_scale[-1],
        ...
        states = sample()
        filename = "mcmc_"+FLAGS.prefix+"_"+FLAGS.target_system+"_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        filehandler = open(filename, 'wb')
        pickle.dump(states.all_states, filehandler)
        w1, w2, r3, xi, weights, smweights = states.all_states # , cdist

        current_state = [w1[-1], w2[-1], r3[-1], xi[-1], weights[-1], smweights[-1]] # x2[-1], weight_mean[-1], weight_scale[-1],
        #current_state2 = [w1[-1], w2[-1], r3[-1], xi[-10], x2[-10], weight_mean[-10], weight_scale[-10], weights[-10], smweights[-10]]
        ...
        def avg_points(xi, weights, smweights, X, Y):
        ...
        def real_points(xi, weights, smweights, X, Y):
            mk, _ = make_joint_mixture_model(X)
            pred = mk(xi, weights, smweights)
            achievable_points = tf.matmul(Y, tf.cast(point_matrix, tf.float32))
        ...
        if True:
            ...
            stepsize = 25
            loss_curve = [target_log_prob_cat(w1[i:(i+stepsize)], w2[i:(i+stepsize)], r3[i:(i+stepsize)], xi[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i
                          in range(0, weights.shape[0], stepsize)] # x2[i:(i+stepsize)], weight_mean[i:(i+stepsize)], weight_scale[i:(i+stepsize)],
            loss_curve_test = [target_log_prob_cat_test(w1[i:(i+stepsize)], w2[i:(i+stepsize)], r3[i:(i+stepsize)], xi[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i in range(0, weights.shape[0], stepsize)]
            avg_points_train = np.concatenate([tf.reduce_mean(avg_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_scaled, outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            avg_points_train_cv = np.concatenate([tf.reduce_mean(avg_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_cv_scaled, cv_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            avg_points_test = np.concatenate([tf.reduce_mean(avg_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                        smweights[i:(i+stepsize)], x_test_scaled, test_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            real_points_train = np.concatenate([tf.reduce_mean(real_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_scaled, outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            real_points_train_cv = np.concatenate([tf.reduce_mean(real_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_cv_scaled, cv_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            real_points_test = np.concatenate([tf.reduce_mean(real_points(xi[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                        smweights[i:(i+stepsize)], x_test_scaled, test_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            ...
            ax[3][1].plot(real_points_train)
            ax[4][0].plot(avg_points_test)
            ax[4][1].plot(real_points_test)
            ax[5][0].plot(avg_points_train_cv)
            ax[5][1].plot(real_points_train_cv)
            sns.kdeplot(avg_points_train, ax=ax[6][0])
            sns.kdeplot(avg_points_test, ax=ax[6][0])
            sns.kdeplot(avg_points_train_cv, ax=ax[6][0])
            sns.kdeplot(real_points_train, ax=ax[6][1])
            sns.kdeplot(real_points_test, ax=ax[6][1])
            sns.kdeplot(real_points_train_cv, ax=ax[6][1])
            plt.show()
            #plt.close()
            ...
            ax[1,0].plot(np.stack([joint_model.parameters["model"]["weights"].log_prob(weights[q]).numpy() for q in range(weights.shape[0])]))
            ax[1,1].plot(np.stack([joint_model.parameters["model"]["smweights"].log_prob(smweights[q]).numpy() for q in range(weights.shape[0])]))
            ax[2,0].plot(w1)
            ax[2,1].plot(w2)
            ax[3,0].plot(r3)
            ax[3,1].plot(xi)
        ...
        if True:
            ...
            #df2 = pd.concat([create_df(create_argmax_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 5)], axis=0), Y_train.shape[0]), Y_train, "Train") for i in range(10)], axis=0)
            t2=np.mean(np.mean(np.array([make_mixture_probs_train(xi[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)]), axis=0), axis=0)
            ...
            t2=np.mean(np.mean(np.array([make_mixture_probs_test(xi[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)]), axis=0), axis=0)
            ...
            t2=np.mean(np.mean(np.array([make_mixture_probs_train_cv(xi[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)]), axis=0), axis=0)
            df2 = create_df(create_argmax_prediction_from_mean(t2, Y_train_cv.shape[0]), Y_train_cv, "CV")
            #df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 5)], axis=0), Y_test.shape[0]), Y_test, "Test") for i in range(10)], axis=0)
            plot_predictions_3( df2, "poisson", "CV", silent=False)
        ...
        df_pred = pd.concat([create_df(create_argmax_prediction_from_mean(np.concatenate(
            [make_mixture_probs_pred(xi[w], weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], weights.shape[0])],
    ...
    if True:
        pred_probs = np.mean(make_mixture_probs_pred(xi, weights, smweights).mean().numpy(), axis=0)
...
plot_softprob_simple(np.mean(cv_outputs_[::2], axis=0), prefix="Train CV Seasons Summary")
...
print("Best points - CV")
print(np.dot(np.mean(cv_outputs_[::2], axis=0), point_matrix).reshape((7,7)).transpose()[::-1])
