    ...
    if __name__ == "__main__":
        ...
        def find_next_params_from_gaussian_process(searchlog, num_iters3=1000):
            ...
            prior_dist = tfd.Independent(tfd.Normal(loc=current_point, scale=1.0), reinterpreted_batch_ndims=1) # prevent movements far away from origin
            # best_index_var = tfp.util.TransformedVariable(
            ...
            # print(current_point)
            # print(last_score)
            # print(expected_improvement(np.array(current_point)[tf.newaxis, ...]))
            # print(prior_dist.log_prob(np.array(current_point)[tf.newaxis, ...]))
            #stop()
        ...
        def make_joint_mixture_model(x):
            def make_outputs(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, ): #tfd.Sample(
               l1w = l1weights * l1w_in[...,tf.newaxis]  * l1w_out[...,tf.newaxis, :]
               l2w = l2weights * l2w_in[...,tf.newaxis]  * l2w_out[...,tf.newaxis, :]
               lgw = lgweights * lgw_in[...,tf.newaxis]  * lgw_out[...,tf.newaxis, :]
        ...
        def loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, X, Y, Y2): # weight_mean, weight_scale,
            ...
            lp = tf.reduce_sum(lpp['l1w_in']+lpp['l1w_out']+lpp['l2w_in']+lpp['l2w_out']+lpp['lgw_in']+lpp['lgw_out']+lpp['w_in']+lpp['w_out']+lpp['sm_in']+lpp['sm_out'])
            ...
            pred = mk(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out)
            ...
            lpp.update({"rl_l1":rl_l1, "rl_l2":rl_l2, "rl_lg":rl_lg, "rl_w":rl_w, "rl_sm":rl_sm, "laplacian_prob_reg_loss":reg_loss3,
                        ...
                        "poisson_lp_mean":tf.reduce_mean(poisson_lp), "gaussian_lp_mean":tf.reduce_mean(gaussian_lp),
                        "logpoints":tf.reduce_mean(logpoints), "mean_points":tf.reduce_mean(actual_points), "real_points":tf.reduce_mean(real_points)
                       })
        ...
        def analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, X, Y, Y2): # weight_mean, weight_scale,
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, X, Y, Y2)

        def target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out):
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, x_train_scaled, outputs_, Y_train)["total"]


        def target_log_prob_cat_test(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out):
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, x_test_scaled, test_outputs_, Y_test)["total"]


        def analyse_target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out):
            print("Train")
            al = analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, x_train_scaled, outputs_, Y_train)
            return {k:v.numpy() for k,v in al.items()}

        def analyse_test_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out):
            print("Test")
            return {k:v.numpy() for k,v in analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out, x_test_scaled, test_outputs_, Y_test).items()}

        print(joint_model)
        jm_components = joint_model.event_shape_tensor().keys()
        surrogate_posterior = tfp.experimental.vi.build_factored_surrogate_posterior(
            event_shape=[joint_model.event_shape_tensor()[c] for c in [ 'l1w_in', 'l1w_out','l2w_in', 'l2w_out', 'lgw_in', 'lgw_out', 'w_in', 'w_out', 'sm_in', 'sm_out']],
        ...
        if FLAGS.action=="train":
            ...
            for i in range(FLAGS.train_steps):
                ...
                l1w_in, l1w_out, l2w_in, l2w_out, lgw_in, lgw_out, w_in, w_out, sm_in, sm_out = surrpost_vars
                ...
                def eval_score(loss0a, loss0b):
                    ...
                    gs0 = 0.5*loss0a["gaussian_lp_mean"] - loss0b["gaussian_lp_mean"]
                    pt0 = 0.5*loss0a["real_points"] - loss0b["real_points"]
                    #score0=sm0+lp0+ps0+pt0-10*loss0b["real_points"]
                    score0 = sm0 + 10*lp0 + 0.3*ps0 + gs0 + 0.1*pt0
                    return(score0)
        ...
        def plot_searchlog(xlabel, ylabel, ylabel2=None, x=None, y=None, y2=None, logy=False, lim=5, lim_x=None, lim_y=None):
            ...
            plt.show()
        if False:
            plot_searchlog("reg1", "pts", "tpts", lim=None)
