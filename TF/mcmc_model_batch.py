    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x):
            def make_outputs(weights, smweights): #tfd.Sample(
               ...
               compdist_logits = 1.0 * smweights[..., tf.newaxis, :mixcom, :] * tf.ones([x.shape[0], 1, 1],
                                                                                            dtype=tf.float64)
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                smweights = tfd.Independent(tfd.Normal(loc=tf.math.log(0.0001+tf.reduce_mean(tf.cast(outputs_, tf.float64), axis=0)),#tf.zeros(shape=[mixcom, 49], dtype=tf.float64),
                                                       scale=3.0*tf.ones(shape=[mixcom, 49], dtype=tf.float64)),
                                            reinterpreted_batch_ndims=2),
        ...
        def loss_function(weight_mean, weight_scale, weights, smweights, X, Y):
            ...
            actual_points = tf.minimum(match_probs, 0.75) * tf.cast(achievable_points, tf.float64) # (5508, 49) # tf.minimum(match_probs, 1.0)
            ...
            lpp.update({"laplacian_reg_loss":reg_loss, "laplacian_reg_loss2":tf.reduce_sum(reg_loss2), "joint_model_log_prob":lp, "mean_points":tf.reduce_mean(actual_points), "real_points":tf.reduce_mean(real_points)})
            #lpp["total"] = tf.reduce_sum(lp) - tf.reduce_sum(reg_loss) + 1.5*tf.reduce_sum(actual_points)#- lpp["outputs"] + lpp["outputs"]*0.3
            lpp.update({"kld": kld})
            lpp["total"] = lpp["weight_mean"]+lpp["weight_scale"]+lpp["weights"]  + 1500*tf.reduce_mean(actual_points) +lpp["smweights"]# - 1*tf.reduce_sum(kld) # - tf.reduce_sum(reg_loss2) - tf.reduce_sum(reg_loss)+ 0.0001 * lpp["outputs"]
            return lpp
        ...
        def target_log_prob_cat_batched( weight_mean, weight_scale, weights, smweights):
            ...
            x0 = xy0[...,:s]
            y0 = xy0[...,s:]
            return loss_function(weight_mean, weight_scale, weights, smweights, tf.cast(x0, tf.float64), y0)["total"]
        ...
        inputfilename = sorted([f for f in os.listdir() if re.search("mcmc_batch"+str(batch_size)+"_"+FLAGS.target_system+".*pickle", f)])
        ...
        states = sample()
        filename = "mcmc_batch"+str(batch_size)+"_"+FLAGS.target_system+"_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        ...
        if True:
            ...
            avg_points_train = np.concatenate([tf.reduce_mean(avg_points(weights[i:(i+stepsize)], smweights[i:(i+stepsize)], x_train_scaled, outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            avg_points_test = np.concatenate([tf.reduce_mean(avg_points(weights[i:(i+stepsize)], smweights[i:(i+stepsize)], x_test_scaled, test_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
