...
from Estimators import Skimmed_Model as themodel
...
plot_prefix_list = ["pg/", "pgpt/", "pghb/", "ps/", "pspt/", "sp/", "sm/", "smpt/"]
...
def plot_predictions_2(predictions, features, labels, team_onehot_encoder, prefix="sp/", is_prediction=False, dataset = "Test", skip_plotting=False):
  ...
  with tf.Session() as sess:
    points_tensor = themodel.calc_points(tensor[:,0],tensor[:,1], tensor[:,2], tensor[:,3], is_home)[0]
    sky_points_tensor = themodel.calc_sky_points(tensor[:,0],tensor[:,1], tensor[:,2], tensor[:,3])
    pistor_points_tensor = themodel.calc_pistor_points(tensor[:,0],tensor[:,1], tensor[:,2], tensor[:,3])
    df['Pt'], df['SkyPt'], df['PistorPt'] = sess.run([tf.cast(points_tensor, tf.int8), tf.cast(sky_points_tensor, tf.int8), tf.cast(pistor_points_tensor, tf.int8)])
    df['Prefix'] = prefix[:-1]
    print()
  ...
  def preparePrintData(prefix, df):
    if prefix!="ens/":
      return df[["Team1", "Team2", "act", "pred", "Where", "est1","est2","Pt", "SkyPt", "PistorPt", "Prefix", "win", "draw", "loss", "winPt", "drawPt", "lossPt"]]
    else:
      return df[["Team1", "Team2", "act", "pred", "Pt", "SkyPt", "PistorPt", "Prefix", "Strategy"]]
  ...
  avg_pistor_points = np.mean(df["PistorPt"])
  avg_sky_points = np.mean(df["SkyPt"])
  ...
  ax1.set_title("{}  -  Sky:{:.04f}   Pistor:{:.04f}".format(prefix[:-1], avg_sky_points, avg_pistor_points)).set_size(15)
...
#  results = results.iloc[-710:]   
  ...
  for point_type in ["p_points", "s_points", "z_points"]:
    if point_type=="z_points":
      hlp=[405/9/27]
    elif point_type=="s_points":
      hlp=[251 / 6 / 27, 274/6/27]
    else:
      hlp=[285/9/27, 247/9/26, 196 / 9 / 27]
    ...
    data.plot(kind='bar', stacked=False, ax=ax, 
              ...
              ylim=(data.Test.max()*0.7, np.max(data[["Train","Test"]].max())))
...
def evaluate_metrics_and_predict(model, features, labels, model_dir, outputname, skip_predictions, pred_X = None, pred_y=None):
...
def train_and_eval(model_dir, train_steps, train_data, test_data, 
                   predict_new, save_steps, skip_download, max_to_keep, evaluate_after_steps, skip_predictions, skip_plotting):
...
#      for prefix in ["ens/"] + themodel.prefix_list ] #["ens/", "p1/", "p1pt/", "p3/", "p2/", "p2pt/", "p5/", "p4/", "p4pt/", "p6/", "p7/", "sp/", "sppt/", "sm/", "smpt/", "smhb/"]]
  ...
  for i in range(train_steps//evaluate_after_steps):
    ...
    test_result, test_prediction = evaluate_metrics_and_predict(model, test_X, test_y, model_dir, "test", skip_predictions, pred_X, pred_y)
    train_result, train_prediction = evaluate_metrics_and_predict(model, train_X, train_y, model_dir, "train", skip_predictions)
    ...
    if not skip_predictions:
      ...
      #for prefix in ["sp/"]:
        test_predictions.extend ([plot_predictions_2(test_prediction, test_X, test_y, team_onehot_encoder, prefix, dataset = "Test", skip_plotting=skip_plotting)])                             
        train_predictions.extend([plot_predictions_2(train_prediction, train_X, train_y, team_onehot_encoder, prefix, dataset = "Train", skip_plotting=skip_plotting)])                             
      train_predictions = pd.concat(train_predictions) 
      test_predictions  = pd.concat(test_predictions)  
    
      test_ens_predictions = pd.concat([plot_predictions_2(test_prediction, test_X, test_y, team_onehot_encoder, "ens/", dataset = "Test", skip_plotting=skip_plotting)])                          
      train_ens_predictions= pd.concat([plot_predictions_2(train_prediction, train_X, train_y, team_onehot_encoder, "ens/", dataset = "Train", skip_plotting=skip_plotting)])                             
...
#  utils.print_tensors_in_checkpoint_file(FLAGS.model_dir, tensor_name="Model/RNN_1/rnn/multi_rnn_cell/cell_0/gru_cell/gates/kernel", target_file_name="rnn_gates_kernel.csv", all_tensor_names=False, all_tensors=False)
  ...
  train_and_eval(FLAGS.model_dir, FLAGS.train_steps,
                 ...
                 FLAGS.evaluate_after_steps, FLAGS.skip_predictions, FLAGS.skip_plotting)
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      #default=True, 
      default=False, 
      help="Create predicted data or not"
  )
  parser.add_argument(
  ...
  parser.add_argument(
      "--model_dir",
      type=str,
      #default="D:/Models/model_map_1415",
      #default="D:/Models/model_map_1516", # always use UNIX-style path names!!!
      default="D:/Models/model_skimmed_1718_2", # always use UNIX-style path names!!!
      ...
      #default="D:/Models/model_map_1718",
