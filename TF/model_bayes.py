...
def plot_checkpoints(df, predictions, prefix=None):
  if prefix is None:
    li = themodel.prefix_list
  else:
    li = [prefix]
  for prefix in li: 
...
#  pred_idx = [val for sublist in pred_idx for val in sublist]
  ...
  # No tensorflow training required
  ...
  classlabels = [str(t1)+":"+str(t2) for t2 in range(7) for t1 in range(7) ]
  ...
  def eval_new_sample(X, Y, pipeline, split):
...
#    print('Accuracy of Test Data :', pipeline.score(X_test,Y_test0))
    ...
    y_pred = [classlabels[i] for i in y_pred.astype(int)]
    print(Counter(y_pred))
    print(Counter(y_true))
    y_traintrue = [classlabels[i] for i in Y_train.astype(int)]
    print(metrics.classification_report(y_true, y_pred))
    print()
    cm = confusion_matrix(y_true, y_pred)
    ul = unique_labels(y_traintrue, y_pred)
    ulcm = unique_labels(y_true, y_pred)
    #print(ul)
    #print(cm.sum(axis=0)) # predicted labels
    #print(cm.sum(axis=1)) # true labels
    ispred = (cm.sum(axis=0)>0)
    predicted_results = [ (i,u) for (i, u) in enumerate(ulcm) if ispred[i] == True]
    #print(predicted_results)
    print(Counter(y_pred))
    sparse_results = {k[1]:Counter({u:cm[j,k[0]] for j,u in enumerate(ulcm) if cm[j,k[0]]>0}) for k in predicted_results}
    ...
    def scoring(y_true, y_pred):
      df = pd.DataFrame({
      "gs" : [int(g[0]) for g in y_true],
      "gc" : [int(g[2]) for g in y_true],
      "pgs" : [int(g[0]) for g in y_pred],
      "pgc" : [int(g[2]) for g in y_pred],
      })
      df["tend"]=1 * (np.sign(df.gs-df.gc)==np.sign(df.pgs-df.pgc))
      df["gdiff"]=1 * ((df.gs-df.gc)==(df.pgs-df.pgc))
      df["full"]=1 * ((df.gs==df.pgs)&(df.pgc==df.gc))
      df["pistor"]=df["tend"]+df["gdiff"]+df["full"]
      df["sky"]=2*df["tend"]+3*df["full"]
      return df
    
    df_test = scoring(y_true, y_pred)
    df_train = scoring(ty_true, ty_pred)
    df_test["dataset"]="test"
    df_train["dataset"]="train"
    df = pd.concat([df_test, df_train], axis=0, ignore_index=True)
    #print(df)
    print(df.groupby("dataset").mean())
    print(df.groupby("dataset").gs.count())
  ...
  def visualize_results(proba, used_labels, prefix, idx_train, idx_test, idx_pred=[]):
    ...
    reverse_mapping={}
    for i,u in enumerate(used_labels):
      gs=int(u[0])
      gc=int(u[2])
  #    print(i, gs, gc, gs*7+gc)
      p_pred_12[:,gs*7+gc]=proba[:,i]
      reverse_mapping[i]=gs*7+gc
  ...
  #  print(np.sum(p_pred_12.reshape(-1,7,7), axis=1).shape)
    ...
    pred[prefix+"p_marg_1"] = np.squeeze(np.vsplit(np.sum(p_pred_12.reshape(-1,7,7), axis=2), n))
    pred[prefix+"p_poisson_1"] = [[0.1]*7]*n
    pred[prefix+"p_marg_2"] = np.squeeze(np.vsplit(np.sum(p_pred_12.reshape(-1,7,7), axis=1), n))
    pred[prefix+"p_poisson_2"] = [[0.1]*7]*n
    pred[prefix+"ev_goals_1"] = [sum([i*marg[i] for i in range(7)]) for marg in pred[prefix+"p_marg_1"]]
    pred[prefix+"ev_goals_2"] = [sum([i*marg[i] for i in range(7)]) for marg in pred[prefix+"p_marg_2"]]
    
    #plot_softprob(pred, 0, 0, prefix=prefix)
  #  print(reverse_mapping)
  
    pgsgc = np.argmax(p_pred_12, axis=1)
    pGS = pgsgc // 7
    pGC = np.mod(pgsgc, 7)
  ...
  #  print(pred[prefix+"pred"].shape)
    print({k:v.shape for k,v in predictions.items()})  
    results = [enrich_predictions(predictions, features_batch, labels_batch, team_onehot_encoder, p, data_set, 0) for p in [prefix] ]
    results = pd.concat(results, sort=False)
  #  print(results)
  ...
  class ColumnSelector(BaseEstimator, TransformerMixin):
      """Select slice from sparse matrix"""
  ...
  X = np.concatenate([X, X_hist], axis=1)    
  Y = (7*np.minimum(labels_array[:,0], 6) + np.minimum(labels_array[:,1], 6))
...
#
#    
#  eval_new_sample(X, Y, pipeline)
  home_only = False
  if home_only:
    test_idx = test_idx[::2]
    train_idx = train_idx[::2]
  
  data_index = test_idx+train_idx
  print("len(data_index)", len(data_index))
      
#  idx_train, idx_test = train_test_split(data_index, shuffle = True) #stratify=Y, 
#  proba, used_labels = eval_new_sample(X, Y, pipeline2, (train_idx, test_idx))
#  visualize_results(proba, used_labels, train_idx, test_idx)
# 
#  proba, used_labels = eval_new_sample(X, Y, pipeline2, (idx_train, idx_test))
#  visualize_results(proba, used_labels, idx_train, idx_test)
...
# Define custom loss
  def softpoint_loss(model):
      # Create a loss function that adds the MSE loss to the mean of all squared activations of a specific layer
#      def loss(y_true,y_pred):
#          return K.mean(K.square(y_pred - y_true) + K.square(layer.output), axis=-1)

      def loss(y_true,y_pred):
        #print("y_true", y_true)
        #print("model.inputs", model.inputs)
        #print("model.output", model.output)
        achievable_points_home = tf.matmul(y_true, tc_home_points_i) 
        achievable_points_away = tf.matmul(y_true, tc_away_points_i) 
        t_is_home_bool = tf.cast(model.inputs[0][:,0], tf.bool)
        achievable_points =  tf.where(t_is_home_bool, achievable_points_home, achievable_points_away)
                               
        #achievable_points = tf.squeeze(tf.gather(tc_home_points_i, tf.cast(y_true, tf.int32) ), axis=1)
        #print("achievable_points", achievable_points)
  ...
  def home_metric(model):
      def is_home(y_true,y_pred):
        return -K.mean(model.inputs[0][:,0])
      return is_home

  def z_points(model):
      def z_points(y_true,y_pred):
        achievable_points_home = tf.matmul(y_true, tc_home_points_i) 
        achievable_points_away = tf.matmul(y_true, tc_away_points_i) 
        t_is_home_bool = tf.cast(model.inputs[0][:,0], tf.bool)
        achievable_points =  tf.where(t_is_home_bool, achievable_points_home, achievable_points_away)
        ...
        print(prediction)
        prediction_oh = tf.one_hot(tf.cast(prediction, tf.int32), depth=49)                       
        print(prediction_oh)
        z_points = achievable_points * prediction_oh  
        return K.mean(K.sum(z_points, axis=1))
      return z_points
    
  X0shape = combined_predictions.fit_transform(X[train_idx], Y[train_idx]).shape
...
#        results.to_csv(f, header=f.tell()==0, quoting=csv.QUOTE_NONNUMERIC, index=False, line_terminator='\n')
  ...
  def build_fn(optimizer="Adam", learning_rate=3e-2, weight_decay=0.02, layers=2, units=16, dropout_rate=0.01):
    ...
    model = mlp_model(layers=layers,
                                  ...
                                  input_shape=X0shape[1:], #X_train_standardized.shape[1:],
  ...
  #  optimizer = keras.optimizers.Adam(lr=learning_rate, decay=0.0)
    ...
    model.compile(optimizer=optimizer, 
                  loss=softpoint_loss(model), #"sparse_categorical_crossentropy", 
                  metrics=["accuracy", z_points(model)])
    print(model.summary())
    return(model)
  ...
  #keras_clf.classes_ = np.array([i for i in range(49)]) 
  #lb = LabelBinarizer()
  #lb.fit(np.array([i for i in range(49)]))  
  pipeline = Pipeline([
  ...
  if False:
    ...
    proba, used_labels = eval_new_sample(X, Y, pipeline, (idx_train, idx_test))
    #visualize_results(proba, used_labels, idx_train, idx_test)
    print("keras_clf.classes_", keras_clf.classes_)
    print("keras_clf.n_classes_", keras_clf.n_classes_)
    
    idx_train, idx_test = train_test_split(data_index[::2])
    proba, used_labels = eval_new_sample(X, Y, pipeline, (idx_train, idx_test))
    #visualize_results(proba, used_labels, idx_train, idx_test)
  ...
  #print(fixed_pipeline.get_params(True))
  #proba, used_labels = eval_new_sample(X, Y, pipeline3, split=(X_train, X_test, y_train, y_test))
  #proba, used_labels = eval_new_sample(X, Y, fixed_pipeline, split=(X_train, X_test, y_train, y_test))
  val_labels = keras_clf2.oh.transform(Y[test_idx].reshape(-1, 1))
...
#  train_slice = slice(np.min(train_idx), np.max(train_idx)+1)
#  test_slice = slice(np.min(test_idx), np.max(test_idx)+1)
#  X_train = X[train_slice]
#  y_train = Y[train_slice]
#  X_test = X[test_slice]
#  y_test = Y[test_slice]
#  
#  proba, used_labels = eval_new_sample(X, Y, pipeline2, split=(X_train, X_test, y_train, y_test))
#  print(used_labels)
...
##  data_set = ["train"]*len(train_idx)+["test"]*len(test_idx)+["pred"]*len(pred_idx)
#  data_index = test_idx
#  data_set = ["test"]*len(test_idx)
#  features_batch = features_arrays['match_input_layer'][data_index]
#  labels_batch = labels_array[data_index]
##  print(labels_batch.shape)
##  print(pred[prefix+"pred"].shape)
#  results = [enrich_predictions(pred, features_batch, labels_batch, team_onehot_encoder, p, data_set, 0) for p in [prefix] ]
#  results = pd.concat(results, sort=False)
##  print(results)
#  plot_checkpoints(results, pred, prefix=prefix)
...
#  pipeline.fit(X_train, y_train)
#  #print(lb.classes_)
#  print(keras_clf.classes_)
#  p_train = pipeline.predict(X_train)
#  p_test = pipeline.predict(X_test)
##  acc_train = pipeline2.score(X_train, y_train)
##  acc_test = pipeline2.score(X_test, y_test)
##  print("train=", acc_train, "test=", acc_test)  
#  print(Counter([classlabels[int(i)] for i in p_test]))
#  print(Counter([classlabels[int(i)] for i in p_train]))
##  print(Counter([classlabels[min(48,int(i))] for i in p_test]))
##  print(Counter([classlabels[min(48,int(i))] for i in p_train]))
##  print(Counter([classlabels[min(48,int(i))] for i in Y]))  
  return
