    ...
    if __name__ == "__main__":
        ...
        def loss_function(w1, w2, r3, weight_mean, weight_scale, weights, smweights, X, Y, X_cv, Y_cv):

            mk, joint_model = make_joint_mixture_model(X)
            mk_cv, joint_model_cv = make_joint_mixture_model(X_cv)
            # if tf.rank(weights)==3:
            #     #Y = tf.tile(Y, [weights.shape[0], 1])
            #     #Y = tf.stack([Y]*weights.shape[0], axis=0)
            ...
            lp = tf.reduce_sum(lpp['w1']+lpp['w2']+lpp['r3']+lpp['weight_mean']+lpp['weight_scale']+lpp['weights']+lpp['smweights'])*(X.shape[-2]) + tf.reduce_sum(lpp['outputs'], axis=-1)*1 # *w1
            reg_loss = create_laplacian_loss(smweights, alpha=r3)
            ...
            pred_cv = mk_cv(weights, smweights)
            ...
            lpp["total"] = tf.reduce_sum(lp) + w2*tf.reduce_sum(actual_points) - w2*tf.exp(10*(tf.stop_gradient(tf.reduce_sum(actual_points)/tf.reduce_sum(actual_points_cv) - 1))) - tf.reduce_sum(reg_loss)#- lpp["outputs"] + lpp["outputs"]*0.3 #

            return lpp

        def analyse_losses(w1, w2, r3, weight_mean, weight_scale, weights, smweights, X, Y, X_cv, Y_cv):
            return loss_function(w1, w2, r3, weight_mean, weight_scale, weights, smweights, X, Y, X_cv, Y_cv)
        ...
        def target_log_prob_cat(w1, w2, r3, weight_mean, weight_scale, weights, smweights):
            return loss_function(w1, w2, r3, weight_mean, weight_scale, weights, smweights, x_train_scaled, outputs_, x_train_cv_scaled, cv_outputs_)["total"]


        def target_log_prob_cat_test(w1, w2, r3, weight_mean, weight_scale, weights, smweights):
            return loss_function(w1, w2, r3, weight_mean, weight_scale, weights, smweights, x_test_scaled, test_outputs_, x_test_scaled, test_outputs_)["total"]


        def analyse_target_log_prob_cat(w1, w2, r3, weight_mean, weight_scale, weights, smweights):
            return analyse_losses(w1, w2, r3, weight_mean, weight_scale, weights, smweights, x_train_scaled, outputs_, x_train_cv_scaled, cv_outputs_)
        ...
        sampler_bij = tfp.mcmc.TransformedTransitionKernel(
            ...
            bijector=[tfb.Softplus(), tfb.Chain([tfb.Softplus(), tfb.Scale(scale=tf.constant(2.0, dtype=tf.float64))]), tfb.Softplus(), tfb.Identity(), tfb.Softplus(), tfb.Identity(), tfb.Identity()]) # tfb.Affine(shift=0.135, scale_identity_multiplier=0.01, dtype=tf.float64)
        ...
        if FLAGS.checkpoints!="" and len(inputfilename) > 0:
            ...
            w1, w2, r3, weight_mean, weight_scale, weights, smweights = all_states
            initial_state = [w1[-1], w2[-1], r3[-1], weight_mean[-1], weight_scale[-1], weights[-1], smweights[-1]]
        ...
        w1, w2, r3, weight_mean, weight_scale, weights, smweights = states.all_states # , cdist

        current_state = [w1[-1], w2[-1], r3[-1], weight_mean[-1], weight_scale[-1], weights[-1], smweights[-1]]
        current_state2 = [w1[-1], w2[-1], r3[-1], weight_mean[-10], weight_scale[-10], weights[-10], smweights[-10]]
        ...
        if True:
            ...
            stepsize = 25
            loss_curve = [target_log_prob_cat(w1[i:(i+stepsize)], w2[i:(i+stepsize)], r3[i:(i+stepsize)], weight_mean[i:(i+stepsize)], weight_scale[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i
                          in range(0, weights.shape[0], stepsize)]
            loss_curve_test = [target_log_prob_cat_test(w1[i:(i+stepsize)], w2[i:(i+stepsize)], r3[i:(i+stepsize)], weight_mean[i:(i+stepsize)], weight_scale[i:(i+stepsize)], weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i in range(0, weights.shape[0], stepsize)]
            ...
            avg_points_train_cv = np.concatenate([tf.reduce_mean(avg_points(weights[i:(i+stepsize)],
            ...
            real_points_train = np.concatenate([tf.reduce_mean(real_points(weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_scaled, outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
            real_points_train_cv = np.concatenate([tf.reduce_mean(real_points(weights[i:(i+stepsize)],
                                                                         smweights[i:(i+stepsize)], x_train_cv_scaled, cv_outputs_), axis=-1) for i in range(0, weights.shape[0], stepsize)], axis=0)
        ...
        if True:
            ...
            t2=np.mean(np.mean(np.array([make_mixture_probs_train_cv(weights[i:(i+stepsize)], smweights[i:(i+stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)]), axis=0), axis=0)
