    ...
    def get_train_test_data(model_dir, train_seasons, test_seasons, data_dir, useBWIN):
        ...
        if False:
            ...
            all_data = all_data.loc[filter].reset_index()
            all_labels = all_labels.loc[filter].reset_index()
            all_data.drop(columns="index", inplace=True)
            all_labels.drop(columns="index", inplace=True)
            print(all_data.shape)
            print(all_labels.shape)


        is_train = all_data.Season.astype(str).apply(lambda x: x.zfill(4)).isin(train_seasons)
        is_test = all_data.Season.astype(str).apply(lambda x: x.zfill(4)).isin(test_seasons)
        is_test.value_counts()
        ...
        feature_names = ['Time', 't1games', 't2games','t1dayssince', 't2dayssince', 't1dayssince_ema', 't2dayssince_ema',
        ...
        feature_names += ['T1_CUM_T1__PT', 'T2_CUM_T2__PT', 'T1_EMA__PT', 'T2_EMA__PT', 'T1_CUM_T1_W__PT', 'T2_CUM_T2_W__PT',
                          'T1_CUM_T2__PT', 'T2_CUM_T1__PT', 'T1_CUM_T2_W__PT', 'T2_CUM_T1_W__PT', 'T12_CUM_T1__PT', 'T12_CUM_T1_W__PT',
                          'T21_CUM_T2__PT', 'T21_CUM_T2_W__PT', 'T12_CUM_T12__PT', 'T12_CUM_T12_W__PT', 'T1221_CUM__PT', 'T1221_CUM_W__PT']
        ...
        feature_names += ['t1_prev_ET', 't2_prev_ET', 't1_prev_PEN', 't2_prev_PEN',
                         'Apr', 'Aug', 'Dec', 'Feb', 'Jan', 'Jul', 'Jun', 'Mar', 'May', 'Nov', 'Oct', 'Sep',
                         't1_prev_FT_ET_PEN_R_draw', 't1_prev_FT_ET_PEN_R_loss', 't1_prev_FT_ET_PEN_R_win',
                         't2_prev_FT_ET_PEN_R_draw', 't2_prev_FT_ET_PEN_R_loss', 't2_prev_FT_ET_PEN_R_win',
                          'cup'
                          ]
    ...
    def get_input_data(model_dir, train_data, test_data, data_dir, useBWIN):
        all_data, all_labels, all_features, teamnames, team_mapping, feature_names = get_train_test_data(model_dir, train_data, test_data,
                                                                                          data_dir, useBWIN)
        print(all_data.iloc[1000])
        ...
        data = (all_data, teamnames, features_arrays, labels_array, team_onehot_encoder, label_column_names, feature_names)
        return data
    ...
    def find_next_params_from_gaussian_process(searchlog, num_iters3=1000):
        ...
        lower_bound = np.quantile(observations_, 0.9) - 5 * (
                    np.quantile(observations_, 0.9) - np.quantile(observations_, 0.35))
        ...
        observation_index_points_ = searchlog.loc[:,
                                    ["reg1", "reg2", "reg2p", "reg2g", "reg3", "reg4", "wkl", "wlpt", "wpois", "wgaus",
                                     "wsmx"]].to_numpy()
        # print(current_point)
        tfk = tfp.math.psd_kernels
        ...
        wgt_min = 0.1
        wgt_max = 1000000
        lower_limit = np.log(np.array([reg_min] * 6 + [wgt_min] * 5))
        upper_limit = np.log(np.array([reg_max] * 6 + [wgt_max] * 5))
        limit_range = upper_limit - lower_limit
        b = 5  # batch_size
        best_index_var = tfp.util.TransformedVariable(
            initial_value=tf.random_uniform_initializer(minval=lower_limit + 0.2 * limit_range,
                                                        maxval=upper_limit - 0.2 * limit_range)(shape=[b, k],
                                                                                                dtype=tf.float64),
            name="best_index_var", dtype=np.float64,
            bijector=tfb.SoftClip(low=tf.constant(lower_limit, dtype=tf.float64),
                                  high=tf.constant(upper_limit, dtype=tf.float64)))
        ...
        print(current_point)
        print({"expected_improvement": [v.numpy() for v in
                                        expected_improvement(np.array(current_point)[tf.newaxis, ...])],
    ...
    def create_maxpoint_prediction(df, point_matrix):
    ...
    def create_maxpoint_prediction_from_mean(probs, Ylen, point_matrix):
    ...
    def create_argmax_prediction_from_mean(probs, Ylen, point_matrix):
    ...
    def plot_weights_distribution(surrogate_posterior, allweights):
        ...
        l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out = surrpost_vars
        l1weights, l2weights, lpweights, lgweights, ohweights, smweights = allweights
        ...
        sns.kdeplot(lpweights.numpy().flatten(), ax=ax[4][2], clip=(np.mean(lpweights.numpy())-3*np.std(lpweights.numpy()), np.mean(lpweights.numpy())+3*np.std(lpweights.numpy())))
        plt.show()
    ...
    def make_joint_mixture_model(x, weights=None):
        ...
        def create_weights():
            ...
            return (
                ...
                , tf.Variable(
                    0.001 * tf.random.normal(shape=[batch_size, L1size + 1, L1size], dtype=tf.float64))
                , tf.Variable(0.01 * tf.random.normal(shape=[batch_size, L1size + 1, len(label_columns_poisson)],
                                                      dtype=tf.float64))
                , tf.Variable(0.01 * tf.random.normal(shape=[batch_size, L1size + 1, len(label_columns_gaussian)],
                                                      dtype=tf.float64))
        ...
        l1weights, l2weights, lpweights, lgweights, ohweights, smweights = weights

        def make_forward_path(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out):
            # print("make_forward_path()")
            # print(x.shape)

            l1w = l1weights[..., :-1, :] * l1w_in[...,tf.newaxis]  * l1w_out[...,tf.newaxis, :]
            l2w = l2weights[..., :-1, :] * l2w_in[...,tf.newaxis] * l2w_out[...,tf.newaxis, :]
            lpw = lpweights[..., :-1, :] * lpw_out[...,tf.newaxis, :]
            ...
            x2 = x1 + tf.matmul(x1, l2w) + l2weights[..., -1:, :]
            xp = tf.matmul(x2, lpw) + lpweights[..., -1:, :]
            x2g = tf.matmul(x2, lgw[..., :, :]) + lgweights[..., -1:, :]
            l = tf.matmul(x2, wgt[..., :, :]) + ohweights[..., -1:, :]
            sml = tf.matmul(x2, smw[..., :, :]) + smweights[..., -1:, :]
            return tfd.JointDistributionNamed(dict(
               mainresult = tfd.Independent(tfd.OneHotCategorical(logits=l), reinterpreted_batch_ndims=1),
               sidebets = tfd.Independent(tfd.Poisson(log_rate=xp), reinterpreted_batch_ndims=2),
        ...
        joint_model = tfd.JointDistributionNamed(dict(
            l1w_in=tfd.Independent(
                tfd.Normal(loc=tf.zeros(shape=[batch_size, x.shape[1] + 0], dtype=tf.float64),
                           scale=sc * tf.ones(shape=[batch_size, x.shape[1] + 0], dtype=tf.float64)), reinterpreted_batch_ndims=2),
        ...
        def make_surrogate_posterior():
            ...
            return tfp.experimental.vi.build_factored_surrogate_posterior(
                event_shape=[joint_model.event_shape_tensor()[c] for c in [ 'l1w_in', 'l1w_out','l2w_in', 'l2w_out', 'lpw_out', 'lgw_out', 'w_out', 'sm_out']],
        ...
        def loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, Y, Y2, metaparams, reduce_axis=None):  # weight_mean, weight_scale,
            reg1, reg2, reg2p, reg2g, reg3, reg4, kl, lpt, pois, gaus, smx = metaparams
            ...
            lpp['l1w_out'] /= tensor_size(l1w_out)
            lpp['l2w_in'] /= tensor_size(l2w_in)
            lpp['l2w_out'] /= tensor_size(l2w_out)
            lpp['lpw_out'] /= tensor_size(lpw_out)
            lpp['lgw_out'] /= tensor_size(lgw_out)
            lpp['w_out'] /= tensor_size(w_out)
            lpp['sm_out'] /= tensor_size(sm_out)

            lp = tf.reduce_sum(
                lpp['l1w_in'] + lpp['l1w_out'] + lpp['l2w_in'] + lpp['l2w_out'] + lpp['lpw_out'] + lpp['lgw_out'] + lpp['w_out'] + lpp['sm_out'])
            # reg_loss = create_laplacian_loss(smweights, alpha=0.3)
            rl_l1 = reg1 * regularizer(l1weights[..., :-1, :]) / tensor_size(l1weights)  # do not penalize offsets
            rl_l2 = reg2 * regularizer2(l2weights[..., :-1, :]) / tensor_size(l2weights)
            rl_lp = reg2p * regularizer2(lpweights[..., :-1, :]) / tensor_size(lpweights)
            rl_lg = reg2g * regularizer2(lgweights[..., :-1, :]) / tensor_size(lgweights)
            rl_w = reg3 * regularizer2(ohweights[..., :-1, :]) / tensor_size(ohweights)
            rl_sm = reg4 * regularizer2(smweights[..., :-1, :]) / tensor_size(smweights)
            reg_loss2 = rl_l1 + rl_l2 + rl_lp + rl_lg + rl_w + rl_sm
            achievable_points = tf.matmul(Y, tf.cast(point_matrix, tf.float32))
            pred = make_forward_path(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out)
            ...
            logpoints = tf.math.log(1 + predprob) * tf.cast(achievable_points,
                                                            tf.float64)  # * (0.0001+tf.stop_gradient(pred.parameters["model"]["softmax"].mean()))
            ...
            # print(X.shape)
            # print(Y.shape)
            # print(logpoints.shape)
            mask_cup = tf.cast(tf.greater(x[:, -1], 0), real_points.dtype)
            mask_league = tf.cast(tf.less_equal(x[:, -1], 0), real_points.dtype)
            cup_fraction = tf.reduce_mean(mask_cup) + 1e-8
            # sample_weights = mask_league + 1.0 * mask_cup
            accuracy = tf.reduce_sum(tf.cast(tf.matmul(prediction, wdl_mask) * tf.matmul(Y, wdl_mask), tf.float32),
                                     axis=-1)
            draw_points = tf.reduce_sum(prediction * draw_mask * achievable_points, axis=-1) / tf.reduce_mean(
                numdraws)
            logpoints = tf.reduce_sum(logpoints, axis=-1)
            softmax_lp = pred.parameters["model"]["softmax"].log_prob(Y) / Y.shape[-2] / Y.shape[-1]
            ...
            lpp.update({"rl_l1": rl_l1, "rl_l2": rl_l2, "rl_lg": rl_lg, "rl_lp": rl_lp, "rl_w": rl_w, "rl_sm": rl_sm,
                        ...
                        "softmax_lp_mean": tf.reduce_mean(softmax_lp, axis=reduce_axis),
                        "softmax_mean_acc": tf.reduce_mean(softmax_acc, axis=reduce_axis),
                        "poisson_lp_mean": tf.reduce_mean(poisson_lp, axis=reduce_axis),
                        "gaussian_lp_mean": tf.reduce_mean(gaussian_lp, axis=reduce_axis),
                        "logpoints": tf.reduce_mean(logpoints, axis=reduce_axis),
                        "mean_points": tf.reduce_mean(actual_points, axis=reduce_axis),
                        "real_points": tf.reduce_mean(real_points, axis=reduce_axis),
                        "cup_fraction": cup_fraction,
                        "accuracy_cup": tf.reduce_mean(accuracy * mask_cup, axis=reduce_axis) / cup_fraction,
                        "accuracy_league": tf.reduce_mean(accuracy * mask_league, axis=reduce_axis) / (
                                    1.0 - cup_fraction),
                        "real_points_cup": tf.reduce_mean(real_points * mask_cup, axis=reduce_axis) / cup_fraction,
                        "real_points_league": tf.reduce_mean(real_points * mask_league, axis=reduce_axis) / (
                                    1.0 - cup_fraction),
                        # "mask_max":tf.reduce_max(mask_cup), "mask_min":tf.reduce_min(mask_cup), "mask_mean":tf.reduce_mean(mask_cup),
                        })
            lpp["total"] = tf.reduce_mean(lp) - kl * tf.reduce_mean(kl_div) + lpt * tf.reduce_mean(
                logpoints) + pois * tf.reduce_mean(poisson_lp) + gaus * tf.reduce_mean(
    ...
    def calc_score_from_trained_model(data):
        ...
        def analyse_target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, reduce_axis=None):
            al = data['train']['loss'](l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, data['train']['Y_OH'],
                                       data['train']['Y'],
                                       metaparams, reduce_axis)
            return {k: v.numpy() for k, v in al.items()}

        def analyse_test_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, reduce_axis=None):
            al = data['test']['loss'](l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, data['test']['Y_OH'],
                                      data['test']['Y'],
                                      metaparams, reduce_axis)
            return {k: v.numpy() for k, v in al.items()}
    ...
    def train_surrogate_posterior(data, metaparams, learning_rate=1e-2, steps=50):
        def target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out):
            return data['train']['loss'](l1w_in, l1w_out, l2w_in, l2w_out, lpw_out, lgw_out, w_out, sm_out, data['train']['Y_OH'],
    ...
    def dispatch_main(target_distr, model_dir, train_steps, train_data, test_data,
        ...
        all_data, teamnames, features_arrays, labels_array, team_onehot_encoder, label_column_names, feature_names = get_input_data(
        ...
        return model_data, point_scheme, label_column_names, team1, team2, feature_names
    ...
    if __name__ == "__main__":
        ...
        print(FLAGS)
        model_data, point_scheme, label_column_names, team1, team2, feature_names = main([sys.argv[0]] + unparsed)
        ...
        metaparams = (reg1, reg2, reg2p, reg2g, reg3, reg4, kl, lpt, pois, gaus, smx )

        optimizer = tf.optimizers.Adam(learning_rate=1e-3)

        searchlog = pd.DataFrame(columns=["dtn", "accept", "accprob", "score", "tpts", "pts", "logp", "pois", "gaus", "smx", "reg1", "reg2", "reg2p", "reg2g", "reg3", "reg4", "wkl", "wlpt", "wpois", "wgaus", "wsmx", "reset"])
        ...
        if True and FLAGS.checkpoints != "" and len(inputfilename) > 0:
        ...
        if FLAGS.action=="train":
            ...
            for round_number in range(FLAGS.train_steps):
                    ...
                    comparedf = pd.DataFrame({"previous":oldoldmetaparams, "candidate":metaparams},
                                             index = ["reg1", "reg2", "reg2p", "reg2g", "reg3", "reg4", "kl", "lpt", "pois", "gaus", "smx"])
                    print(comparedf)
                    print(metaparams)
