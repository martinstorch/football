    ...
    if __name__ == "__main__":
        ...
        #plot_gaussian_process(searchlog, 4, 7)
        def plot_gaussian_process(searchlog, xlabel, ylabel):
            ...
            observation_index_points_ = searchlog.loc[:,
                                        ["reg1", "reg2", "reg2g", "reg2p", "reg3", "reg4", "wkl", "wlpt", "wpois", "wgaus", "wsmx"]].to_numpy()
            tfk = tfp.math.psd_kernels
            ...
            #plotdata = searchlog.loc[:, [xlabel, ylabel]]
            names = ["reg1", "reg2", "reg2g", "reg2p", "reg3", "reg4", "wkl", "wlpt", "wpois", "wgaus", "wsmx"]
            ...
            pointdata = np.concatenate([xdata[...,np.newaxis],
                                       ydata[...,np.newaxis],
                                       current_point[2:]*np.ones((k,9))], axis=-1)
            ...
            #                            Y[...,np.newaxis],
            #                            current_point[2:]*np.ones((100,100,9))], axis=-1)

            griddata = np.concatenate([
                current_point[:xlabel] * np.ones((100, 100, xlabel)),
                X[...,np.newaxis],
                current_point[slice(xlabel+1, ylabel)] * np.ones((100, 100, ylabel-xlabel-1)),
                Y[...,np.newaxis],
                current_point[slice(ylabel+1, None)]*np.ones((100,100,observation_index_points_.shape[1]-ylabel-1))], axis=-1)
            ...
            ax.set_xlabel(names[xlabel])
            ax.set_ylabel(names[ylabel])
            plt.show()
        ...
        def create_weights():
            return (
                tf.Variable(0.001*tf.random.normal(shape=[batch_size, x.shape[1] + 1, L1size], dtype=tf.float64))
                 , tf.Variable(0.001 * tf.random.normal(shape=[batch_size, L1size + 1, L2size], dtype=tf.float64))
                 , tf.Variable(0.01*tf.random.normal(shape=[batch_size, L2size + 1, len(label_columns_gaussian)], dtype=tf.float64))
                 , tf.Variable(0.01*tf.random.normal(shape=[batch_size, L2size + 1, len(label_columns_poisson)], dtype=tf.float64))
                 , tf.Variable(0.01*tf.random.normal(shape=[batch_size, L2size + 1, 49], dtype=tf.float64))
                 , tf.Variable(0.01*tf.random.normal(shape=[batch_size, L2size + 1, 49], dtype=tf.float64))
            )
        l1weights, l2weights, lgweights, lpweights, weights, smweights  = create_weights()
        ...
        def make_joint_mixture_model(x):
            def make_outputs(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, ): #tfd.Sample(
               ...
               lpw = lpweights *  lpw_out[...,tf.newaxis, :]
               wgt = weights *  w_out[...,tf.newaxis, :]
               smw = smweights  * sm_out[...,tf.newaxis, :]
               x1 = tf.nn.relu(tf.matmul(x, l1w[..., :-1, :]) + l1w[..., -1:, :]) # tf.math.tanh
               x2 = tf.nn.relu(tf.matmul(x1, l2w[..., :-1, :]) + l2w[..., -1:, :]) # tf.math.tanh
               x2p = tf.matmul(x2, lpw[..., :-1, :]) + lpw[..., -1:, :]
               x2g = tf.matmul(x2, lgw[..., :-1, :]) + lgw[..., -1:, :]
               l = tf.matmul(x2, wgt[..., :-1, :]) + wgt[..., -1:, :]
               sml = tf.matmul(x2, smw[..., :-1, :]) + smw[..., -1:, :]
               return tfd.JointDistributionNamed(dict(
                   mainresult = tfd.Independent(tfd.OneHotCategorical(logits=l), reinterpreted_batch_ndims=1),
                   sidebets = tfd.Independent(tfd.Poisson(log_rate=x2p), reinterpreted_batch_ndims=2),
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                l2w_in=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[batch_size, L1size + 1], dtype=tf.float64),
                               scale=sc * tf.ones(shape=[batch_size, L1size + 1], dtype=tf.float64)), reinterpreted_batch_ndims=2),
                l2w_out=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[batch_size, L2size], dtype=tf.float64),
                               scale=sc * tf.ones(shape=[batch_size, L2size], dtype=tf.float64)), reinterpreted_batch_ndims=2),
        ...
        # Sky 64
        reg1, reg2, reg2g, reg2p, reg3, reg4, kl, lpt, pois, gaus, smx = [
        65.95459628358374,16.401733089714277,
        1.3146886990891065,16.401733089714277,
        1.0183184492433304,
        3.520335892462611,
        4.882879118372497,
        5780.341648714803,
        ...
        metaparams = (reg1, reg2, reg2g, reg2p, reg3, reg4, kl, lpt, pois, gaus, smx )
        ...
        lossplot_fig, lossplot_ax = plt.subplots()

        def loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, X, Y, Y2, reduce_axis=None): # weight_mean, weight_scale,
            ...
            lp = tf.reduce_sum(lpp['l1w_in']+lpp['l1w_out']+lpp['l2w_in']+lpp['l2w_out']+lpp['lpw_out']+lpp['lgw_out']+lpp['w_out']+lpp['sm_out'])
            ...
            rl_lp = reg2p*regularizer2(lpweights)
            rl_w = reg3*regularizer2(weights)
            rl_sm = reg4*regularizer2(smweights)
            reg_loss2 = rl_l1+rl_l2+rl_lg+rl_lp+rl_w+rl_sm
            achievable_points = tf.matmul(Y, tf.cast(point_matrix, tf.float32))
            pred = mk(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out)
        ...
        def analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, X, Y, Y2, reduce_axis=None): # weight_mean, weight_scale,
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, X, Y, Y2, reduce_axis)

        def target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out):
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, x_train_scaled, outputs_, Y_train)["total"]


        def target_log_prob_cat_test(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out):
            return loss_function(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, x_test_scaled, test_outputs_, Y_test)["total"]


        def analyse_target_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, reduce_axis=None):
            #print("Train")
            al = analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, x_train_scaled, outputs_, Y_train, reduce_axis)
            return {k:v.numpy() for k,v in al.items()}

        def analyse_test_log_prob_cat(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, reduce_axis=None):
            #print("Test")
            return {k:v.numpy() for k,v in analyse_losses(l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out, x_test_scaled, test_outputs_, Y_test, reduce_axis).items()}
        ...
        def make_surrogate_posterior():
            return tfp.experimental.vi.build_factored_surrogate_posterior(
                event_shape=[joint_model.event_shape_tensor()[c] for c in [ 'l1w_in', 'l1w_out', 'l2w_in', 'l2w_out', 'lgw_out', 'lpw_out', 'w_out', 'sm_out']],
        ...
        searchlog = pd.DataFrame(columns=["dtn", "accept", "accprob", "score", "tpts", "pts", "logp", "pois", "gaus", "smx", "reg1", "reg2", "reg2g", "reg2p", "reg3", "reg4", "wkl", "wlpt", "wpois", "wgaus", "wsmx", "reset"])
        ...
        if FLAGS.checkpoints != "" and len(inputfilename) > 0:
            ...
            for value, variable in zip(all_weights, [l1weights, l2weights, lgweights, lpweights, weights, smweights]):
                variable.assign(value)

            reg1, reg2, reg2g, reg2p, reg3, reg4, kl, lpt, pois, gaus, smx = metaparams
        ...
        if FLAGS.action=="train":
            ...
            for round_number in range(FLAGS.train_steps):
                ...
                if os.path.exists("stop_train.cmd"):
                    print("Stop file detected. Training loop finished. ")
                    break
                ...
                if  False and round_number>0 and es2<best_score:
                    ...
                    if rounds_without_score_improvement>=0 and round_number != FLAGS.train_steps-1:
                        ...
                        for initvalue, variable in zip(create_weights()[l:], [l1weights, l2weights, lgweights, lpweights, weights, smweights]):
                            variable.assign(initvalue)
                        best_score = 1e10
                        rounds_without_score_improvement = 0
                ...
                comparedf = pd.DataFrame({"previous":oldoldmetaparams, "candidate":metaparams},
                                         index = ["reg1", "reg2", "reg2g", "reg2p", "reg3", "reg4", "kl", "lpt", "pois", "gaus", "smx"])
                print(comparedf)
                reg1, reg2, reg2g, reg2p, reg3, reg4, kl, lpt, pois, gaus, smx = metaparams
                print(metaparams)
                ...
                l1w_in, l1w_out, l2w_in, l2w_out, lgw_out, lpw_out, w_out, sm_out = surrpost_vars
                ...
                my_optimizer = tf.optimizers.Adam(learning_rate=1e-2)
                def train_surrogate_posterior(learning_rate=1e-2, steps=FLAGS.save_steps, vi_only=False):
                    my_optimizer.lr.assign(learning_rate)
                    losses = tfp.vi.fit_surrogate_posterior(
                        target_log_prob_cat,
                        surrogate_posterior,
                        optimizer=tf.optimizers.Adam(learning_rate=learning_rate),
                        num_steps=steps,
                        trainable_variables=surrogate_posterior.trainable_variables if vi_only else None,
                        seed=42,
                        sample_size=8)
                    ...
                    #lossplot_fig.clear()
                    lossplot_ax.clear()
                    lossplot_ax.plot(losses)
                    lossplot_fig.suptitle("Training Loss")
                    lossplot_fig.canvas.draw()
                    #plt.draw()
                    plt.pause(0.02)
                ...
                def optimize_surrogate_posterior(learning_rate, stepsize = 200, vi_only=False):
                    oldscore = 1e10
                    r = 0
                    while(True):
                        score, loss_train, loss_test = train_surrogate_posterior(learning_rate, steps=stepsize, vi_only=vi_only)
                        r += stepsize
                        print((r, score, vi_only, loss_train["real_points"], loss_test["real_points"]))
                        if score > oldscore:
                ...
                oldvalues = [v.numpy() for v in list(surrogate_posterior.trainable_variables) + [l1weights, l2weights, lgweights, lpweights, weights, smweights]]
                ...
                if FLAGS.save_steps==0:
                    ...
                    es3, loss3a, loss3b  = optimize_surrogate_posterior(1e-3, vi_only=True)
                    loss3b["score"] = es3
                    print(pd.DataFrame([loss1a, loss3a, dictdiff(loss1a, loss3a), loss1b, loss3b, dictdiff(loss1b, loss3b)]).T.iloc[11:])

                    es2, loss2a, loss2b  = optimize_surrogate_posterior(1e-4, vi_only=True)
                    loss2b["score"] = es2
                ...
                if accept:
                    ...
                    pickle.dump((surrogate_posterior.trainable_variables, (l1weights, l2weights, lgweights, lpweights, weights, smweights), metaparams), filehandler)
                    ...
                    for value, variable in zip(oldvalues[:l], surrogate_posterior.trainable_variables):
                        variable.assign(value)
                    for value, variable in zip(oldvalues[l:], [l1weights, l2weights, lgweights, lpweights, weights, smweights]):
                        variable.assign(value)
        ...
        if False:
            ...
            plot_searchlog("reg2", "pts", "tpts", lim_y=2)
            plot_searchlog("reg2p", "pois", y=np.max(searchlog.pois)-searchlog.pois+0.01, logy=True)
        ...
        sns.distplot(lpw_out.mean().numpy().flatten(), ax=ax[5][0], rug=True, hist=False)
        sns.distplot(lpw_out.stddev().numpy().flatten(), ax=ax[5][1], rug=True, hist=False)
        ...
        sns.kdeplot(lpweights.numpy().flatten(), ax=ax[5][2], clip=(np.mean(lpweights.numpy())-3*np.std(lpweights.numpy()), np.mean(lpweights.numpy())+3*np.std(lpweights.numpy())))
        ax[0][2].set_title("L1")
        ax[1][2].set_title("L2")
        ax[2][2].set_title("LPT")
        ax[3][2].set_title("SMX")
        ax[4][2].set_title("GAUS")
        ax[5][2].set_title("POIS")
        # try:
