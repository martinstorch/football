    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x):
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                weights=lambda weight_scale, weight_mean: tfd.Independent(
                    tfd.Normal(loc=weight_mean, scale=1.0 * weight_scale), reinterpreted_batch_ndims=2),
                l2weights=tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[l1dim+1, mixcom], dtype=tf.float64), scale=tf.ones(shape=[l1dim+1, mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=2),
...
#        cdist[-1, 2]-cdist[0, 2]
        ...
        if True:
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[w], smweights[w], l2weights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0), Y_train.shape[0]), Y_train, "Train") for i in range(10)], axis=0)
            ...
            # df2 = create_maxpoint_prediction(df)
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[w], smweights[w], l2weights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0), Y_test.shape[0]), Y_test, "Test") for i in range(10)], axis=0)
    ...
    for i in range(mixcom):
        plot_softprob_simple(make_mixture_probs_train(weights[w], smweights[w], l2weights[w]).components_distribution[0].mean()[i], title="component "+str(i))
