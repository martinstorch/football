    ...
    if __name__ == "__main__":
        ...
        def create_argmax_prediction_from_mean(probs, Ylen):
            l = probs.shape[0]//Ylen
            expPoints = np.mean(np.matmul(probs, point_matrix).reshape((-1, Ylen,49)), axis=0)
            maxpoints = pd.DataFrame({"pGS": np.argmax(probs, axis=-1) // 7,
                                      "pGC": np.mod(np.argmax(probs, axis=-1), 7),
                                      "points": np.amax(expPoints, axis=1),
                                      "est1": np.mean(np.sum(probs * arraygs, axis=1).reshape((-1, Ylen)), axis=0),
                                      "est2": np.mean(np.sum(probs * arraygc, axis=1).reshape((-1, Ylen)), axis=0)
                                      })
        ...
        made = tfb.AutoregressiveNetwork(params=2, hidden_units=[10, 10], event_shape=[x_train_scaled.shape[1]+54], activation=tf.nn.tanh, input_order="left-to-right", kernel_regularizer="l2") # tf.keras.regularizers.L2(1.0)
        made.event_shape
        sns.kdeplot(made.trainable_variables[0].numpy().flatten())
        distribution = tfd.TransformedDistribution(
            distribution=tfd.Normal(loc=tf.zeros(shape=[], dtype=tf.float64),
                                   scale=tf.ones(shape=[], dtype=tf.float64)
                                    ),
            bijector=tfb.MaskedAutoregressiveFlow(made),
            event_shape=[x_train_scaled.shape[1]+54]
        )
        distribution.sample().shape
        distribution.log_prob(tf.ones(shape=[x_train_scaled.shape[1]+54], dtype=tf.float64))
        #distribution.mode()

        xy_train = np.concatenate([x_train_scaled, Y_train[:, 0:2], outputs_[:, 49:],
                        tf.matmul(tf.cast(outputs_[:, 49:], tf.float64), tf.cast(wdl_mask, tf.float64))], axis=1)
        scaler_xy = preprocessing.StandardScaler().fit(xy_train)
        xy_train_scaled = scaler_xy.transform(xy_train)
        # Construct and fit model.
        x_ = tfkl.Input(shape=(x_train_scaled.shape[1]+54 , ), dtype=tf.float64)
        ...
        batch_size = 128
        model.fit(x=xy_train_scaled,
                  y=np.zeros((x_train_scaled.shape[0], 0), dtype=np.float64),
                  batch_size=batch_size,
                  epochs=100,
                  steps_per_epoch=x_train_scaled.shape[0] // batch_size,  # Usually `n // batch_size`.
                  shuffle=True,
                  verbose=True)
        # x12 = np.concatenate([x_train_scaled, outputs_[:, 49:]], axis=1)
        # x12t = np.concatenate([x_test_scaled, test_outputs_[:, 49:]], axis=1)
        # model.predict(x12[1000:1010])
        # model.predict(x12t[1000:1010])
        # model.predict(x12[1000:1001])
        x13 = np.stack([scaler_xy.transform(tf.concat([x_train_scaled, (i//7)*tf.ones_like(x_train_scaled)[:,0:1], np.mod(i,7)*tf.ones_like(x_train_scaled)[:,0:1], tf.one_hot(i, 49, dtype=tf.float64)[tf.newaxis, ...]*tf.ones_like(x_train_scaled)[:,0:1],
                                   tf.matmul(tf.one_hot(i, 49, dtype=tf.float64)[tf.newaxis, ...]*tf.ones_like(x_train_scaled)[:,0:1], tf.cast(wdl_mask, tf.float64))], axis=1)) for i in range(49)])
        x13t = np.stack([scaler_xy.transform(tf.concat([x_test_scaled, i//7*tf.ones_like(x_test_scaled)[:,0:1], np.mod(i,7)*tf.ones_like(x_test_scaled)[:,0:1], tf.one_hot(i, 49, dtype=tf.float64)[tf.newaxis, ...]*tf.ones_like(x_test_scaled)[:,0:1],
                                    tf.matmul(tf.one_hot(i, 49, dtype=tf.float64)[tf.newaxis, ...]*tf.ones_like(x_test_scaled)[:,0:1], tf.cast(wdl_mask, tf.float64))], axis=1)) for i in range(49)])
        x13.shape
        tf.reshape(distribution.log_prob(x13[:,1000:1001]), (7,7))
        tf.nn.softmax(distribution.log_prob(x13[:,1001:1002]), axis=0)
        tf.nn.softmax(distribution.log_prob(x13t[:,1000:1001]), axis=0)
        Y_test[1000]

        #tf.reduce_mean(outputs_[:,:49], axis=0).numpy().reshape((7,7))
        tf.reduce_mean(outputs_[:,49:], axis=0).numpy().reshape((7,7))
        idx = random.sample(range(Y_train.shape[0]), 2000)
        df2 = create_df(create_maxpoint_prediction_from_mean(tf.transpose(tf.nn.softmax(distribution.log_prob(x13[:,idx]), axis=0)), 2000 ), Y_train[idx], "Train") # Y_train.shape[0]
        plot_predictions_3(df2, "poisson", "Train", silent=False)
        df2t = create_df(create_maxpoint_prediction_from_mean(tf.transpose(tf.nn.softmax(distribution.log_prob(x13t), axis=0)), Y_test.shape[0] ), Y_test, "Test") # Y_test.shape[0]
        plot_predictions_3(df2t, "poisson", "Test", silent=False)

        g = 1004
        plot_softprob_simple(tf.nn.softmax(distribution.log_prob(x13[:,g]), axis=0))
        print(Y_train[g])
        plot_softprob_simple(tf.nn.softmax(distribution.log_prob(x13t[:,g]), axis=0))
        print(Y_test[g])
        plot_softprob_simple(0.1*tf.matmul(tf.nn.softmax(distribution.log_prob(x13t[:,1001]), axis=0)[tf.newaxis, ...], tf.cast(point_matrix, tf.float64)))

        model.predict(x_test_scaled[1000:1010])
        distribution.log_prob(x_train_scaled[1000:1010])
        distribution.log_prob(x_test_scaled[1000:1010])
        def make_joint_mixture_model(x, y=None):
        ...
        def loss_function(weight_mean, weight_scale, weights, smweights, X, Y):
            ...
            wdl1h_loss = 100*tf.reduce_sum(wdl1h_loss, axis=-1)
            ...
            wdl2h_loss = 100*tf.reduce_sum(wdl2h_loss, axis=-1)
            ...
            lpp["total"] = lp + h1_log_prob - reg_loss - wdl1h_loss - wdl2h_loss
        ...
        initial_state = [#np.random.randn(x_train.shape[1]+1, mixcom), # np.zeros # weight_mean
                         tf.zeros(shape=[nchains, 4, x_train.shape[1]+1, mixcom], dtype=tf.float64), # weight_mean
                         tf.ones(shape=[nchains, 4, x_train.shape[1]+1, mixcom], dtype=tf.float64), # weight_scale
                         tf.ones(shape=[nchains, 4, x_train.shape[1]+1, mixcom], dtype=tf.float64), # weights
            #np.ones(shape=[mixcom, 49]) / 49.
            tf.zeros(shape=[nchains, 2*mixcom, 49], dtype=tf.float64),  # smweight
        ...
        inputfilename = "mcmc_model2_20200908_124549.pickle"
        ...
        if True:
            df2ht = pd.concat([create_df_HT(create_argmax_prediction_from_mean(np.concatenate([make_h1_probs_train(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_train.shape[0]), Y_train, "Train") for i in range(10)], axis=0)
            print(df2ht)
            ...
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_train.shape[0]), Y_train, "Train") for i in range(10)], axis=0)
            ...
            plot_predictions_3( df2, "poisson", "Train", silent=False)
            #
            df2ht = pd.concat([create_df_HT(create_argmax_prediction_from_mean(np.concatenate([make_h1_probs_test(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_test.shape[0]), Y_test, "Test") for i in range(10)], axis=0)
            print(df2ht)
            print(Counter(df2ht.GS-df2ht.GC))
            print(Counter(df2ht.pGS-df2ht.pGC))
            print(Counter(df2ht.pGS+df2ht.pGC))
            ...
            # df2 = create_maxpoint_prediction(df)
            df2 = pd.concat([create_df(create_maxpoint_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[w], smweights[w]).mean().numpy() for w in range(i, weights.shape[0], 10)], axis=0)[plotcompontent], Y_test.shape[0]), Y_test, "Test") for i in range(10)], axis=0)
    ...
    for i in range(2):
        plot_softprob_simple(make_h1_probs_train(weights[w], smweights[w]).distribution.components_distribution.mean()[plotcompontent,0,i], title="component "+str(i))
    for i in range(2):
        plot_softprob_simple(make_mixture_probs_train(weights[w], smweights[w]).distribution.components_distribution.mean()[plotcompontent,0,i], title="component "+str(i))
