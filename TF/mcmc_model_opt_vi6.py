    ...
    if __name__ == "__main__":
        ...
        def find_next_params_from_gaussian_process(searchlog):
            #observations_ = -searchlog.pts
            observations_ = searchlog.score
            observations_ = -searchlog.score
            observation_index_points_ = searchlog.loc[:,
                                        ["reg1", "reg2", "reg3", "reg4", "wkl", "wlpt", "wpois", "wsmx"]].to_numpy()
            last_score = observations_[searchlog.accept].iloc[-1]
            tfk = tfp.math.psd_kernels
            ...
            wgt_min=0.1
            wgt_max=10000
            lower_limit=np.log(np.array([reg_min]*4+[wgt_min]*4))
            upper_limit=np.log(np.array([reg_max]*4+[wgt_max]*4))
            limit_range=upper_limit-lower_limit
            b = 10 # batch_size
            best_index_var = tfp.util.TransformedVariable(
                initial_value=tf.random_uniform_initializer(minval=lower_limit+0.2*limit_range, maxval=upper_limit-0.2*limit_range)(shape=[10,k], dtype=tf.float64),
            ...
            # Store the likelihood values during training, so we can plot the progress
            lls_ = np.zeros((num_iters, b), np.float64)
            for i in range(num_iters):
                with tf.GradientTape() as tape:
                    loss = -expected_improvement(best_index_var)+0.0001*tf.reduce_sum(tf.math.reduce_variance(tf.concat([observation_index_points_, best_index_var], axis=0), axis=0))
        ...
        if FLAGS.action=="train":
            for i in range(FLAGS.train_steps):
                ...
                print(pd.DataFrame([loss0a, loss1a, dictdiff(loss0a, loss1a), loss0b, loss1b, dictdiff(loss0b, loss1b)]).T.iloc[9:])
                ...
                print(pd.DataFrame([loss0a, loss2a, dictdiff(loss0a, loss2a), loss0b, loss2b, dictdiff(loss0b, loss2b)]).T.iloc[9:])

                accept_prob = np.exp((es0-es2)/0.03)
                best_points = searchlog.pts.max()
                accept = es2 < es0 or random.random() < accept_prob or i<FLAGS.warmup or pd.isna(best_points) or loss2b["real_points"]>best_points or loss2b["real_points"]>loss0b["real_points"]
                if es2 < es0:
                    accept_prob = np.nan
        ...
        def plot_searchlog(xlabel, ylabel, ylabel2=None, x=None, y=None, y2=None, logy=False, lim=5, lim_x=None, lim_y=None):
            ...
            if xlabel!="logp" and xlabel!="score":
                ...
                if lim is not None:
                    plt.xlim(np.exp(np.log(x).median() - lim_x * iqrx), np.exp(np.log(x).median() + lim_x * iqrx))
            else:
                iqrx = x.quantile(.75) - x.quantile(.25)
                if lim is not None:
                    plt.xlim((x.median() - lim_x * iqrx, x.median() + lim_x * iqrx))
            ...
            if logy:
                ...
                if lim is not None:
                    plt.ylim(np.exp(np.log(y3).median()-lim_y*iqry), np.exp(np.log(y3).median()+lim_y*iqry))
            else:
                iqry = y3.quantile(.75) - y3.quantile(.25)
                if lim is not None:
                    plt.ylim((y3.median()-lim_y*iqry, y3.median()+lim_y*iqry))
            plt.show()
        if False:
