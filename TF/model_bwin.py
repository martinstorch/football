...
#  teamnames.extend(new_data["AwayTeam"].tolist())
  ...
  file_name = "quotes_bwin.csv"
  cur_dir = os.path.dirname(model_dir + "/quotes_bwin.csv") # Dir from where search starts 
  new_quotes = pd.DataFrame()
  while True:
    file_list = os.listdir(cur_dir)
    parent_dir = os.path.dirname(cur_dir)
    if file_name in file_list:
      print ("Using " + cur_dir + "/quotes_bwin.csv as input")
      new_quotes =  pd.read_csv(
          tf.gfile.Open(cur_dir + "/quotes_bwin.csv"),
        skipinitialspace=True,
        engine="python",
        skiprows=0)
      break
      ...
      if cur_dir == parent_dir: #if dir is root dir
        print(model_dir+"/quotes_bwin.csv not found in any parent directory")
      else:
        cur_dir = parent_dir  
