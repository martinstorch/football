    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x, y=None):
            def make_h1dist(weights, smweights):
                ...
                compdist_logits_h1 = 1.0*smweights[..., tf.newaxis,:mixcom,:]*tf.ones([x.shape[0], 1, 1], dtype=tf.float64)
                # print(h1logits.shape)
                # print(compdist_logits_h1.shape)
                h1dist = tfd.Independent(
                   tfd.MixtureSameFamily(
                       mixture_distribution=tfd.Categorical(logits = 0.01*h1logits),
            ...
            def make_h2dist(weights, smweights):
                ...
                h2logits = tf.reduce_sum(tf.cast(tf.linalg.matrix_transpose(component_selection), tf.float64)[...,tf.newaxis] * h2logits_all, axis=-3)
                # print(h2logits.shape)
                h2dist = tfd.Independent(
                       tfd.MixtureSameFamily(
                           mixture_distribution=tfd.Categorical(logits = 0.01*h2logits),
                           components_distribution=tfd.OneHotCategorical(logits=compdist_logits_h2)
                       ), reinterpreted_batch_ndims=1)
            ...
            return \
                ...
                tfd.JointDistributionNamed(dict(
                    weight_mean=tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[4, x.shape[1] + 1, l1dim], dtype=tf.float64),
                                                           scale=100*tf.ones(shape=[4, x.shape[1] + 1, l1dim], dtype=tf.float64)),
                    ...
                    weight_scale=tfd.Independent(
                        ...
                        reinterpreted_batch_ndims=3),
                    weights=lambda weight_scale, weight_mean: tfd.Independent(
                        tfd.Normal(loc=weight_mean, scale=100.0 * weight_scale), reinterpreted_batch_ndims=3),
                    ...
                    smweights = tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[2*mixcom, 49], dtype=tf.float64),
                                                           scale=10.0*tf.ones(shape=[2*mixcom, 49], dtype=tf.float64)),
                                                reinterpreted_batch_ndims=2),
                    #h1dist=make_h1dist,
                    outputs=make_h2dist
             )
        ...
        _, joint_model = make_joint_mixture_model(x_train_scaled)
        _, joint_model_test = make_joint_mixture_model(x_test_scaled)
        _, joint_model_batch = make_joint_mixture_model(x_train_scaled[:batch_size])
        print(joint_model)
        print(joint_model.parameters)
        print(joint_model.event_shape)
        print(joint_model.batch_shape)
    
        joint_model.sample()
        joint_model_batch.sample(2)
        ...
        def loss_function(weight_mean, weight_scale, weights, smweights, X, Y):
            ...
            reg_loss = create_laplacian_loss(smweights, alpha=0.01)
            return lp - reg_loss + h1_log_prob
        ...
        # ]  # compdist
        s = joint_model_batch.sample(nchains)
        print(joint_model_batch.log_prob(s))
        initial_state = [s['weight_mean'].numpy(), s['weight_scale'].numpy(), s['weights'].numpy(), s['smweights'].numpy()]
        print("initial_state")
        print([v.shape for v in initial_state])
        print(target_log_prob_cat(*initial_state))
        print(analyse_losses(*initial_state, x_train_scaled[:batch_size], outputs_[:batch_size]))
    
        inputfilename = "mcmc_condprob_20200904_115536.pickle"
        ...
        #sns.kdeplot(initial_state[0].numpy().flatten()) # weight_mean
        #sns.kdeplot(initial_state[1].numpy().flatten()) # weight_scale
        # sns.kdeplot(np.log(initial_state[1].numpy().flatten()))
        #sns.kdeplot(initial_state[2].numpy().flatten())
        #sns.kdeplot(initial_state[3].numpy().flatten())
        ...
        states = sample()
        filename = "mcmc_condprob_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        ...
        print("before 100 steps")
        def analyze_losses(iter):
            l = analyse_target_log_prob_cat(weight_mean[iter], weight_scale[iter], weights[iter], smweights[iter])
            return l
        print(analyze_losses(-100))
        ...
        if True:
            #lc = target_log_prob_cat(weight_mean, weight_scale, weights, smweights)
        ...
        if False:
            #r = make_mixture_probs_train(weights[::50], smweights[::50])

            r = make_mixture_probs_train(weights[100], smweights[100])
            r[0].distribution.mixture_distribution.logits_parameter()[-2]
            r[0].mean()[-1]
            r[0].mean()[-2]
            plot_softprob_simple(r[0].mean()[-1])
            plot_softprob_simple(r[0].mean()[-2])
            Y_train[-2]
            tf.reduce_sum(tf.cast(r[1].distribution.mixture_distribution.sample() != r[1].distribution.mixture_distribution.sample(), tf.int32))
    ...
    if False:
        ...
        for i in range(mixcom):
            plot_softprob_simple(make_h1_probs_train(weights[w], smweights[w]).distribution.components_distribution.mean()[
                                     plotcompontent, 0, i], title="component " + str(i))
