...
if __name__ == "__main__":
    ...
    def make_joint_mixture_model(x):
        def make_h1dist(weights, smweights):
            ...
            compdist_logits_h1 = 100.0*smweights[..., tf.newaxis,:mixcom,:]*tf.ones([x.shape[0], 1, 1], dtype=tf.float64)
            # print(h1logits.shape)
            # print(compdist_logits_h1.shape)
            h1dist = tfd.Independent(
               tfd.MixtureSameFamily(
                   mixture_distribution=tfd.Categorical(logits = h1logits*10),
        ...
        def make_h2dist(weights, smweights):
            ...
            compdist_logits_h2 = 100.0 * smweights[..., tf.newaxis, mixcom:, :] * \
                                 tf.ones([x.shape[0], 1, 1], dtype=tf.float64)
            h2logits_all = tf.matmul(x, weights[..., 1:,:-1, :]) + weights[..., 1:,-1:, :]
            # print(compdist_logits_h2.shape)
            # print(h1dist)
            # print(h2logits_all.shape)
            #h2logits = tf.reduce_sum(tf.cast(tf.linalg.matrix_transpose(h1dist.sample()), tf.float64)[...,tf.newaxis] * h2logits_all, axis=-3)
            h2logits = tf.boolean_mask(h2logits_all, tf.cast(tf.linalg.matrix_transpose(h1dist.sample()), tf.boolean)[...,tf.newaxis], axis=-3)
            # print(h2logits.shape)
            h2dist = tfd.Independent(
                   tfd.MixtureSameFamily(
                       mixture_distribution=tfd.Categorical(logits = h2logits*10),
        ...
        return \
            ...
            tfd.JointDistributionNamed(dict(
                weight_mean=tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[50, x.shape[1] + 1, l1dim], dtype=tf.float64),
                                                       scale=tf.ones(shape=[50, x.shape[1] + 1, l1dim], dtype=tf.float64)),
                                            reinterpreted_batch_ndims=3),
                weight_scale=tfd.Independent(
                    tfd.LogNormal(loc=tf.zeros(shape=[50, x.shape[1] + 1, l1dim], dtype=tf.float64),
                                  scale=tf.ones(shape=[50, x.shape[1] + 1, l1dim], dtype=tf.float64)),
                    reinterpreted_batch_ndims=3),
                weights=lambda weight_scale, weight_mean: tfd.Independent(
                    tfd.Normal(loc=weight_mean, scale=1.0 * weight_scale), reinterpreted_batch_ndims=3),
                ...
                smweights = tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[2*mixcom, 49], dtype=tf.float64),
                                                       scale=1.0*tf.ones(shape=[2*mixcom, 49], dtype=tf.float64)),
                                            reinterpreted_batch_ndims=2),
                #h1dist=make_h1dist,
                outputs=make_h2dist
         )
    ...
    _ = joint_model_batch.sample(2)["outputs"]
    ...
    def analyze_losses(iter):
        ...
        l["total"] = target_log_prob_cat(weight_mean[iter], weight_scale[iter], weights[iter], smweights[iter])
        return l
    print(analyze_losses(-1))
