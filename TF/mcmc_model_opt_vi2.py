    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x):
            def make_outputs(l1weights, l2weights, weights, smweights): #tfd.Sample(
               x1 = tf.math.tanh(tf.matmul(x, l1weights[..., :-1, :]) + l1weights[..., -1:, :])
               x2 = tf.matmul(x1, l2weights[..., :-1, :]) + l2weights[..., -1:, :]
               l = tf.matmul(x1, weights[..., :-1, :]) + weights[..., -1:, :]
               compdist_logits = 100.0 * smweights[..., tf.newaxis, :mixcom, :] * tf.ones([x.shape[0], 1, 1],
                                                                                            dtype=tf.float64)
               return tfd.JointDistributionNamed(dict(
                   mainresult = tfd.MixtureSameFamily(
                       mixture_distribution=tfd.Categorical(logits = l),
                       components_distribution=tfd.OneHotCategorical(logits=compdist_logits)
                   ),
                   sidebets = tfd.Independent(tfd.Poisson(log_rate=x2), reinterpreted_batch_ndims=1)
               ))
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                l1weights=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[x.shape[1] + 1, L1size], dtype=tf.float64),
                               scale=tf.ones(shape=[x.shape[1] + 1, L1size], dtype=tf.float64)), reinterpreted_batch_ndims=2),
                l2weights=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L1size + 1, L2size], dtype=tf.float64),
                               scale=tf.ones(shape=[L1size + 1, L2size], dtype=tf.float64)), reinterpreted_batch_ndims=2),
                weights=tfd.Independent(
                    tfd.Normal(loc=tf.zeros(shape=[L1size + 1, mixcom], dtype=tf.float64),
                               scale=tf.ones(shape=[L1size + 1, mixcom], dtype=tf.float64)), reinterpreted_batch_ndims=2),
        ...
        def loss_function(l1weights, l2weights, weights, smweights, X, Y, Y2): # weight_mean, weight_scale,
            ...
            lpp = joint_model.log_prob_parts({
                ...
                'weights': weights,
                'smweights': smweights,
                'outputs': {"mainresult":Y, "sidebets":Y2}
            })
            #print(lpp)
            ...
            lp = tf.reduce_sum(lpp['l1weights']+lpp['l2weights']+lpp['weights']+lpp['smweights'])*(X.shape[-2]) #+ tf.reduce_sum(lpp['outputs'], axis=-1)*0.00001 # lpp['weight_mean']+lpp['weight_scale']+
            reg_loss = create_laplacian_loss(smweights, alpha=0.1)

            achievable_points = tf.matmul(Y, tf.cast(point_matrix, tf.float32))
            pred = mk(l1weights, l2weights, weights, smweights)

            actual_points = tf.nn.softmax(pred.parameters["model"]["mainresult"].components_distribution.logits, axis=-1) * tf.cast(achievable_points, tf.float64)[...,tf.newaxis, :] # [1,mixcom,1]
            dist_probs = tf.nn.softmax(pred.parameters["model"]["mainresult"].mixture_distribution.logits_parameter(), axis=-1)
            ...
            real_points = tf.reduce_sum(tf.one_hot(tf.argmax(pred.parameters["model"]["mainresult"].mean(), axis=-1), 49) * achievable_points, axis=-1)
            softmax_lp = pred.parameters["model"]["mainresult"].log_prob(Y)
            ...
            lpp.update({"laplacian_reg_loss":reg_loss, "joint_model_log_prob":lp, "mean_points":tf.reduce_mean(actual_points), "real_points":tf.reduce_mean(real_points),
                        "softmax_lp":softmax_lp, "poisson_lp":poisson_lp})
            lpp["total"] = tf.reduce_sum(lp) + 15.0*tf.reduce_sum(actual_points) - tf.reduce_sum(reg_loss) + 0.001*tf.reduce_sum(poisson_lp) + 0.1*tf.reduce_sum(softmax_lp)#- lpp["outputs"] + lpp["outputs"]*0.3 #

            return lpp

        def analyse_losses(l1weights, l2weights, weights, smweights, X, Y, Y2): # weight_mean, weight_scale,
            return loss_function(l1weights, l2weights, weights, smweights, X, Y, Y2)
        ...
        def target_log_prob_cat(l1weights, l2weights, weights, smweights):
            return loss_function(l1weights, l2weights, weights, smweights, x_train_scaled, outputs_, Y_train)["total"]


        def target_log_prob_cat_test(l1weights, l2weights, weights, smweights):
            return loss_function(l1weights, l2weights, weights, smweights, x_test_scaled, test_outputs_, Y_test)["total"]


        def analyse_target_log_prob_cat(l1weights, l2weights, weights, smweights):
            return analyse_losses(l1weights, l2weights, weights, smweights, x_train_scaled, outputs_, Y_train)

        print(joint_model)
        jm_components = joint_model.event_shape_tensor().keys()
        surrogate_posterior = tfp.experimental.vi.build_factored_surrogate_posterior(
            event_shape=[joint_model.event_shape_tensor()[c] for c in ['l1weights', 'l2weights', 'weights', 'smweights']],
        ...
        (l1weights, l2weights, weights, smweights), _ = surrogate_posterior.sample_distributions()
        print(analyse_target_log_prob_cat(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean()))

        (l1weights0, l2weights0, weights0, smweights0), _ = surrogate_posterior.sample_distributions()
        l1weights0_mean = l1weights0.mean().numpy()
        l2weights0_mean = l2weights0.mean().numpy()
        weights0_mean = weights0.mean().numpy()
        smweights0_mean = smweights0.mean().numpy()
        l1weights0_stddev = l1weights0.stddev().numpy()
        ...
        filename = "mcmc_deterministic_"+FLAGS.prefix+"_"+FLAGS.target_system+"_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        filehandler = open(filename, 'wb')
        #pickle.dump((l1weights.mean(), l1weights.stddev(), l2weights.mean(), l2weights.stddev(), weights.mean(), weights.stddev(), smweights.mean(), smweights.stddev()), filehandler)
        pickle.dump(surrogate_posterior.trainable_variables, filehandler)

        fig, ax = plt.subplots(5, 2)
        plt.subplots_adjust(left=0.04, bottom=0.03, right=0.96, top=0.97, wspace=0.08, hspace=0.18)

        sns.kdeplot(l1weights.mean().numpy().flatten(), ax=ax[0][0])
        sns.kdeplot(l1weights.stddev().numpy().flatten(), ax=ax[0][1])
        sns.kdeplot(l2weights.mean().numpy().flatten(), ax=ax[1][0])
        sns.kdeplot(l2weights.stddev().numpy().flatten(), ax=ax[1][1])
        sns.kdeplot(weights.mean().numpy().flatten(), ax=ax[2][0])
        sns.kdeplot(weights.stddev().numpy().flatten(), ax=ax[2][1])
        sns.kdeplot(smweights.mean().numpy().flatten(), ax=ax[3][0])
        sns.kdeplot(smweights.stddev().numpy().flatten(), ax=ax[3][1])
        #
        ...
        target_log_prob_cat_test(l1weights.sample(), l2weights.sample(), weights.sample(), smweights.sample())
        train_points = [analyse_losses(l1weights.sample(), l2weights.sample(), weights.sample(), smweights.sample(), x_train_scaled, outputs_, Y_train)["real_points"].numpy() for i in range(100)]
        train_mean_points = analyse_losses(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean(), x_train_scaled, outputs_, Y_train)["real_points"].numpy()

        test_points = [analyse_losses(l1weights.sample(), l2weights.sample(), weights.sample(), smweights.sample(), x_test_scaled, test_outputs_, Y_test)["real_points"] .numpy() for i in range(100)]
        test_mean_points = analyse_losses(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean(), x_test_scaled, test_outputs_, Y_test)["real_points"].numpy()

        train_softpoints = [analyse_losses(l1weights.sample(), l2weights.sample(), weights.sample(), smweights.sample(), x_train_scaled, outputs_, Y_train)["mean_points"].numpy() for i in range(100)]
        train_mean_softpoints = analyse_losses(l1weights.sample(), l2weights.sample(), weights.mean(), smweights.mean(), x_train_scaled, outputs_, Y_train)["mean_points"].numpy()

        test_softpoints = [analyse_losses(l1weights.sample(), l2weights.sample(), weights.sample(), smweights.sample(), x_test_scaled, test_outputs_, Y_test)["mean_points"] .numpy() for i in range(100)]
        test_mean_softpoints = analyse_losses(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean(), x_test_scaled, test_outputs_, Y_test)["mean_points"].numpy()
        ...
        t2 = make_mixture_probs_train(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean()).parameters["model"]["mainresult"].mean().numpy()
        ...
        t2 = make_mixture_probs_test(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean()).parameters["model"]["mainresult"].mean().numpy()
        ...
        pred_probs = make_mixture_probs_pred(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean()).parameters["model"]["mainresult"].mean().numpy()
        ...
        if True:
            ...
            for i in range(mixcom):
                pr = make_mixture_probs_train(l1weights.mean(), l2weights.mean(), weights.mean(), smweights.mean()).parameters["model"]["mainresult"].components_distribution[0].mean()[i]
