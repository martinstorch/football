...
if __name__ == "__main__":
    ...
    N = 1000
    #feature_idx = list(range(0,300))
    feature_idx = bwin_index

    Y_train = labels_array[train_idx, 0:16]
    Y_test = labels_array[test_idx, 0:16]

    small_x_train0 = X_train[200:(200 + N), :].astype(np.float64)
    small_x_train = X_train[200:(200+N), feature_idx].astype(np.float64)
    small_y_train = Y_train[200:(200+N), ...].astype(np.float64)
    small_x_train = np.concatenate([small_x_train, small_y_train], axis=1) # use outputs as features for similarity building

    scaler = preprocessing.StandardScaler().fit(small_x_train)
    small_x_train = scaler.transform(small_x_train)
    ...
    observations_ = small_x_train.transpose()
    ...
    init_ = np.random.normal(size=(N, latent_dim))
    latent_index_points = tf.Variable(init_, name='latent_index_points')
    print(latent_index_points.shape)
    ...
    # OptimizerOptions.global_jit_level = OptimizerOptions.ON_2
    ...
    num_iters = 300
    log_interval = 20
    lips = np.zeros((num_iters, N, latent_dim), np.float64)
    ...
    fig, ax = plt.subplots(latent_dim, latent_dim, figsize=(7, 7))
    #ax.grid(False)
    plt.title("Before training")
    for i in range(latent_dim):
        for j in range(latent_dim):
            scatter = ax[i][j].scatter(x=init_[:, i], y=init_[:, j],
                         c=np.sign(small_y_train[:N,0]-small_y_train[:N,1]), cmap=plt.get_cmap('Paired'), s=50, label=np.sign(small_y_train[:N,0]-small_y_train[:N,1]))
    ...
    #           c=y_train[:N], cmap=plt.get_cmap('Paired'), s=50, label=y_train[:N])
    # plt.legend()
    plt.show()

    fig, ax = plt.subplots(latent_dim-1, latent_dim-1, figsize=(7, 7))
    col = ['r', 'g', 'b'] #ax.grid(False)
    plt.title("After training")
    for i in range(latent_dim-1):
        for j in range(i+1, latent_dim):
            ax0 = ax[i][j-1] if latent_dim>2 else ax
            scatter = ax0.scatter(x=lips[-1, :, i], y=lips[-1, :, j], alpha=0.3,
                         c=np.take(col, (1+np.sign(small_y_train[:N,0]-small_y_train[:N,1])).astype(int)), s=5, label=np.sign(small_y_train[:N,0]-small_y_train[:N,1])) # cmap=plt.get_cmap('Paired'),
    ...
    model = tf.keras.Sequential([
        tfp.layers.DenseVariational(latent_dim*2, posterior_mean_field, prior_trainable, kl_weight=1 / N, input_dim=small_x_train0.shape[1]),
        tfp.layers.DistributionLambda(
            lambda t: tfd.Normal(loc=t[..., :latent_dim],
                                 scale=1e-3 + tf.math.softplus(0.01 * t[..., latent_dim:]))),
    ])
    ...
    scaler2 = preprocessing.StandardScaler().fit(small_x_train0)
    model.fit(x=scaler2.transform(small_x_train0), y=latent_index_points.numpy(), epochs=300, verbose=True, batch_size=64);

    plt.figure()
    m1 = model(scaler2.transform(small_x_train0)).mean()
    plt.scatter(m1[:,0], m1[:,1])
    plt.show()
    plt.figure()
    s1 = model(scaler2.transform(small_x_train0)).stddev()
    plt.scatter(s1[:,0], s1[:,1])
    plt.show()

    {w.name:w.shape for w in model.weights}
    ...
    #
    #
    # sample_grid_points = 20
    # grid_ = np.linspace(-15, 15, sample_grid_points).astype(np.float64)
    ...
    gprm = tfd.GaussianProcessRegressionModel(
        ...
        observations=small_y_train.transpose()) #observations_)
    latent_index_points.shape
    observations_.shape
    gprm.sample()[:8]
    gprm.mean()[:,0]
    gprm.stddev()
    ...
    observations_[:,0]
    m1[0]
    latent_index_points[0]
    pd.DataFrame(gprm.mean()[:,10],small_y_train[10])
