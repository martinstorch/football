    ...
    if __name__ == "__main__":
        ...
        def make_joint_mixture_model(x):
            def make_outputs(weights, smweights): #tfd.Sample(
               ...
               l2 = tf.matmul(l, smweights[..., :-1, :]) + smweights[..., -1:, :]
               return tfd.Independent(tfd.OneHotCategorical(logits=l2), reinterpreted_batch_ndims=1)
            ...
            return make_outputs, tfd.JointDistributionNamed(dict(
                ...
                weight_scale=tfd.Independent(
                    tfd.LogNormal(loc=tf.zeros(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64),
                                  scale=tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)),
                    reinterpreted_batch_ndims=2),
                weights=lambda weight_scale, weight_mean: tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64),
                                                       scale=0.1*tf.ones(shape=[x.shape[1] + 1, mixcom], dtype=tf.float64)),
                                            reinterpreted_batch_ndims=2),
                ...
                smweight_mean=tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[mixcom + 1, 49], dtype=tf.float64),
                                                       scale=tf.ones(shape=[mixcom + 1, 49], dtype=tf.float64)),
                                            reinterpreted_batch_ndims=2),
                smweight_scale=tfd.Independent(
                    tfd.LogNormal(loc=tf.zeros(shape=[mixcom + 1, 49], dtype=tf.float64),
                                  scale=0.1*tf.ones(shape=[mixcom + 1, 49], dtype=tf.float64)),
                    reinterpreted_batch_ndims=2),
                # smweights=lambda smweight_scale, smweight_mean: tfd.Independent(tfd.Normal(loc=tf.zeros(shape=[mixcom + 1, 49], dtype=tf.float64),
                #                                        scale=0.2*tf.ones(shape=[mixcom + 1, 49], dtype=tf.float64)),
                #                             reinterpreted_batch_ndims=2),
                smweights=lambda smweight_scale, smweight_mean: tfd.Independent(
                    tfd.Normal(loc=smweight_mean, scale=0.01 * smweight_scale), reinterpreted_batch_ndims=2),
        ...
        cd = joint_model.parameters["model"]["smweight_mean"]
        cd.sample()
        ...
        def loss_function(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, X, Y):
            mk, joint_model = make_joint_mixture_model(X)
            lpp = joint_model.log_prob_parts({
                'weight_mean': weight_mean,
                'weight_scale': weight_scale,
                'weights': weights,
                'smweight_mean': smweight_mean,
                'smweight_scale': smweight_scale,
                'smweights': smweights,
                'outputs': Y
            })
            ...
            pred = mk(weights, smweights).distribution
            match_probs = tf.nn.softmax(pred.logits, axis=-1) #(5508, 10, 49)
            #match_probs = tf.reduce_sum(comp_probs , axis=-1) #(5508, 49)
            actual_points = tf.minimum(match_probs, 1.0) * tf.cast(achievable_points, tf.float64) # (5508, 49) # tf.minimum(match_probs, 1.0)
            ...
            lpp["total"] = lpp["weight_mean"]+lpp["weight_scale"]+lpp["weights"]+lpp["smweight_mean"]+lpp["smweight_scale"]+lpp["smweights"] + 0.01*lpp["outputs"]+ 300*tf.reduce_mean(actual_points) # - 1*tf.reduce_sum(kld) # - tf.reduce_sum(reg_loss2) - tf.reduce_sum(reg_loss)+ 0.0001 * lpp["outputs"]
            return lpp

        def analyse_losses(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, X, Y):
            return loss_function(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, X, Y)
        ...
        def target_log_prob_cat(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights):
            return loss_function(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, x_train_scaled, outputs_)["total"]

        def target_log_prob_cat_batched( weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights):
            index = tf.random.stateless_uniform(shape=[batch_size], maxval=x.shape[0], dtype=tf.int32, seed=(1,2))
            ...
            x0 = xy0[...,:s]
            y0 = xy0[...,s:]
            return loss_function(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, tf.cast(x0, tf.float64), y0)["total"]

        def target_log_prob_cat_test(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights):
            return loss_function(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, x_test_scaled, test_outputs_)["total"]


        def analyse_target_log_prob_cat(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights):
            return analyse_losses(weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights, x_train_scaled, outputs_)
        ...
        sampler_bij = tfp.mcmc.TransformedTransitionKernel(
            ...
            bijector=[tfb.Identity(), tfb.Softplus(), tfb.Identity(),
                      tfb.Identity(), tfb.Softplus(), tfb.Identity()
                      ]
        )

        #tfb.Affine(shift=tf.math.log(0.0001+tf.reduce_mean(tf.cast(outputs_, tf.float64), axis=0)))]) # tfb.Affine(shift=0.135, scale_identity_multiplier=0.01, dtype=tf.float64)
        ...
        initial_state = [#np.random.randn(x_train.shape[1]+1, mixcom), # np.zeros # weight_mean
                         ...
                         tf.zeros(shape=[x_train.shape[1]+1, mixcom], dtype=tf.float64), # weights
                        tf.zeros(shape=[mixcom+1, 49], dtype=tf.float64),  # smweight_mean
                        tf.ones(shape=[mixcom+1, 49], dtype=tf.float64),  # smweight_scale
                        tf.zeros(shape=[mixcom+1, 49], dtype=tf.float64),  # smweights
        ...
        inputfilename = sorted([f for f in os.listdir() if re.search("mcmc_straight2_"+FLAGS.target_system+".*pickle", f)])
        if len(inputfilename) > 0:
            inputfilename = inputfilename[-1]
            print(inputfilename)
            filehandler = open(inputfilename, 'rb')
            all_states = pickle.load(filehandler)
            weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights = all_states
            initial_state = [weight_mean[-1], weight_scale[-1], weights[-1], smweight_mean[-1], smweight_scale[-1], smweights[-1]]
        ...
        states = sample()
        filename = "mcmc_straight2_"+FLAGS.target_system+"_"+datetime.now().strftime("%Y%m%d_%H%M%S")+".pickle"
        filehandler = open(filename, 'wb')
        pickle.dump(states.all_states, filehandler)
        weight_mean, weight_scale, weights, smweight_mean, smweight_scale, smweights = states.all_states # , cdist

        current_state = [weight_mean[-1], weight_scale[-1], weights[-1], smweight_mean[-1], smweight_scale[-1], smweights[-1]]
        current_state2 = [weight_mean[-10], weight_scale[-10], weights[-10], smweight_mean[-10], smweight_scale[-10], smweights[-10]]
        ...
        if True:
            stepsize = 20
            loss_curve = np.concatenate([target_log_prob_cat(weight_mean[i:(i+stepsize)], weight_scale[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                             smweight_mean[i:(i+stepsize)], smweight_scale[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i in range(0, weights.shape[0], stepsize)], axis=0)
            loss_curve_test = np.concatenate([target_log_prob_cat_test(weight_mean[i:(i+stepsize)], weight_scale[i:(i+stepsize)], weights[i:(i+stepsize)],
                                                                       smweight_mean[i:(i+stepsize)], smweight_scale[i:(i+stepsize)], smweights[i:(i+stepsize)]).numpy() for i in range(0, weights.shape[0], stepsize)], axis=0)
            ...
            sns.kdeplot(weight_scale[-1].numpy().flatten(), ax=ax[1][0])
            sns.kdeplot(weights[-1].numpy().flatten(), ax=ax[2][0])
            sns.kdeplot(smweight_mean[-1].numpy().flatten(), ax=ax[0][1])
            sns.kdeplot(smweight_scale[-1].numpy().flatten(), ax=ax[1][1])
            sns.kdeplot(smweights[-1].numpy().flatten(), ax=ax[2][1])
            ax[3][0].plot(loss_curve)
            ax[3][1].plot(loss_curve_test)
            sns.kdeplot(weight_mean[0].numpy().flatten(), ax=ax[0][0])
            sns.kdeplot(weight_scale[0].numpy().flatten(), ax=ax[1][0])
            sns.kdeplot(weights[0].numpy().flatten(), ax=ax[2][0])
            sns.kdeplot(smweight_mean[0].numpy().flatten(), ax=ax[0][1])
            sns.kdeplot(smweight_scale[0].numpy().flatten(), ax=ax[1][1])
            sns.kdeplot(smweights[0].numpy().flatten(), ax=ax[2][1])
            ax[4][0].plot(avg_points_train)
            ax[4][1].plot(avg_points_test)
            ax[5][0].plot(real_points_train)
            ax[5][1].plot(real_points_test)
            plt.show()
            #plt.close()
            ...
            ax[1,0].plot(np.stack([joint_model.parameters["model"]["weight_scale"].log_prob(weight_scale[q]).numpy() for q in range(weight_scale.shape[0])]))
            ax[2,0].plot(np.stack([joint_model.parameters["model"]["weights"](weight_scale[q], weight_mean[q]).log_prob(weights[q]).numpy() for q in range(weights.shape[0])]))
            ax[0,1].plot(np.stack([joint_model.parameters["model"]["smweight_mean"].log_prob(smweight_mean[q]).numpy() for q in range(smweight_scale.shape[0])]))
            ax[1,1].plot(np.stack([joint_model.parameters["model"]["smweight_scale"].log_prob(smweight_scale[q]).numpy() for q in range(smweight_scale.shape[0])]))
            ax[2,1].plot(np.stack([joint_model.parameters["model"]["smweights"](smweight_scale[q], smweight_mean[q]).log_prob(smweights[q]).numpy() for q in range(weight_scale.shape[0])]))
            plt.show()
        ...
        if True:
            ...
            df2 = create_df(create_argmax_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[i: (i + stepsize)], smweights[i: (i + stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)], axis=0), Y_train.shape[0]), Y_train, "Train")
            ...
            if False:
                df3 = create_df(sample_argmax_prediction_from_mean(np.concatenate([make_mixture_probs_train(weights[i: (i + stepsize)], smweights[i: (i + stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)], axis=0), Y_train.shape[0], s=10), Y_train, "Train")
                print(df3)
            ...
            # df2 = create_maxpoint_prediction(df)
            df2 = create_df(create_argmax_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[i: (i + stepsize)], smweights[i: (i + stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)], axis=0), Y_test.shape[0]), Y_test, "Test")
            ...
            if False:
                df3 = create_df(sample_argmax_prediction_from_mean(np.concatenate([make_mixture_probs_test(weights[i: (i + stepsize)], smweights[i: (i + stepsize)]).mean().numpy() for i in range(0, weights.shape[0], stepsize)], axis=0), Y_test.shape[0], s=10), Y_test, "Test")
                print(df3)
    ...
    if False:
        ...
        for i in range(4):
            plot_softprob_simple(make_mixture_probs_train(weights[w], smweights[w]).distribution.components_distribution[0].mean()[i], title="component "+str(i))

    for i in range(4):
        s = random.sample(range(outputs_.shape[0]), 1)
        w = -1
        plot_softprob_simple(make_mixture_probs_train(weights[w], smweights[w]).distribution[s].mean(), title="match " + str(s))