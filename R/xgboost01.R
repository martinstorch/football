...
evaluateXgbWinLoss<-function(model, newdata, newlabels, label="", threshold=0.5, levels1=levels(buildFactorScore(train)$score)) {
  print(threshold)
  comparedata<-as.data.frame(as.matrix(newlabels))
  comparedata$where=as.factor(ifelse(newdata[,"whereHome"]==1, "Home", "Away"))
  winlosspred<-predict(model, newdata=newdata)
  gspred<-ifelse(winlosspred>ifelse(comparedata$where=="Home", threshold, 1-threshold), 2, 1) 
  gcpred<-ifelse(winlosspred>ifelse(comparedata$where=="Home", threshold, 1-threshold), 1, 2) 
  summarizePrediction(data = comparedata, gspred, gcpred, label)
  return(data.frame(gspred, gcpred))
}

evaluateXgbWinLoss2<-function(model, model2, newdata, newlabels, label="", threshold=0.6, threshold2=0.5, levels1=levels(buildFactorScore(train)$score)) {
  print(threshold)
  comparedata<-as.data.frame(as.matrix(newlabels))
  comparedata$where=as.factor(ifelse(newdata[,"whereHome"]==1, "Home", "Away"))
  winlosspred<-predict(model, newdata=newdata)
  manygoalspred<-predict(model2, newdata=newdata)
  extraGoal<-ifelse(manygoalspred>threshold2, 1, 0)
  gspred<-extraGoal+ifelse(winlosspred>ifelse(comparedata$where=="Home", threshold, 1-threshold), 1, 0) 
  gcpred<-extraGoal+ifelse(winlosspred>ifelse(comparedata$where=="Home", threshold, 1-threshold), 0, 1) 
  summarizePrediction(data = comparedata, gspred, gcpred, label)
  return(data.frame(gspred, gcpred))
}
...
teamresults<-read.csv(file="teamresults.csv", stringsAsFactors = TRUE, as.is = FALSE)
...
trainY2<-ifelse(train$GS+train$GC>1, 1, 0)
testY2<-ifelse(test$GS+test$GC>1, 1, 0)

trainY<-(((teamresults[!oobidx,] %>% 
  mutate(win = sign(GS-GC)) %>% 
  mutate(win = ifelse(win==0, sign(Shotstarget-oppShotstarget), win)) %>%
  mutate(win = ifelse(win==0, sign(Shots-oppShots), win)) %>%
  mutate(win = ifelse(win==0, sign(Corners-oppCorners), win)) %>%
  mutate(win = ifelse(win==0, sign(oppFouls-Fouls), win)) %>%
  select(win))$win+1)/2)


testY<-(((teamresults[oobidx,] %>% 
                 mutate(win = sign(GS-GC)) %>% 
                 mutate(win = ifelse(win==0, sign(Shotstarget-oppShotstarget), win)) %>%
                 mutate(win = ifelse(win==0, sign(Shots-oppShots), win)) %>%
                 mutate(win = ifelse(win==0, sign(Corners-oppCorners), win)) %>%
...
trainMatrix<-xgb.DMatrix(trainXM, label=trainY, missing = -999)
attr(trainMatrix, "where")<-(as.data.frame(as.matrix(trainXM)) %>% select(whereHome))$where

testMatrix<-xgb.DMatrix(testXM, label=testY, missing = -999)
...
trainMatrix2<-xgb.DMatrix(trainXM, label=trainY2, missing = -999)
testMatrix2<-xgb.DMatrix(testXM, label=testY2, missing = -999)
watchlist2<-list(train=trainMatrix2, test=testMatrix2)

xgbparamsFactor<-list(gamma=1, max_depth = 10, eta = 0.002, subsample=0.4, colsample_bytree=0.1, nthread = 10)
modeleval<-xgb.train(objective = "reg:logistic", nrounds =3000, watchlist=watchlist, verbose=1, params = xgbparamsFactor, data = trainMatrix , maximize=FALSE, print_every_n=20)
printLearningCurve(modeleval, maximise = FALSE)

optThreshold<-optimize(lower = 0.1, upper=0.9, maximum = TRUE, f = function(threshold){
  print(threshold)
  result<-evaluateXgbWinLoss(modeleval, trainXM, trainYM, threshold=threshold, label="train XGB optimize")
  return(sum(calcPoints(GS1 = result$gspred, GC1 = result$gcpred, 
                        GS2 = trainYM[,1], GC2 = trainYM[,2], 
                        home = trainXM[,"whereHome"]==1)))
})
print(optThreshold)
result<-evaluateXgbWinLoss(modeleval, testXM, testYM, threshold = optThreshold$maximum, "test XGB objective function")
result<-evaluateXgbWinLoss(modeleval, testXM, testYM, threshold = 0.6, "test XGB objective function")
result<-evaluateXgbWinLoss(modeleval, trainXM, trainYM, threshold = optThreshold$maximum, "train XGB objective function")
plotImportance(modeleval, 40)
printImportance(modeleval)


xgbparamsFactor2<-list(gamma=5, max_depth = 3, eta = 0.0025, subsample=0.3, colsample_bytree=0.03, nthread = 10)
modeleval2<-xgb.train(nrounds =1000, watchlist=watchlist2, verbose=1, params = xgbparamsFactor2, data = trainMatrix2 , maximize=FALSE, print_every_n=10, objective = "reg:logistic")
printLearningCurve(modeleval2, maximise = FALSE)

optThreshold2<-optimize(lower = 0.1, upper=0.9, maximum = TRUE, f = function(threshold2){
  print(threshold2)
  result<-evaluateXgbWinLoss2(modeleval, modeleval2, trainXM, trainYM, threshold=optThreshold$maximum, threshold2=threshold2, label="train XGB optimize")
  return(sum(calcPoints(GS1 = result$gspred, GC1 = result$gcpred, 
                        GS2 = trainYM[,1], GC2 = trainYM[,2], 
                        home = trainXM[,"whereHome"]==1)))
})
print(optThreshold2)
result<-evaluateXgbWinLoss2(modeleval, modeleval2, testXM, testYM, threshold = optThreshold$maximum, threshold2=optThreshold2$maximum, "test XGB 2:1 / 1:0")
result<-evaluateXgbWinLoss2(modeleval, modeleval2, testXM, testYM, threshold = 0.6, threshold2=0.74, "test XGB 2:1 / 1:0")
result<-evaluateXgbWinLoss2(modeleval, modeleval2, trainXM, trainYM, threshold = optThreshold$maximum, threshold2=optThreshold2$maximum, "test XGB 2:1 / 1:0")
plotImportance(modeleval2, 40)
printImportance(modeleval2)


manygoalspred<-predict(modeleval2, newdata=testXM)
summary(manygoalspred)
hist(manygoalspred)
