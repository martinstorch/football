...
expectedValues<-sapply(1:25, function(i) prediction$predoutcomes %*% unlist(mask[i]))
colnames(expectedValues)<-names(mask)
cbind(recommend(prediction), expectedValues)[4,]
x<-8; matrix(expectedValues[x,], ncol=5, nrow=5, dimnames = list(paste(prediction$newmatches$team[x], 0:4),paste(prediction$newmatches$otherTeam[x], 0:4)))
x<-8; matrix(prediction$predoutcomes[x,]*100, ncol=5, nrow=5, dimnames = list(paste(prediction$newmatches$team[x], 0:4),paste(prediction$newmatches$otherTeam[x], 0:4)))
