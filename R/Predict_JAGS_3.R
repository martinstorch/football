...
model {
  for(i in 1:length(team)) {
    goals[i] ~ dpois(mu[i])
    mu[i]<-exp(MM[team[i],otherTeam[i]])
  }
...
samplesize<-200
sampleindex <- sample(nrow(inputData), samplesize, replace = FALSE)
outofsampleindex <- setdiff(1:nrow(inputData) , sampleindex)
model <- jags.model(textConnection(model_string), 
                    data = c(as.list(inputData[sampleindex,]), outputGames),
                    n.chains = 3, n.adapt= 100)


adapt(model, 2000)
update(model, 1000); # Burnin for 10000 samples
mcmc_mm <- coda.samples(model, variable.names=c("MM", "mu", "L", "R"), n.iter=2000)
#mcmc_mm <- coda.samples(model, variable.names=c("mu", "lt"), n.iter=1000)
# summary(mcmc_mm)

varidx<-grep(pattern = "mu\\[", x = varnames(mcmc_mm))
compare<-data.frame(act=inputData$goals, pred=summary(mcmc_mm[,varidx])$statistics[,1])
plot(compare$act, compare$pred)
cor(compare$act, compare$pred)


varidx<-grep(pattern = "MM\\[", x = varnames(mcmc_mm))
predmatrix<-matrix(data = summary(mcmc_mm[,varidx])$statistics[,1], ncol=18, nrow=18, dimnames = list(paste(teams, "Home"), paste(teams, "Away")))
exp(predmatrix)

compare<-data.frame(act=inputData$goals - inputData$otherGoals, pred=sapply(1:nrow(inputData), function(i) predmatrix[inputData$team[i],inputData$otherTeam[i]]))
compare_new<-compare[outofsampleindex,]
plot(compare$act, compare$pred)
plot(compare_new$act, compare_new$pred)
cor(compare_new$act, compare_new$pred)
