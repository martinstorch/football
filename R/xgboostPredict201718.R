...
for (j in 1:10){
  ...
  model3<-xgb.train(nrounds =100,  #early_stopping_rounds = 50, 
                    params = xgbparamsFactor, 
                    data = trainMatrix , 
                    obj=objectiveStaticPrior, feval=evaltarget, maximize=TRUE, 
                    print_every_n=1, verbose=1, 
                    nthread = 10,  num_class=nlevels(gLev) 
                    , watchlist = watchlist
  )
  ...
  plot(model3$evaluation_log$train_avgpoints[-1:-5], type="l")
  ...
  models[[j]]<-model3
  predictions<-rbind(predictions, cbind(new_data %>% select(team, oppTeam), testResult))
  
}
write.csv(predictions, "newpredictions.csv", row.names = FALSE)

print(predictions %>% arrange(team, gspred, gcpred))


i<-1; printSoftMaxGame(model3, testMatrix, trainMatrix, i)
