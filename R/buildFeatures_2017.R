...
download.file("http://www.football-data.co.uk/mmz4281/1718/D1.csv", "BL2017.csv", method = "libcurl")
#download.file("http://217.160.223.109/mmz4281/1718/D1.csv", "BL2017.csv", method = "libcurl")
...
#download.file("http://www.football-data.co.uk/mmz4281/1314/D1.csv", "BL2013.csv")
...
inputFile<-"BL2017.csv"
new_games<-"NewGames.csv"
season<-"2017_18"
output_teamresults<-"teamresults_2017.csv"
output_features<-"BLfeatures_2017.csv"
...
new_games_data<-utils::read.csv(new_games, strip.white = TRUE)
data<-bind_rows(data, new_games_data)
data[is.na(data<-data)]<-0
data$season<-season
teams <- unique(data$HomeTeam)
print(sort(teams))
print(tail(data[,1:10], 18))
...
teamresults<-teamresults %>% mutate(
  team_promoted = as.integer(
    season=="2013_14" & (team=="Hertha" | team=="Braunschweig") |
  ...
  oppTeam_promoted = as.integer(
    season=="2013_14" & (oppTeam=="Hertha" | oppTeam=="Braunschweig") |
...
for(f in features){
  ...
  for (s in c("t1_own_", "t1_opp_", "t2_own_", "t2_opp_")) {
    ...
    for (iseason in levels(teamresults$season)){
      ...
      newgames <- which(teamresults$cnt_own_Fouls==0 & teamresults$cnt_opp_Fouls==0)                                
      qqplot(data$svdx_output1[-newgames], data$svdx_input[-newgames])
      data[-newgames,]  %>% dplyr::select(svdx_output1, svdx_input) %>% smoothScatter(main=paste(s,f,(data[-newgames,] %>% dplyr::select(svdx_output1, svdx_input) %>% cor(use = "complete.obs"))[1,2]))
      abline(lm(svdx_input ~ svdx_output1, data=data[-newgames,]), col="red")
      print(paste0(s,f))
      print(data[-newgames,] %>% dplyr::select(svdx_output1, svdx_input) %>% cor(use = "complete.obs"))
      print(data[-newgames,] %>% dplyr::select(svdx_output2, svdx_input) %>% cor(use = "complete.obs"))
    }
  }
}
...
enrichPoisson<-function(input_data) {
  poiss_data<-input_data %>% 
    dplyr::select(poisx_input, gameindex, season, where, round, team, oppTeam, starts_with("mt_")
                  ...
                  ) %>% dplyr::select(-contains("cnt_"), -contains("_diff_"), -contains("P2H"), -contains("_where"), -contains("t12"), -contains("_opp_")) %>% 
  ...
  for (iseason in levels(input_data$season)){
      ...
      for (iround in 5:(idata %>% summarise(maxround=max(round)))$maxround) {
        ...
        limit<-max(traindata_poiss$poisx_input)*2
        #print(limit)
        model<-glm(formula = poisx_input ~ .+(team+oppTeam+.)*where,#. + (team+oppTeam+.)*where , 
                   data=traindata_poiss %>% dplyr::select(-season, -poisx_output, -simple_poisx_output, -gameindex, -round), 
                   family = poisson,
  ...
  input_data  %>% filter(round < iround) %>% dplyr::select(poisx_output, poisx_input) %>% smoothScatter(main=(input_data  %>% filter(round < iround)%>% dplyr::select(poisx_output, poisx_input) %>% cor(use = "complete.obs"))[1,2])
  abline(lm(poisx_input ~ poisx_output, data=input_data %>% filter(round < iround)), col="red")
  print(input_data  %>% filter(round < iround) %>% dplyr::select(poisx_output, poisx_input) %>% cor(use = "complete.obs"))
  qqplot(input_data$simple_poisx_output, input_data$poisx_input)
  input_data  %>% filter(round < iround)%>% dplyr::select(simple_poisx_output, poisx_input) %>% smoothScatter(main=(input_data  %>% filter(round < iround)%>% dplyr::select(simple_poisx_output, poisx_input) %>% cor(use = "complete.obs"))[1,2])
  abline(lm(poisx_input ~ simple_poisx_output, data=input_data %>% filter(round < iround)), col="red")
  print(input_data %>% filter(round < iround) %>% dplyr::select(simple_poisx_output, poisx_input) %>% cor(use = "complete.obs"))
  return(input_data)
}
...
buildFactorScore3<-function(dataframe) {
  
  dataframe %>% 
    mutate(score=factor(paste(GS, GC, sep = "."), 
                        levels=c("0.3", "1.3", "0.2", "2.3", "1.2", "0.1",
                                 "0.0", "1.1", "2.2", "3.3", 
                                 "1.0", "2.1", "3.2", "2.0", "3.1", "3.0" ), ordered=TRUE)) %>%
    dplyr::select(-GS, -GC)
}

scorecountsTeam<-sparse.model.matrix(~.-1, data = teamresults %>% dplyr::select(GS=cnt_own_Goals, GC=cnt_opp_Goals) %>% limitMaxScore(3) %>% buildFactorScore3(), drop.unused.levels = FALSE)
scorecountsOppTeam<-sparse.model.matrix(~.-1, data = teamresults %>% dplyr::select(GS=cnt_opp_Goals, GC=cnt_own_Goals) %>% limitMaxScore(3) %>% buildFactorScore3(), drop.unused.levels = FALSE)
...
write.csv(x = teamresults, file=output_teamresults, quote = TRUE, row.names = FALSE)

#teamresults<-read.csv(file=output_teamresults)

x<-teamresults %>% dplyr::select(-starts_with("cnt_"))
y<-teamresults %>% dplyr::select(GS=cnt_own_Goals,GC=cnt_opp_Goals)
xy<-data.frame(y, x)

write.csv(x = xy, file=output_features, quote = TRUE, row.names = FALSE)
