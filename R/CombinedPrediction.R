...
combined_data <- predictions_data%>%inner_join(point_data)
...
onematch <- onematch %>% filter(Prefix %in% c("smpt", "sp", "pspt", "pgpt", "pghb"))
#onematch <- onematch %>% filter(Prefix %in% c("sm", "smpi", "sk"))
# eliminate groups with only one sample
onematch <- onematch %>% group_by(pred, Prefix) %>% mutate(N=n()) %>% filter(N > 2) %>% ungroup() %>% mutate(legend_text=factor(paste(Prefix, pred, "-", N) )) 
matchname <- unique(paste(onematch$Team1, onematch$Team2, sep=" - "))
levels(onematch$legend_text)
nlevels(onematch$legend_text)
onematch %>% group_by(pred) %>% summarize(N=n()) %>% arrange(-N) 
car::dataEllipse(x=onematch$Train, y=onematch$Test, 
                 groups = onematch$legend_text, group.labels = levels(onematch$legend_text), 
                 col=brewer.pal(nlevels(onematch$legend_text),"Paired"), lwd = 2,
                 add=FALSE, robust = FALSE,
                 levels = c(0.6), 
                 fill=T, fill.alpha = 0.05*(log(onematch$N)),
                 main=matchname)
legend("topleft", legend=levels(onematch$legend_text), pch = 1:nlevels(onematch$legend_text), col=brewer.pal(nlevels(onematch$legend_text),"Paired"))
