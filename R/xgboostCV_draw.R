...
train_num<-xy
labels_num<-ifelse(train_num$GS==train_num$GC, 1, 0)
labels_factor<-as.factor(labels_num)
...
for (i in 1:1) {
  ...
  parameters<-data.frame(
    seed=eff_seed,
    gamma=5,  
    max_depth = 40, # sample(c(1,2,3,4,5,10),1), 
    eta = 0.03, # sample(c(0.1,0.01,0.001,0.003,0.03, 0.3),1), 
    subsample=0.6, #sample(c(0.3, 0.45, 0.5, 0.6, 0.8, 1.0),1), 
    colsample_bytree=0.4, #sample(c(0.1, 0.2, 0.5, 0.8),1),
    nparallel = 4,
    algo=2
  )
  print (parameters)
  ...
  for (fn in names(folds)){
    ...
    for (p in 1:parameters$nparallel){
      ...
      allScores<-train_num 
      allScores$oob<-FALSE
      allScores$oob[f]<-TRUE
      labels<-labels_num
      allgamesFillNA<-allScores %>% 
        # mutate(l1_diff=l1_GS-l1_GC, l1_oppDiff=l1_oppGS-l1_oppGC) %>% 
        select(-gameindex, -GS, -GC, -oob)        # , -season, -round -GS, -GC, 
      allgamesFillNA[is.na(allgamesFillNA <- allgamesFillNA)] <- -999
      
      xgbMatrix<-sparse.model.matrix(~.-1, data = allgamesFillNA)
      trainMatrix<-xgb.DMatrix(xgbMatrix[!allScores$oob,], label=as.integer(labels[!allScores$oob]), missing = -999)
      testMatrix <-xgb.DMatrix(xgbMatrix[allScores$oob,], label=as.integer(labels[allScores$oob]), missing = -999)
      watchlist<-list(train=trainMatrix, test=testMatrix)
      
      model_draw<-xgb.train(nrounds =100,  num_parallel_tree = 3,  early_stopping_rounds = 50, 
                        params = xgbparamsFactor, 
                        data = trainMatrix , 
                        print_every_n=1, verbose=1, 
                        nthread = 10 #, num_class=2
                        , watchlist = watchlist, objective=c("reg:logistic") #, "binary:logitraw", "reg:logistic", "binary:logistic"
      )
      printLearningCurve(model_draw)      
      printLearningCurve(model_draw, maximise = TRUE)      

      # inputs<-allScores %>% 
      #   mutate(l1_diff=l1_GS-l1_GC, l1_oppDiff=l1_oppGS-l1_oppGC) %>% 
      #   mutate(t_diffShots = t_Shots-t_oppShots,
      #          t_diffShots_where = t_Shots_where-t_oppShots_where,
      #          t_diffShotstarget = t_Shotstarget-t_oppShotstarget,
      #          t_diffShotstarget_where = t_Shotstarget_where-t_oppShotstarget_where
      #   ) %>%
      #   mutate(label = ifelse(GS==GC,1,0)) %>%
      #   select(-gameindex, -GS, -GC, -oob)         # , -season, -round -GS, -GC, 
      # trainMatrix<-inputs[-f,]
      # testMatrix<-inputs[f,]
      # 
      # model_draw<-cforest( label ~ ., data=trainMatrix %>%
      #                        select(label, mt_teamDraw_where, mt_oppTeamDraw_where)
      #                               #team, oppTeam, 
      #                               #starts_with("mt_team"), starts_with("mt_oppTeam"), 
      #                               #starts_with("t_diff"), 
      #                               #starts_with("l1"))
      #                               #contains("yellow"), starts_with("t_diff"))
      ...
      probtrain<-predict(model_draw, newdata=trainMatrix, type="prob")
      probtrain<-unlist(probtrain)
      cor(probtrain, labels[!allScores$oob])
      sm.density.compare(probtrain, labels[!allScores$oob], col=c("red", "black"), method="cv")
      
      probtest<-predict(model_draw, newdata=testMatrix, type="prob")
      probtest<-unlist(probtest)
      cor(probtest, labels[allScores$oob])
      sm.density.compare(x=probtest, group = labels[allScores$oob], col=c("red", "black"), method="cv")

      #vi<-sort(varimp(model_draw), decreasing = TRUE)
      #plot(vi)
      #vi
      
      cutoff<- (probtest>= 0.35)
      cor(probtest[cutoff], labels[allScores$oob][cutoff])
      sm.density.compare(x=probtest[cutoff], group = labels[allScores$oob][cutoff], col=c("red", "black"), method="cv")

      predtest<-predict(model_draw, newdata=testMatrix, type="response")
      #AUC::roc(prediction=predtest, labels=labels[allScores$oob])      
      predtest<-ifelse(predtest>=quantile(predtest, 0.75), 1, 0)
      confMatrix<-table(predtest, labels[allScores$oob])
      confMatrix
      confMatrix / rowSums(confMatrix)
      table(labels[allScores$oob]) / length(labels[allScores$oob])
      #plotImportance(model_draw, n=50, cols=colnames(trainMatrix))
      #printImportance(model_draw, n=ncol(trainMatrix), cols=colnames(trainMatrix))
      
      trainResult<-evaluateXgbFactorSimple(model_draw, trainMatrix, labels[!allScores$oob,], "train XGB objective function")
      ...
      testResult<-evaluateXgbFactorSimple(model_draw, testMatrix, labels[allScores$oob,], "test XGB objective function")
      ...
      result_rf_log<-read.csv(file = "xgb\\result_xgb_draw_log.csv")
      ...
      write.csv(result_rf_log, file = "xgb\\result_xgb_draw_log.csv", row.names = FALSE)
      ...
      printLearningCurve(model_draw, maximise = TRUE, cutoff=10, parameters[,-(2:6)])
      dev.off()
