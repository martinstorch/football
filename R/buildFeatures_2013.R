...
#download.file("http://www.football-data.co.uk/mmz4281/1718/D1.csv", "BL2017.csv")
...
write.csv(x = teamresults, file="teamresults_2013.csv", quote = TRUE, row.names = FALSE)

#teamresults<-read.csv(file="teamresults_2013.csv")

x<-teamresults %>% dplyr::select(-starts_with("cnt_"))
y<-teamresults %>% dplyr::select(GS=cnt_own_Goals,GC=cnt_opp_Goals)
xy<-data.frame(y, x)

write.csv(x = xy, file="BLfeatures_2013.csv", quote = TRUE, row.names = FALSE)
